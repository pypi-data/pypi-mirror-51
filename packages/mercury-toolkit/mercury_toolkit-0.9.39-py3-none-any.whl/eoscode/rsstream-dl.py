...
'''
Usage:  
    rsexport <db> <schema> --table <table> --bucket <s3_bucket> [--iam <iam_role>] --format=<fmt> [--delimiter=<delimiter>] [--directory=<directory>] 
    rsexport <db> <schema> --query <query> --bucket <s3_bucket> [--iam <iam_role>] --format=<fmt> [--delimiter=<delimiter>] [--directory=<directory>]
    rsexport <db> <schema> --query_template <template_file> --bucket <s3_bucket> [--iam <iam_role>] --format=<fmt> [--delimiter=<delimiter>] [--directory=<directory>]
    rsexport <db> <schema> --bucket <s3_bucket> [--iam <iam_role>] --format=<fmt> [--delimiter=<delimiter>] [--directory=<directory>]
'''
...
def generate_format_clause(format):
    return 'FORMAT AS %s' % format
...
def generate_iam_auth_clause(**kwargs):
    ...
    kwreader.read(**kwargs)
    return "iam role '{iam_role_arn}'".format(**kwargs)


@contextmanager
def connect_redshift(hostname, database, schema, username, password, ):
...
def main(args):
    ...
    target_uri = generate_s3_uri(args['<s3_bucket>'], args.get('--directory'))
