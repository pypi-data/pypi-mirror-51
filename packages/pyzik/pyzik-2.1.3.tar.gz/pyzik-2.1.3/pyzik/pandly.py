...
@ set_spy
def scatter1D(self,x='lamb',titre='',xlabel='',other_df=None,style_tracer='|'):
    self['__color'] = wavelength_to_rgb(self[x])
    self['__y']=0
    
    xyz=go.Scatter(x=self[x],y=self['__y'],marker=dict(size=4),
                               name=f"{ordonnee}/({nom})"[:50])
    quiver_fig = ff.create_quiver(self[x],0, 0, 1,
                       angle=0,
                       line=dict(width=1))
    xx_label = xlabel if x_label=='' else x
    layout= go.Layout(title= titre,xaxis= dict(title=xx_label ),yaxis=dict(title= ''))
    quiver_fig.add_trace(xyz)
    quiver_fig['layout'].update(layout)
    return iplot(quiver_fig, filename='LO')
...
@ set_spy
def draw_vectors(self,point=('&','&'),vector=('vx','vy'),unsur=1,scalvect=1,titre="sans titre",quadril_x=0,quadril_y=0,**d):
    ...
    if len(point) == len(vector) == 2:
        ...
        self.vector(x=x,y=y,ux=ux,uy=uy,unsur=unsur,scalvect=scalvect,titre=titre,quadril_x=quadril_x,quadril_y=quadril_y,xlabel=xlabel,ylabel=ylabel)
    elif len(point) == len(vector) == 3:
        x,y,z = point
        ux,uy,uz = vector
        xlabel,ylabel,zlabel = d.get('xlabel',''),d.get('ylabel',''),d.get('zlabel','')
        self.vector3D(x=x,y=y,z=z,ux=ux,uy=uy,uz=uz,unsur=unsur,scalvect=scalvect,titre=titre,quadril_x=quadril_x,quadril_y=quadril_y,xlabel=xlabel,ylabel=ylabel,zlabel=zlabel)
    else:
...
def vector(self,ux,uy,x='&',y='&',unsur=1,scalvect=1,titre="sans titre",quadril_x=0,quadril_y=0,xlabel='',ylabel='',**d):
    """
        Méthode/Fonction appliquée à une dataframe -> retourne la trajectoire avec des vecteurs.
        Conditions d'utilisation :
...
def vector3D(self,ux,uy,uz,x='&',y='&',z='&',unsur=1,scalvect=1,titre="sans titre",quadril_x=0,quadril_y=0,quadril_z=0,xlabel='',ylabel='',zlabel=''):
    """
        Méthode/Fonction appliquée à une dataframe -> retourne la trajectoire avec des vecteurs.
