...
from pandly import scatter2D,scatter3D,draw_vectors,histogram,scatterPolar
...
class Fplot:
    ...
    def plot(self,x,y,label='',**d):
        ...
        assert self.__kind == Fplot.__G2D,f"use add_plot2D only if u want draw 2D gfx, change ur kind type to: {Fplot.__G2D}"
    ...
    def polar(self,theta,r,label='',**d):
        ...
        assert self.__kind == Fplot.__P,f"use add_plot2D only if u want draw 2D gfx, change ur kind type to: {Fplot.__G2D}"
