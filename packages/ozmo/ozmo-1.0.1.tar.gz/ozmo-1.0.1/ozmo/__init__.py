...
class SpotArea(Clean):
    def __init__(self, action='start', speed='normal', area='', map_position='', cleanings='1'):
        if area != '': #For cleaning specified area
            super().__init__('spot_area', FAN_SPEED_TO_ECOVACS[speed], act=CLEAN_ACTION_TO_ECOVACS[action], mid=area)
        elif map_position != '': #For cleaning custom map area, and specify deep amount 1x/2x
            super().__init__('spot_area', FAN_SPEED_TO_ECOVACS[speed], act=CLEAN_ACTION_TO_ECOVACS[action], p=map_position, deep=cleanings)
        else:
            #no valid entries
