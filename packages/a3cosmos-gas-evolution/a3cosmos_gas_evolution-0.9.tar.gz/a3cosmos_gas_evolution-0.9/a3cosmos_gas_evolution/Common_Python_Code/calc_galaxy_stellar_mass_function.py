...
def calc_Mstar_integrating_CSFRD_dzliu2018(z):
    ...
    for i in range(len(z_list)-1):
        ...
        t_bin = cosmo.age(z_list[i+1]).value # Gyr, time since the Big Bang
        time_bin = cosmo.age(z_list[i+1]).value - cosmo.age(z_list[i]).value # Gyr, time interval within the redshift bin
        ...
        Mstar_formed = CSFRD[i] * time_bin * 1e9 # Msun/yr * Gyr * 1e9 => Msun
...
def calc_SMF_dzliu2018(z=None, lgMstar=None, galaxy_type='SFG', z_list=None):
    # 
    ...
    for i in range(len(z_list)-1):
        ...
        # 
        # 
        # then re-normalize SMF to total stellar mass integrated from CSFRD
        Mstar_cumulated = calc_Mstar_integrating_CSFRD_dzliu2018(z_bin)
        #CSFRD_at_z_bin = calc_MadauDickinson2014_CSFRD(z_bin)
        Schechter_P_int = (Schechter_P_QG+Schechter_P_SFG)*Schechter_M
        Schechter_M_total = sum(Schechter_P_int*Schechter_Mstep) # *Schechter_Mstep to account for Psi unit [dex-1]
        Schechter_P_SFG = Schechter_P_SFG * (Mstar_cumulated / Schechter_M_total)
        Schechter_P_QG = Schechter_P_QG * (Mstar_cumulated / Schechter_M_total)
        Schechter_P_ALL = Schechter_P_SFG + Schechter_P_QG
        print('z = %.04f, lgMstar_CSFRD = %0.2f, lgMstar_SMF = %0.2f, renorm = %s'%(z_bin, np.log10(Mstar_cumulated), np.log10(Schechter_M_total), (Mstar_cumulated / Schechter_M_total)))
    ...
    if z is None:
        ...
        #print(np.min(SMF_z_list), np.max(SMF_z_list), z)
