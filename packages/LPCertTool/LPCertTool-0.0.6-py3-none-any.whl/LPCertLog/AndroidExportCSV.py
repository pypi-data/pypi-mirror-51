...
EVENT_TAG = "/GenericSendEvent: jsondata" # audioplayer speak
"i/SpeechSendAudio: SpeechSendEvent" #recognize

PlaybackStarted = "name\":\"PlaybackStarted"
ProgressReportDelayElapsed = "name\":\"ProgressReportDelayElapsed"
ProgressReportIntervalElapsed = "name\":\"ProgressReportIntervalElapsed"
PlaybackStopped = "name\":\"PlaybackStopped"
PlaybackNearlyFinished = "name\":\"PlaybackNearlyFinished"
PlaybackFinished = "name\":\"PlaybackFinished"
RecognizeIdentify = "i/SpeechSendAudio: SpeechSendEvent : jsondata"
SpeechFinished = "SpeechFinished complete"
SpeechStarted = "name\":\"SpeechStarted"
EventResponse = "[AVS] URLSession http response events:"
PlaybackStutterStarted = "name\":\"PlaybackStutterStarted"
PlaybackStutterFinished = "name\":\"PlaybackStutterFinished"
StopCapture = "SpeechRecognizer\",\"name\":\"StopCapture"
Stop = "AudioPlayer\",\"name\":\"Stop"
PlayPrepareAsync = "play prepareAsync"
RecognizerComplete = "SpeechSendAudio : audioRequest         complete"
...
class ParseEventObj:
    ...
    def parse(self):
        ...
        microsecond = dates[-1]
        self.date = self.date.replace(":%s" % microsecond, ".%s" % microsecond)
    ...
    def parseRecognizer(self):
        self.parse()
        audioplayer_m = re.search(r'context.*?\"namespace\":\"AudioPlayer\"', self.log)
        audioplayer = audioplayer_m.group(0)
        offset_m = re.search(r'(?<=offsetInMilliseconds":).*?(?=,)', audioplayer)
        self.offset = str(offset_m.group(0))
...
class AndroidExportCSV:
    ...
    def exportReport(self, logs):
        ...
        path = 'Temp/Android_%s.xlsx' % timeStr
...
if __name__ == "__main__":
    ...
    if len(filePath) > 0:
        ...
        reportPath = androidExport.exportReport(events)
        print(reportPath)
