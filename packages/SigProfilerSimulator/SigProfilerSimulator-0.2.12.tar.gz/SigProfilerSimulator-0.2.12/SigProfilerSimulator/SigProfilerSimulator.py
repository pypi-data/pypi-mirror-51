...
def SigProfilerSimulator (project, project_path, genome, contexts, exome=None, simulations=1, updating=False, bed_file=None, overlap=False, gender='female', seqInfo=False, chrom_based=False, seed_file=None, spacing=1, noisePoisson=False, noiseAWGN=False):
	...
	# 	mut_start = 1
	# 	mut_save = 4
	# elif context == 'DINUC' or 'DBS' in context:
	# 	sim = 5
	# 	mut_start = 0
	# 	mut_save = 0
	# elif context == 'INDEL' or 'ID' in context:
	...
	for i in range (0, len(chromosomes_parallel), 1):
		...
		r = pool.apply_async(simScript.simulator, args=(sample_names, mut_dict_parallel, chromosome_string_path, tsb_ref, tsb_ref_rev, simulations, seeds[i], output_path, updating, chromosomes_parallel[i], project, genome, bed, bed_file, contexts, exome, overlap, project_path, seqInfo, log_file, spacing, noisePoisson, noiseAWGN))
		results.append(r)
	pool.close()
	pool.join()
