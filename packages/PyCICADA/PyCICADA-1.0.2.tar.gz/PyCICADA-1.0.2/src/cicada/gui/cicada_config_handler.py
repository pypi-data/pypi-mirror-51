...
class ConfigHandler:
    ...
    def __init__(self, path_to_config_file="../config/config.yaml"):
        ...
        self._main_window_bg_pictures_folder = config_dict.get('main_window_bg_pictures_folder', None)
        self._main_window_bg_pictures_file_names = []
        if self._main_window_bg_pictures_folder is not None:
            # we list the pictures
            if os.path.isdir(self._main_window_bg_pictures_folder):
                file_names = []
                for (dirpath, dirnames, local_filenames) in os.walk(self._main_window_bg_pictures_folder):
                ...
                for file_name in file_names:
                    if (file_name.lower().endswith(".png") or file_name.lower().endswith(".jpg") or
                        ...
                        self._main_window_bg_pictures_file_names.append(os.path.join(
                            self._main_window_bg_pictures_folder, file_name))
    ...
    def get_random_main_window_bg_picture(self, widget_id=None):
        ...
        Args:
            ...
            could be: "sessions", "tree", "overwiew"

        Returns:

        """
        if len(self._main_window_bg_pictures_file_names) == 0:
            return None
        pic_index = randint(0, len(self._main_window_bg_pictures_file_names) - 1)
        return self._main_window_bg_pictures_file_names[pic_index]
