	...
	def convert(self, df, how='one_to_one', remove_duplicates=None): 
		...
		if how.startswith('one_to'): 
			...
			if remove_duplicates == 'mean': 
				...
				mapped_df = mapped_df.sort_values('row_sum', ascending=False).drop(columns=['row_sum']).drop_duplicates(keep='first')

			elif remove_duplicates == 'na_count': 
				mapped_df['na_count'] = mapped_df.isna().sum(axis=1)
				mapped_df = mapped_df.sort_values('na_count').drop(columns=['na_count']).drop_duplicates(keep='first')

			else: 
				mapped_df = mapped_df.drop_duplicates(keep='first')

			assert mapped_df.index.is_unique
