...
def list_users():
    ...
    try:
        ...
        logging.error("Invalid credentials provided. \n Use `okta configure` command to configure credentials")
    except Exception as exc:
        logging.info("Exception occured: ", exc_info=exc)

@click.option('--password', prompt='User\'s password', help='User\'s password')
...
def create_user(password, number, email, last_name, first_name):
    base_url, headers = get_api_info()
