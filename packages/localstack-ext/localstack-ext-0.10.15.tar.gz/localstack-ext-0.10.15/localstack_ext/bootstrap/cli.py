...
def import_auth():
 try:
  ...
  raise wvuBz('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=wvuBY)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=wvuBk)
def get_params(args,names=[],mandatory=wvuBY):
 result=()
 for name in names:
  value=wvuBL
  if name in args:
  ...
  if not value and mandatory:
   raise wvuBz('Please provide %s=... parameter'%name)
  result+=(value)
 if wvuBh(result)==1:
  return result[0]
 return result
