...
def import_auth():
 try:
  ...
  raise sUfTg('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=sUfTE)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=sUfTi)
def get_params(args,names=[],mandatory=sUfTE):
 result=()
 for name in names:
  value=sUfTt
  if name in args:
  ...
  if not value and mandatory:
   raise sUfTg('Please provide %s=... parameter'%name)
  result+=(value)
 if sUfTk(result)==1:
  return result[0]
 return result
