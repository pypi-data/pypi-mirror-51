...
def import_auth():
 try:
  ...
  raise jSKJY('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=jSKJx)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=jSKJr)
def get_params(args,names=[],mandatory=jSKJx):
 result=()
 for name in names:
  value=jSKJz
  if name in args:
  ...
  if not value and mandatory:
   raise jSKJY('Please provide %s=... parameter'%name)
  result+=(value)
 if jSKJN(result)==1:
  return result[0]
 return result
