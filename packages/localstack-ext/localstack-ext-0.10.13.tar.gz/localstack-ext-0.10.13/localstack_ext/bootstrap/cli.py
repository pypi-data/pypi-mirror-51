...
def import_auth():
 try:
  ...
  raise OiAsd('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=OiAsm)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=OiAso)
def get_params(args,names=[],mandatory=OiAsm):
 result=()
 for name in names:
  value=OiAsT
  if name in args:
  ...
  if not value and mandatory:
   raise OiAsd('Please provide %s=... parameter'%name)
  result+=(value)
 if OiAsP(result)==1:
  return result[0]
 return result
