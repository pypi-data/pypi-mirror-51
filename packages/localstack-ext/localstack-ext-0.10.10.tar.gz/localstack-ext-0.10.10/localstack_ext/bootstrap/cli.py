...
def import_auth():
 try:
  ...
  raise RzBoK('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=RzBoW)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=RzBon)
def get_params(args,names=[],mandatory=RzBoW):
 result=()
 for name in names:
  value=RzBoU
  if name in args:
  ...
  if not value and mandatory:
   raise RzBoK('Please provide %s=... parameter'%name)
  result+=(value)
 if RzBox(result)==1:
  return result[0]
 return result
