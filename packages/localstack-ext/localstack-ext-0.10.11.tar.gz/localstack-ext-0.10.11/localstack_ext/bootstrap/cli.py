...
def import_auth():
 try:
  ...
  raise roVuK('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=roVuX)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=roVuU)
def get_params(args,names=[],mandatory=roVuX):
 result=()
 for name in names:
  value=roVud
  if name in args:
  ...
  if not value and mandatory:
   raise roVuK('Please provide %s=... parameter'%name)
  result+=(value)
 if roVuP(result)==1:
  return result[0]
 return result
