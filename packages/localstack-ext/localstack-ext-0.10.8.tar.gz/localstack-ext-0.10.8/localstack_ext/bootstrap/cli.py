...
def import_auth():
 try:
  ...
  raise CdKlE('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=CdKlw)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=CdKlB)
def get_params(args,names=[],mandatory=CdKlw):
 result=()
 for name in names:
  value=CdKlf
  if name in args:
  ...
  if not value and mandatory:
   raise CdKlE('Please provide %s=... parameter'%name)
  result+=(value)
 if CdKlv(result)==1:
  return result[0]
 return result
