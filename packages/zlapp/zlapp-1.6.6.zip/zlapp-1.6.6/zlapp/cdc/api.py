...
def pre_meta():
    conp_app=['gpadmin','since2015','192.168.4.206','biaost','public']
    conp_gp=['gpadmin','since2015','192.168.4.183:5433','base_db','etlmeta']
    sql="""
    with a as (select  
...
def add_quyu_app(quyu,loc="aliyun"):
    ...
    sql="update etl.t_html_key set max_gg_meta=%d where quyu='%s' "%(max_html_key1,quyu)
    ...
    if max_html_key2 is  None:
        ...
        sql="update etl.t_html_key set max_gg=%d where quyu='%s' "%(max_html_key2,quyu)
        db_command(sql,dbtype='postgresql',conp=conp_gp)
    ...
    if max_html_key3 is  None:
        ...
        sql="update etl.t_html_key set max_gg_zhongbiao=%d where quyu='%s' "%(max_html_key3,quyu)
        db_command(sql,dbtype='postgresql',conp=conp_gp)
        print("更新后最大html_key :",max_html_key3)
