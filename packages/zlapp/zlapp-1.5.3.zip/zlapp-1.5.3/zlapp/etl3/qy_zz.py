#企业from lmf.bigdata import pg2csv ,pg2pg
...
def ptb3():
    ...
    SELECT href
    ,jianzhu.json_kv(unnest(pageresult),'zzbh') as zzbh
    ,jianzhu.json_kv(unnest(pageresult),'gsd') as gsd
    ,jianzhu.json_kv(unnest(pageresult),'jglx') as jglx
    ,jianzhu.json_kv(unnest(pageresult),'zzmc') as zzmc
    ,jianzhu.json_kv(unnest(pageresult),'bgdate') as bgdate
    ,jianzhu.json_kv(unnest(pageresult),'eddate') as eddate
    ,jianzhu.json_kv(unnest(pageresult),'fbjg') as fbjg
    ,jianzhu.json_kv(unnest(pageresult),'tydm') as tydm
    ,jianzhu.json_kv(unnest(pageresult),'fddbr') as fddbr
    ,jianzhu.json_kv(unnest(pageresult),'zzlb') as zzlb
    ,jianzhu.json_kv(unnest(pageresult),'entname') as entname
    ,jianzhu.json_kv(unnest(pageresult),'jgdz') as jgdz
    ,jianzhu.json_kv(unnest(pageresult),'qita') as qita
    into jianzhu.qyzz 
...
def est():
    ...
    sql="""
    drop table if exists etl.qyzz;
    select * into etl.qyzz from jianzhu.qyzz;
    """
    db_command(sql,dbtype="postgresql",conp=conp_hawq)
