...
def est2():
    ...
    sql="truncate table public.app_qy_zz"

    db_command(sql,dbtype="postgresql",conp=conp_pg)

    sql="""
    insert into public.app_qy_zz
    with a as (SELECT  ent_key, entname,zzlb,zzmc,zzbh,zzcode,xzqh,fddbr,alias  FROM "public"."qy_zz" where ent_key is not null )
    ,b as (select jgmc,clrq,zczj,logo from public.qy_base)

    ,c as (select zhongbiaoren,fabu_time[1] as fabu_time,zhongbiao_counts from public.qy_zhongbiao )

    select 
    a.*,b.clrq,b.zczj,b.logo
