...
def est_gg_meta_quyu(quyu,conp_gp):
    ...
    sql="""
    CREATE TABLE if not exists etl.gg_meta_%s (
    html_key bigint,
    guid text,
    gg_name text,
    href text,
...
def write_meta(conp_app,conp_gp):
    ...
    sql="""SELECT quyu,max(html_key) as max_html_key FROM "public"."gg_meta" 
    group by quyu
    """
...
def pre_quyu_cdc(quyu,conp_gp):
    ...
    sql1="insert into etl.gg_meta_%s select * from dst.gg_meta where quyu='%s' and html_key>%d "%(quyu,quyu,max_html_key)
