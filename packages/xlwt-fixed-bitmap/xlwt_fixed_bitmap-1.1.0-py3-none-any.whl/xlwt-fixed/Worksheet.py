...
from xlwt import Style, BIFFRecords, Row, Column
...
class Worksheet(object):
    ...
    .. warning::
      ...
      They are returned from calls to :meth:`~xlwt_fix.Workbook.Workbook.add_sheet`.
    """
    ...
    def __init__(self, sheetname, parent_book, cell_overwrite_ok=False):
        ...
        # and previous versions of xlwt_fix.
        #   if panes_frozen:
    ...
    def write(self, r, c, label="", style=Style.default_style):
        ...
        :param label:
           ...
           An :class:`xlwt_fix.Formula` instance causes an Excel formula to be
           written.

        :param style:

           A style, also known as an XF (extended format), is an
           :class:`~xlwt_fix.Style.XFStyle` object, which encapsulates the
           formatting applied to the cell and its contents.

           :class:`~xlwt_fix.Style.XFStyle` objects are best set up using the
           :func:`~xlwt_fix.Style.easyxf` function. They may also be set up by
           ...
           :class:`~xlwt_fix.Style.XFStyle` object.
        """
