...
class PropertyRemapping:
    ...
    def _get_merged_property_data(self, offer_id):
        ...
        2. From Offer --> Additional information
        3. From Property --> Offer location information
        4. From Property --> The root of the property object not including location or verified information.
        5. From Offer --> User provided information
        ...
        for key in property_item:
            if key == "id":
                # Do not merge in the property record's ID value.
                continue

            value = property_item.get(key)
            if isinstance(value, dict):
                # It is a nested dictionary. Skip it because it is probably `verifiedInfo` and `location`.
                continue
            base_property_dict[key] = value
        ...
        # 5. Merge in the user provided property information
        combined_property_dict.update(user_supplied_property_dict)

        # 4. Merge in the root of the information in the Property table
        combined_property_dict.update(base_property_dict)
