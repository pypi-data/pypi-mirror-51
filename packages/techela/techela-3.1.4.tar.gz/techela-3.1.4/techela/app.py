...
if os.path.exists(os.path.join(COURSEDIR, 'local-course-data.json')):
    with open(os.path.join(COURSEDIR, 'local-course-data.json')) as f:
...
def open_lecture(label):
    ...
    cmd = ["jupyter", "notebook", "--browser=chrome", mname]
    print(cmd)
...
def open_course_lecture(label):
    ...
    # Now open the notebook.
    cmd = ["jupyter", "notebook", "--browser=chrome", fname]
    subprocess.Popen(cmd, stdout=subprocess.PIPE,
                     stderr=subprocess.PIPE,
                     stdin=subprocess.PIPE)
...
def new_notebook():
    CWD = os.getcwd()
    os.chdir(COURSEDIR)
    cmd = ["jupyter", "notebook", "--browser=chrome"]
    subprocess.Popen(cmd, stdout=subprocess.PIPE,
                     stderr=subprocess.PIPE,
                     stdin=subprocess.PIPE)
    os.chdir(CWD)
...
def grade(andrewid, label):
    ...
    # Now open the notebook.
    cmd = ["jupyter", "notebook", "--browser=chrome", GFILE]
    subprocess.Popen(cmd, stdout=subprocess.PIPE,
                     stderr=subprocess.PIPE,
                     stdin=subprocess.PIPE)
...
def open_for_grading(label, n=5):
    ...
    for i in range(min(n, len(ungraded))):
        ...
        cmd = ["jupyter", "notebook", "--browser=chrome", ungraded[i]]
        print(f'Running "{cmd}")')
        subprocess.Popen(cmd, stdout=subprocess.PIPE,
                         stderr=subprocess.PIPE,
                         stdin=subprocess.PIPE)
