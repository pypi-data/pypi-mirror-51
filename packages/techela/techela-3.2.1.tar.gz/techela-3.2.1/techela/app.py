...
if os.path.exists(os.path.join(COURSEDIR, 'local-course-data.json')):
    with open(os.path.join(COURSEDIR, 'local-course-data.json'), encoding="utf-8") as f:
...
def get_course_data():
    ...
    if os.path.exists('announcements.html'):
        with open('announcements.html', encoding="utf-8") as f:
...
TODO: this is not graceful if the repo is not clean.'''
    pwd = os.getcwd()
    print('Updating the course repo with git pull')
    try:
        os.chdir(COURSEDIR)
