...
#	CONST_CULL_METHOD_EQUAL_SEX_RATIOS="equal_sex_ratio"
	...
	def __get_config( self ):
		...
		'''
		20190610.  In prep for submiting a programming note, Brian asked me whether the sim that uses
		gamma distribution of repro prob was or was not in our program (though in Tiago's code).  My check
		shows that I did not disable it correctly so that, before this input checkis written, a conf file
		that has gamma distribution istsz for male and/or female does not load in the GUI (i.e. an error is
		thrown, but is uninformative), but in the pgdrivesimulation.py terminal program, the program loads
		the conf and executes.  Here, we head off any execution with a conf that has gamma lists, so that 
		we can both deny it in the terminal program, and give an informative error message:
		'''

		if config.has_option("pop", "gamma.b.male"):
			s_msg="In PGInputSimuPop instance, def __get_config, " \
					+ "the program does not accept gamma distribuiion lists, " \
					+ "as the code that simulates populations using gamma distribution " \
					+ "reproductive probability lists is not supported in the current version."  \

			raise Exception( s_msg )
	
			##### rem out, as no longer wanted, given we have raised an exception 
			#to prevent any execution of a sim using gamma listllys
			'''
