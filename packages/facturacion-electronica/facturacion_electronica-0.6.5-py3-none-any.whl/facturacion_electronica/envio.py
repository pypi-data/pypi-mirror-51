...
{1}</LibroGuia>'''.format(xsd, doc)
    ...
    def envio_recep(self):
        caratula = self.RecepMer.caratula_recep
        recibos = self.RecepMer.Recibos
...
xsi:schemaLocation="http://www.sii.cl/SiiDte RespuestaEnvioDTE_v10.xsd" >
    ...
    def do_receipt_deliver(self):
        ...
        for r in self.Recepciones:
            ...
            resp = {
                ...
                'respuesta': 'recepcion_envio_%s_%s' %
    ...
    def do_recep_merc(self):
        for r in self.RecepMer.DTEs:
            self.recibo(r)
            self.firmar(
                'Recibo',
                'recep')
            self.RecepMer.Recibos = self.sii_xml_request
        self.envio_recep()
        self.firmar(
                'SetDteRecibidos',
                'env_recep'
        ...
        return {
                'respuesta': self.sii_xml_request,
            }

    def do_validar_com(self):
        for r in self.ValidacionCom.DTEs:
            self.resultado(r)
            self.ValidacionCom.Resultados = self.sii_xml_request
