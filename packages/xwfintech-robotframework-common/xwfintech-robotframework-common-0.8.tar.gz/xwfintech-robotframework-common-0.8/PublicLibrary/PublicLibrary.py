...
class PublicLibrary(object):
    ...
    def get_dict_list(self,data, dict_list):
        '''
        解析嵌套加入字典
        :param data:
        :param dict_list:
        ...
        if isinstance(data, list):
            d_dict = [d for d in data if isinstance(d, dict)]
            for d in d_dict:
                self.get_dict_list(d, dict_list)
    ...
    @keyword('Get Results')
    def get_results(self,data, key: str):
        '''
        嵌套列表取值：列表,字段
        :param key:
        :return:
        ...
        results = []
        contancs = []
        self.get_dict_list(data, contancs)
        # print(len(contancs))
        for a in contancs:
            result = a.get(key)
    ...
    def get_testfile_name(self,file_patah):
        '''
        给定一个路径取其下面的文件名
        :param file_patah:
        :return:
    ...
    @keyword('Filling Excel')
    def filling_excel(self,file_patah):
        '''
        Excel文件补齐，如果只有一行表头则补齐第二行，如果是多列多行则补齐空值，需要给定文件路径
        :param file_patah:
        :return:
        '''
        os.chdir(file_patah)
        excel_test = xlrd.open_workbook(self.get_testfile_name(file_patah))
        mySheet = excel_test.sheets()[0]

        data_list = []

        workbook = xlrd.open_workbook(self.get_testfile_name(file_patah))
        workbooknew = copy(workbook)
        ws = workbooknew.get_sheet(0)

        if mySheet.nrows > 1:
            for i in range(mySheet.nrows):
                for a, b in enumerate(mySheet.row_values(i)):
                    if b == '':
                        lin_list.append(i)
                        data_list.append([a, i])
            for c in data_list:
                ws.write(c[1], c[0], label=random.randint(1, 100000000000))
            workbooknew.save(self.get_testfile_name(file_patah))
        elif mySheet.nrows == 1:
            for h in range(mySheet.ncols):
                ws.write(1, h, label=random.randint(1, 100000000000))
            workbooknew.save(self.get_testfile_name(file_patah))