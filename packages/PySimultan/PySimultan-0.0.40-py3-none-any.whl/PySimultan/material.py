    ...
    def calc_g_value(self):
        ...
        for layer in self.Layers:
            if layer.Transparent:
                g_value = g_value * (1/layer.Absorption)
            else:
                g_value = 0
...
class Window(Part):
    ...
    def __init__(self,
        ...
        :param g_value: solar transmittance through the hole part:
        ...
        self._Rho_e = None
        self._Alpha_e = None
        self._Q_i = None,  # secondary heat dissipation to inside
        self._Tau_uv = None,

        self.OriginalFace = face
        self.Q_i = q_i,  # secondary heat dissipation to inside
        self.Tau_uv = tau_uv,
        self.U_g = u_g
        self.U_f = u_f
        self.G_Value = g_value
    ...
    def create_window_part(self,
        ...
        window_glazing_material = Material(name='WindowGlazingMaterial1_{}'.format(self.Name),
                                           ...
                                           transparent=True,
                                           solar_absorption_coefficient=solar_absorption_coefficient,
                                           absorption_coefficient=self.G_Value**(1/self.NumberOfGlazings),
                                           wvdrf=999999,
                                           w20=0,
                                           w80=0)
    ...
    def calc_absorption(self):
        ...
        if isinstance(self.Material, list):
            if self.Material[0].Transparent:
                self.Absorption = np.exp(self.Material[0].AbsorptionCoefficient * self.Thickness)
            else:
                self.Absorption = 99999
        else:
