...
class BaseDTSelector(BaseDialog):
    ...
    def __init__(self, pw_parent, **kw):
        ...
        if self._dtselected and not self._truncated:
            self._chh.set('%02d' % self._dtselected.hour)
            self._cmm.set('%02d' % self._dtselected.minute)
            self._css.set('%02d' % self._dtselected.second)
    ...
    def show(self, **kw):
        ...
        if not self._truncated:
            ...
            lw_thhlist = ttkCombobox(lw_tcontrol,
                                     values=tuple(['%02d' % x for x in range(0, 24)]),
            ...
            lw_tmmlist = ttkCombobox(lw_tcontrol,
                                     values=tuple(['%02d' % x for x in range(0, 60)]),
...
def run_test():
    ...
    ld_params['title'] = 'Select date/time'
    ld_params['truncated'] = False
    ld_params['selected'] = datetime.datetime.today() - datetime.timedelta(days=4)
    lo_dialog = BaseDTSelector(root,
                               **ld_params)
    print(lo_dialog.show())
