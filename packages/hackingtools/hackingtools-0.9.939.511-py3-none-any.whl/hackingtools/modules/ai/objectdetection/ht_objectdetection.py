...
class StartModule():
	...
	def saveCroppedImage(self, img_path, coords, model_path, name, counter=1):
		...
		try:
			...
			cropped_file = os.path.join(output_dir, n2.split('.')[0], name.decode("UTF-8"), 'cropped_{c}_{n}'.format(c=counter, n=n))
			print(cropped_file)
	...
	def show_prediction_labels_on_image(self, img_path, predictions, model_path):
		...
		for name, (top, right, bottom, left) in predictions:
			...
			if 'unknown' == str(name):
				...
				print('There is an unknown object... Should be blur on newer version')
	...
	def predictFromZip(self, zipFile, model_path='trained.clf', first_folder_name=None, trainZipFile=None, first_folder_name_training_zip=None, n_neighbors=1):
		if trainZipFile:
			...
			model_path = self.trainFromZip(zipFile=trainZipFile, new_model_name='{f}.{clf}'.format(f=first_folder_name_training_zip, clf=config['models_tests_extension']), first_folder_name=first_folder_name_training_zip, n_neighbors=int(n_neighbors))
		else:
		...
		if not first_folder_name:
			_, first_folder_name = os.path.split(zipFile)
		...
		for imageFile in os.listdir(folder):
			if not os.path.isdir(os.path.join(folder, imageFile)):
				images.append(self.predictImage(imageFile, model_path))
		
		return unzipper.zipFiles(images)
