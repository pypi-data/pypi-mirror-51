...
def getCombinationPosibilitiesLength(alphabet, length):
    longitud = len([''.join(x) for x in product(config_utils[alphabet], repeat=length)])
    Logger.printMessage(message='getCombinationPosibilitiesLength', description=longitud, color=Fore.YELLOW)
    return longitud

def getDict(length=8, maxvalue=10000, alphabet='lalpha', consecutive=False):
    res = []
    maxvalue = getCombinationPosibilitiesLength(alphabet=alphabet, length=length)
    Logger.printMessage(message='getDict', description='{data} - {count}'.format(data=res[:10], count=maxvalue), debug_core=True)
