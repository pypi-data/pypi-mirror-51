...
class StartModule():
	...
	def trainFromZip(self, zipFile, new_model_name='new_trained.clf', first_folder_name='model', model_count=1):
		...
		classifier = self.train(os.path.join(output_dir, first_folder_name), model_save_path=new_model, n_neighbors=model_count)
		return new_model
	
	def test_image(self, imageFile, model_path='trained.clf', trainZipFile=None, first_folder_name=None):
		if trainZipFile:
			if not first_folder_name:
				first_folder_name = imageFile.split('.')[0]
			model_path = self.trainFromZip(trainZipFile, first_folder_name=first_folder_name)
