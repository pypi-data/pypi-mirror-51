...
class StartModule():
	...
	def show_prediction_labels_on_image(self, img_path, predictions):
		...
		for name, (top, right, bottom, left) in predictions:
			...
			crop_img = image[right:right + left, top:top + bottom]
			new_path_name = ''
			path, n = os.path.split(img_path)
			cv2.imwrite(os.path.join(path, 'cropped_{n}'.format(n=n)), crop_img)
			...
			if 'unknown' == str(name):
				...
				pil_image[right:right+crop_img.shape[0], top:top+crop_img.shape[1]] = cv2.GaussianBlur(crop_img,(23, 23), 30)
