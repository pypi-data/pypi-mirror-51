...
class StartModule():
	...
	def train(self, train_dir, model_save_path=None, n_neighbors=None, knn_algo='ball_tree', verbose=False):
		...
		"""
		Logger.printMessage(message="train", description="Training directory: {d}".format(d=train_dir))
		X = []
		y = []
		...
		if n_neighbors is None:
			...
			if verbose:
				Logger.printMessage(message="train", description="Chose n_neighbors automatically: {d}".format(d=n_neighbors))
	...
	def show_prediction_labels_on_image(self, img_path, predictions):
		...
		:return:
		"""
		Logger.printMessage(message="show_prediction_labels_on_image", description="Testing: {d}".format(d=img_path))
	...
	def test_image(self, imageFile, model_path='trained.clf', trainZipFile=None, first_folder_name=None, n_neighbors=1):
		if trainZipFile:
			...
			Logger.printMessage(message="test_image", description="Model saved as {f}".format(f=model_path))
		else:
