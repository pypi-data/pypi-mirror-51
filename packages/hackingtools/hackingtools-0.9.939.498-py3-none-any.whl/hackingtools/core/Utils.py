...
def getDict(length=8, maxvalue=10000, alphabet='lalpha', consecutive=False):
    ...
    if ('numeric' in alphabet and consecutive) or consecutive:
        res.sort()
    Logger.printMessage(message='getDict', description='{data} - {count}'.format(data=res[:10], count=len(values)), debug_core=True)
    return res
