...
def load_remote_skills_data(branch, path):
...
def load_skills_data(branch, path, remote_update):
    if remote_update:
        try:
            return load_remote_skills_data(branch, path)
...
class SkillRepo(object):
    def __init__(self, path=None, url=None, branch=None, remote_update=True):
