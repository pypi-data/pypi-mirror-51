    ...
    def as_tensor(self, padding_length: Dict[str, int]) -> torch.Tensor:
        ...
        if self._pad_on_left:
            padded_indexed_tokens = torch.cat([torch.ones([n_padded_elements], dtype=torch.long)*self._pad_token_index,
                                               torch.LongTensor(self._post_processing_of_indexed_tokens(self._indexed_tokens))])
        else:
            padded_indexed_tokens = torch.cat([torch.LongTensor(self._post_processing_of_indexed_tokens(self._indexed_tokens)),
                                               torch.zeros([n_padded_elements], dtype=torch.long)*self._pad_token_index])
