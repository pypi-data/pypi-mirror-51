...
from arps.core.environment import (list_registered_policies,
                                   load_policy)
...
class SimulatorEnvironmentAgentManager(AgentManager):
    ...
    @tracked_action
    async def spawn_agent(self, *, policies: List[Any],
                          period: int = None) -> JSONType:
        '''
        Spawn agent with default policies:
        - Info: client can request agent capabilities
        - TouchPoint: client can read sensor/actuator current state
        - Meta: client can modify policy (add or remove)

        Args:
        - policies: A user policy is required. Available policies can be listed using policy_repository()
        or looking at the configuration file
        - period: if a policy need to check a condition every X epochs, this parameter must be specified
        '''
        try:
            if not policies:
            ...
            try:
                agent = self._agent_factory(policies, period=period)
    ...
    def _agent_factory(self, policies, period):
        agent_id = self.agent_id_manager.next_available_id()

        policy_loader = partial(load_policy, self.identifier)
        agent = build_agent(self.agent_factory, policy_loader, agent_id, policies, period, self.clock)
    ...
    async def policy_repository(self) -> JSONType:
        return list_registered_policies(self.identifier)
