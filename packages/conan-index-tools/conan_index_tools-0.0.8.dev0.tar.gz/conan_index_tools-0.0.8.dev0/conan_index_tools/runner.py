...
from conan_index_tools import HOOKS_URL
...
def run_create(index_path, ref, profile_name, profile_url, docker=False):
    ...
    if docker and ((settings and settings["os_build"] == "Linux") or
       (not settings and cur_os == "Linux")) and docker_present():
        ...
        hooks = " && conan config install {} -sf hooks -tf hooks &&" \
                " conan config set hooks.conan-center".format(HOOKS_URL)

        inside = "pip uninstall -y conan-package-tools && " \
                 "pip install conan --upgrade && " \
                 "rm -rf ./recipe/test_package/build" + hooks + create
        ...
        cmd = "conan create . {}@{} --profile {}".format(ref, USER_CHANNEL,  profile_path)
