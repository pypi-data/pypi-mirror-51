...
from conan_index_tools.index_storage import IndexStorage
...
def command_new_version(storage, name, version, folder, url):
    ...
    return storage.recipe_folder(name, folder), version
...
def new_recipe(base_path, name, version, folder, url):
    ...
    if not os.path.exists(storage.package_folder(name)):
        os.mkdir(storage.package_folder(name))
    ...
    recipe_folder, version = command_new_version(storage, name, version, folder, url)

    print("""All done!
    
- Edit your conanfile.py here: '{}'
- Create or modify a 'test_package' folder with a small program testing your package. 
  More info here: https://docs.conan.io/en/latest/creating_packages/getting_started.html
- You can test your recipe running: 
     $ cit run . {}/{} [--profile XXX] [--profile_url YYY]
""".format(recipe_folder, name, version))
