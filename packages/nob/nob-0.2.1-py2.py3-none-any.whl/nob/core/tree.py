...
class _TreeUtils:
    ...
    def __setattr__(self, name, value):
        ...
        if name in self.__dict__:
            ...
            raise AttributeError("Don't set val, set the tree directly, i.e.:\n"
                                 "  tree.key = 'value'  (and not tree.key.val = 'value')")
        else:
            paths = self.find(name)
            if len(paths) == 0:
                raise AttributeError(f"{name} does not seem to be a known key.\n"
                                     f"To create a new key, please use tree['{name}'] = {value}\n"
                                     "like you would for a normal dict.")
            elif len(paths) > 1:
                raise AttributeError(f'{name} yielded {len(paths)} result(s).\n'
