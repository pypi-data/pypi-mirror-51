...
class PaymentChannel(object):
    ...
    def __init__(self, from_fbs=None):
        ...
        self._close_pubkey = None
        self._close_key_id = None
        self._close_channel_seq = None
        self._close_amount = None
    ...
    def marshal(self):
        obj = {
            ...
            'close_pubkey': bytes(self.close_pubkey) if self.close_pubkey else None,
            'close_key_id': bytes(self.close_key_id) if self.close_key_id else None,
    ...
    @property
    def close_pubkey(self):
        if self._close_pubkey is None and self._from_fbs:
            if self._from_fbs.ClosePubkeyLength():
                self._close_pubkey = self._from_fbs.ClosePubkeyAsBytes()
        return self._close_pubkey

    @close_pubkey.setter
    def close_pubkey(self, value):
        assert value is None or type(value) == bytes
        self._close_pubkey = value

    @property
    def close_key_id(self):
        if self._close_key_id is None and self._from_fbs:
            if self._from_fbs.CloseKeyIdLength():
                self._close_key_id = self._from_fbs.CloseKeyIdAsBytes()
        return self._close_key_id

    @close_key_id.setter
    def close_key_id(self, value):
        assert value is None or type(value) == bytes
        self._close_key_id = value
    ...
    def close_amount(self):
        if self._close_amount is None and self._from_fbs:
            if self._from_fbs.CloseAmountLength():
                _close_amount = self._from_fbs.CloseAmountAsBytes()
                self._close_amount = unpack_uint256(bytes(_close_amount))
        return self._close_amount

    @close_amount.setter
    def close_amount(self, value):
        assert value is None or type(value) == int
        self._close_amount = value
    ...
    def build(self, builder):
        ...
        close_pubkey = self.close_pubkey
        if close_pubkey:
            close_pubkey = builder.CreateString(bytes(close_pubkey))

        close_key_id = self.close_key_id
        if close_key_id:
            close_key_id = builder.CreateString(bytes(close_key_id))

        close_amount = self.close_amount
        if close_amount:
            close_amount = builder.CreateString(pack_uint256(close_amount))
        ...
        if close_pubkey:
            PaymentChannelGen.PaymentChannelAddClosePubkey(builder, close_pubkey)

        if close_key_id:
            PaymentChannelGen.PaymentChannelAddCloseKeyId(builder, close_key_id)

        if self.close_channel_seq:
            PaymentChannelGen.PaymentChannelAddCloseChannelSeq(builder, self.close_channel_seq)

        if close_amount:
            PaymentChannelGen.PaymentChannelAddCloseAmount(builder, close_amount)
