...
class Utils:
        ...
        while True:
            try:
                ...
                if logger:
                    ...
                    logger.info(f"Succeeded {logger_msg} (after {elapsed.total_seconds()}s and {attempt_number} attempts)")
                return res
    ...
    def _handle_exception(attempt_number, ce, logger, logger_msg, start, timeout_in_sec, interval_in_sec, log_every_n_attempts):
        ...
        msg = f"Failed {logger_msg} (after {in_seconds}s and {attempt_number} attempts)"
