...
def f3(driver, url):
    driver.get(url)
    locator = (By.XPATH, "//div[@id='ArticleContent'][string-length()>60] | "
                         "//div[@class='table'][string-length()>60] | "
                         "//div[@class='as-article'][string-length()>30] | //div[@id='main_dom'][string-length()>100]")
    WebDriverWait(driver, 20).until(EC.presence_of_all_elements_located(locator))
    if "chinabidding" in driver.current_url:
        return "登录查看详情"
