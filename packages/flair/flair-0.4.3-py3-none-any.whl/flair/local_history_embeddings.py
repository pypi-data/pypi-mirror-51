...
class MemoryCell(nn.Module):
    def __init__(
        ...
        self.empty_word_tensor = torch.zeros(self.input_length, device=flair.device)
        ...
        # default unknown word vector (trainable if set)
        if train_initial:
            self.empty_word_tensor = torch.nn.Parameter(
                self.empty_word_tensor,
                requires_grad=True,
            )
    ...
    def forward(self, feature_tensor, batch_dp_names) -> torch.tensor:

        # if 'Blargl' in self.word_history:
        #     print(f'Berlin word history: {len(self.word_history["Blargl"])} {self.word_history["Blargl"]}')
        #     print(f'Berlin state history: {self.state_history["Blargl"]}')

        # determine and add to history of each token
        counter_of_surface_forms_in_batch = Counter()
        surface_form_history = {}
        for batch_item_no, dp_names in enumerate(batch_dp_names):

            for dp_no, dp_name in enumerate(dp_names):

                dp_embedding = feature_tensor[batch_item_no, dp_no]

                counter_of_surface_forms_in_batch[dp_name] += 1

                # detach previous gradients otherwise all hell breaks loose
                if counter_of_surface_forms_in_batch[dp_name] == 1 and dp_name in self.word_history:
                    for idx, tensor in enumerate(self.word_history[dp_name]):
                        tensor = tensor.detach()
                        self.word_history[dp_name][idx] = tensor

                # empty tensor represents word we've never seen before
                if dp_name not in self.word_history:
                    self.word_history[dp_name] = [self.empty_word_tensor]

                self.word_history[dp_name].append(dp_embedding)

                surface_form_history[dp_name] = self.word_history[dp_name]
        ...
        all_surface_form_histories = self.locked_dropout(all_surface_form_histories)
        ...
        for batch_item_no, dp_names in enumerate(batch_dp_names):
            ...
            for dp_no, dp_name in enumerate(dp_names):
                ...
                idx = surface_forms_sorted_by_memory_length.index(dp_name)

                # get index of present token in memory
                memory_index = lengths[idx] - counter_of_surface_forms_in_batch[dp_name] + \
                               counter_of_handled_surface_forms[dp_name] - 1
                ...
                memory_tensor[batch_item_no, dp_no] = embedding

                counter_of_handled_surface_forms[dp_name] += 1

        memory_tensor = self.dropout(memory_tensor)
        memory_tensor = self.locked_dropout(memory_tensor)

        if self.concat_word_embeddings:
            memory_tensor = torch.cat([feature_tensor, memory_tensor], dim=2)
...
class MemoryCellEmbeddings(TokenEmbeddings):
    def __init__(
        ...
        self.memory_cell = MemoryCell(
            input_length=contextual_embeddings.embedding_length,
        ...
        self.__embedding_length = hidden_states + contextual_embeddings.embedding_length
