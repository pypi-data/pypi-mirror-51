from lp_api_wrapper.parsers.messaging_interactions.events import (
    AgentParticipant, Campaign, CoBrowseSession, ConsumerParticipant, ConversationSurvey, CustomerInfo, Info, Intent,
...
class Conversations:
    __slots__ = ['agent_participants', 'campaign', 'cobrowse_sessions', 'consumer_participants', 'conversation_surveys',
                 ...
                 'personal_info', 'summary', 'transfers','dialogs', 'responseTime', 'monitoring', 'intents']
    ...
    def append_records(self, records):
        for record in records:
            ...
            for event, data in record.items():
                ...
                elif 'intents' in event:
                    self.__parse_intents(data=data, conversation_id=conversation_id)
    ...
    def __parse_intents(self, data, conversation_id):
        for intent in data:
            event = Intent.parse_from_data(data=intent, conversation_id=conversation_id)
