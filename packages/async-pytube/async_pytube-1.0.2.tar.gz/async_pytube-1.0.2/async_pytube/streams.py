...
combined). This was referred to as ``Video`` in the legacy async_pytube version, but
...
class Stream(object):
    ...
    async def download(self, output_path=None, filename=None, filename_prefix=None):
        ...
        fz = await self.filesize()
        bytes_remaining = fz
        total_count = await self.filesize()
        logger.debug(
            'downloading (%s total bytes) file to %s',
            fz, fp,
        )

        async with aiof.open(fp, 'wb') as fh:
            gen = await request.get(self.url, streaming=True)
            async for chunk in gen:
                # reduce the (bytes) remainder by the length of the chunk.
                bytes_remaining -= len(chunk)
                # send to the on_progress callback.
                await self.on_progress(chunk, fh, bytes_remaining)
        await self.on_complete(fh)
        return fp
    ...
    async def stream_to_buffer(self):
        ...
        async for chunk in request.get(self.url, streaming=True):
            # reduce the (bytes) remainder by the length of the chunk.
            bytes_remaining -= len(chunk)
            # send to the on_progress callback.
            await self.on_progress(chunk, buffer, bytes_remaining)
        await self.on_complete(buffer)
        return buffer

    async def on_progress(self, chunk, file_handler, bytes_remaining):
        ...
        """
        if type(file_handler) == io.BytesIO:
        ...
        if on_progress:
            ...
            await on_progress(self, chunk, file_handler, bytes_remaining)

    async def on_complete(self, file_handle):
        ...
        if on_complete:
            ...
            await on_complete(self, file_handle)
