...
class OpenADRVenAgent:
    ...
    def handle_oadr_event(
        ...
        @orm.db_session
        def _reset_status_to_vtn_status(
            event: database.EiEvent, vtn_status: enums.EventStatus
        ):
            """
            If the VEN thinks the event is canceled and the VTN doesn't think that,
            then un-cancel it.
