...
def _yield_from_ordered_shard(datafile):
    ...
    print("Parsed {} records from shard {}".format(n,datafile))
...
def _interleave_shard_and_empower_records(empower_gen, shard_gen,replace_slice_year=None, replace_slice_year_interval_min = None, replace_slice_year_interval_max = None):
    '''
    :param replace_slice_year: Year for the slice we want to replace.  Slices can only span a single year (because multiple years would be slower)
    :param replace_slice_year_interval_min: Start interval (usually month) for the slice we want to replace
    :param replace_slice_year_interval_max:  End interval (usually month) for the slice we want to replace
    ...
    print(datetime.datetime.now())
    print('records in both',count_keys_same)
    print('empower only',count_keys_empower-count_keys_same)
    print('shard only', count_keys_shard-count_keys_same)
...
def _apply_data_changes(cnxn,shard_suffix):
    print(datetime.datetime.now(),'applying changes')
    print(datetime.datetime.now(),'starting deletes')
    _apply_deletes(cnxn,shard_suffix)
    print(datetime.datetime.now(),'starting updates')
    _apply_updates(cnxn,shard_suffix)
    print(datetime.datetime.now(),'starting inserts')
    _apply_inserts(cnxn,shard_suffix)
    print(datetime.datetime.now(),'completed applying changes')

def upsert_shard(site_locator,shard_directory,shard_prefix,shard_suffix,interval_type = 3,replace_slice_year=None, replace_slice_year_interval_min = None, replace_slice_year_interval_max = None):
    
    #TODO - take site location string
    
    site_locator
    
    sql_working_site_connection_string  = r"{SQL-KVP}|Data Source='<server_name>';Initial Catalog='<database_name>';Connection Timeout=<connection_timeout>;Encrypt=<encrypt_connection>;TrustServerCertificate=<trust_connection_certificate>;|QA-SQLServer16, 49501|SunriseBrandsUnitTestWork|0|||C:\Empower Sites\SunriseBrandsUnitTestWork||||5|False|False|"
    ...
    for r in _interleave_shard_and_empower_records(empower_gen=empower_gen
        ...
        if change_status == 'insert':
            ...
            count_inserts +=1
            if count_inserts % 100000 == 0:
                print(datetime.datetime.now(),"inserts",count_inserts)

        elif change_status == 'update':
            updates_hose.insert_record(r)
            count_updates +=1
            if count_updates % 100000 == 0:
                print(datetime.datetime.now(),"updates",count_updates)
        elif change_status == 'delete':
            deletes_hose.insert_record(r)
            count_deletes +=1
            if count_deletes % 100000 == 0:
                print(datetime.datetime.now(),"deletes",count_deletes)
        elif change_status == 'no_change':
            #TODO - remove this
            #deletes_hose.insert_record(r)
            count_unchanged +=1
            if count_unchanged % 100000 == 0:
                print(datetime.datetime.now(),"unchanged",count_unchanged)
        else:
            raise ValueError()
...
#upsert_shard(cnxn,source_file,shard_suffix,interval_type = 3,replace_slice_year=None, replace_slice_year_interval_min = None, replace_slice_year_interval_max = None)	