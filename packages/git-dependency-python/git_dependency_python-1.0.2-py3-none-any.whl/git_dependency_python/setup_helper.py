...
def install_deps(external_dependencies, internal_dependencies):
    ...
    if should_install_externals():
        ...
        logr.info('Skipping external dependencies installation because of the same project version.')
...
def project_version():
    # type: () ->  Optional[str]
    """
    Returns currently installed projects version.

    :return: Currently installed projects version. Returns none if project is not installed
    in current python environment.
    """
    packages = [package for package in PipHelper.get_all_pip_dependencies() if PROJECT_PREFIX in package]

    packages = {package.split('==')[0]: package.split('==')[1] for package in packages}
    # Make sure we have hyphens in names.
    packages = {key.replace('_', '-'): value for key, value in packages.items()}

    # Make sure we have hyphens in names.
    name = PROJECT_NAME.replace('_', '-')

    installed_version = packages.get(name)

    return installed_version


def should_install_externals():
    # type: () -> bool
    """
    Determines whether external dependencies for current project under install should be installed.
    If project has the same version as an installed one - external dependencies installation should be skipped.

    :return: Boolean value indicating whether to install external dependencies or not.
    """
    installed_version = project_version()
    logr.info('Installed version: {}, Source code version: {}.'.format(installed_version, PACKAGE_VERSION))

    if installed_version != PACKAGE_VERSION:
        logr.warning('Package needs replacement because versions differ.')
        return True
