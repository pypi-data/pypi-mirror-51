...
def startextract(modified_tif,mapfile,tiflayers,iteration):
    ...
    extractw = Canvas(extractframe, width=450, height=450, bg='white')
    extractw.old_coords = None
    extractw.old_drawcoords = None
    tup=(labels,extractw,colortable)
    extractw.bind('<Motion>',lambda event,arg=tup:extractimg(event,tup))
    extractw.create_image(0, 0, image=image, anchor=NW)
    extrabutton=Button(extractframe,text='reset',command=partial(extrareset,extractw))
...
def QGIS_NDVI(usertype):
    global panelA,panelB,RGBlabel,Inflabel,Heightlabel,panelTree,treelist,rotation,root,panelWelcome,root
...
def export_result(usertype):
    ...
    if len(path)>0:
        ...
        for uni in colortable:
            ...
            uniloc=np.where(originrestoredband==float(uni))
            ...
            length=rly-uly
            print(width,length)
            subarea=originrestoredband[uly:rly+1,ulx:rlx+1]
