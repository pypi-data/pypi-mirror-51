...
def get_model(config, depth, anchors, experimental_loss, do_fine_tune, num_gpus, pretrained_weights):
    ###############################
    #   Construct the model
    ###############################
...
def main(args=None):
    ...
    while True:
            start = time.time()
            #graph = tf.Graph()
            yolo = get_model(config, depth, anchors, experimental_loss, do_fine_tune, num_gpus, pretrained_weights)
