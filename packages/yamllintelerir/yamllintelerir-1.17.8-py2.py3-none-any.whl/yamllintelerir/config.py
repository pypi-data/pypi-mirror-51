...
class YamlLintConfig(object):
    def __init__(self, content=None, file=None):
        ...
        self.yaml_files = pathspec.PathSpec.from_lines(
            'gitwildmatch', ['*.yaml', '*.yml', '.yamllintelerir'])
    ...
    def enabled_rules(self, filepath):
        return [yamllintelerir.rules.get(id) for id, val in self.rules.items()
    ...
    def validate(self):
        for id in self.rules:
            try:
                rule = yamllintelerir.rules.get(id)
            except Exception as e:
                raise YamlLintConfigError('invalid config: %s' % e)
...
def get_extended_config_file(name):
    # Is it a standard conf shipped with yamllintelerir..
    if '/' not in name:
