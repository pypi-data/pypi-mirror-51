...
def induce_embeddings(d, embeds_transaction, embeds_client, embeds_merchant, overall_mean):
    ...
    for i, row in d.iterrows():
        ...
        if (client_bool)&(merchant_bool):
            transaction_emb[i] = np.maximum(embeds_client[client] + embeds_merchant[merchant])
...
def run(embeds_client_df, embeds_merchant_df, embeds_transaction_df,
    ...
    val_df_embeddings = val_df_embeddings.merge(val_df, how='left', left_index=True, right_index=True,
                                                  validate="one_to_one")
