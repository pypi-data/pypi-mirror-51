...
static PyObject * py_sample_buf;	// sample byte array provided by the Python hardware file
...
static int py_sample_read(complex double * cSamples)
{
	...
	if (PyObject_GetBuffer(py_sample_buf, &view, PyBUF_SIMPLE) != 0) {
		printf("py_sample_read: Can not view sample buffer\n");
		Py_DECREF(py_sample_buf);
		py_sample_buf = NULL;
		return 0;
	}
	if (view.len % (py_sample_rx_bytes * 2) != 0) {
		printf ("py_sample_read: odd number of bytes in sample buffer\n");
	}
...
{  /* Call with keyword arguments ONLY; change local parameters */
	static char * kwlist[] = {"quisk_is_vna", "rx_bytes", "rx_endian", "read_error", "clip", "rx_samples", NULL} ;
	int nbytes, read_error, clip;
	PyObject * samples = NULL;

	nbytes = read_error = clip = -1;
	if (!PyArg_ParseTupleAndKeywords (args, keywds, "|iiiiiO", kwlist,
	&quisk_is_vna, &nbytes, &py_sample_rx_endian, &read_error, &clip, &samples))
		return NULL;
	if (nbytes != -1) {
	...
	if (samples) {
		if ( ! PyObject_CheckBuffer(samples)) {
			printf("py_samples: Invalid object sent as samples\n");
		}
		else if (py_sample_buf) {
			printf("py_samples: No space left for samples\n");
		}
		else {
			py_sample_buf = samples;
			Py_INCREF(py_sample_buf);
		}
	}
	Py_INCREF (Py_None);
	return Py_None;
