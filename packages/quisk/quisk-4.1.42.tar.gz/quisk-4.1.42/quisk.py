...
class App(wx.App):
  ...
  def OnInit(self):
    ...
    self.HeartBeatSecs = 0.10		# time between calls to Hardware.HeartBeat() in seconds
    self.heart_time0 = self.timer	# timer to call HeartBeat at intervals
    self.worker_time0 = self.timer	# a timer for various jobs
...
The new code supports multiple corrections per band.""")
  ...
  def OnReadSound(self):	# called at frequent intervals
    ...
    if self.timer - self.heart_time0 > self.HeartBeatSecs:	# call hardware to perform background tasks:
      Hardware.HeartBeat()
    if self.timer - self.worker_time0 > 0.10:
      self.worker_time0 = self.timer
      if self.screen == self.config_screen:
