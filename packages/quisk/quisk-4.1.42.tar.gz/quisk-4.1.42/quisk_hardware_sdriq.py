...
class Hardware(BaseHardware):
  ...
  def StartSamples(self):	# called by the sound thread
    return self.device.StartSamples()
  def GetRxSamples(self):	# called by the sound thread
    # Quisk will call this from the sound thread to get the Rx samples. 
    return self.device.GetRxSamples()
...
class Sdriq:
  ...
  def __init__(self, hardware, name, clock, decim):
    ...
    self.sdriq_decimation = decim
    self.sdriq_idle = -1
    self.port = None
    self.sdriq_gstate = 2
    self.sdriq_gain = 127
    self.sdriq_freq = 7220000
  def open(self):
    ...
    self.rx_samples = bytearray(0)	# return these samples to Quisk
    self.sdr_state = 0
    if not serial:
      self.port = None
    ...
    self.SetItem(0x0018, "\x81\x01\x00\x00")
    self.port.reset_input_buffer()
    self.port.reset_output_buffer()
    self.SetItem(0x0018, "\x81\x01\x00\x00")
    self.GetItem(0x0002, '')		# request serial number
    self.GetItem(0x0005, '')		# request status
    self.GetItem(0x0001, '')		# request name
    for i in range(50):
      self.GetRxSamples()	# read the port for data from the GUI thread
      if self.sdr_name:
        break
    else:
      self.port.close()
  ...
  def SetDecimation(self, decim):
    if decim != self.sdriq_decimation:
      self.sdriq_decimation = decim
      if self.port:
        self.StopSamples()
        try:
  ...
  def SetGain(self, gstate, gain):
    self.sdriq_gstate = gstate
    self.sdriq_gain = gain
    if not self.port:
      return
    buf = bytearray(2)
    if gstate == 0:
  ...
  def GetRxSamples(self):	# Read all data from the SDR-IQ and process it.
      ...
      if self.sdr_state == 0:		# read the first byte
        ...
        if len(self.sdr_data) >= self.sdr_length:	# we have all the data for this record
          ...
          if self.sdr_length == 1 and self.sdr_type == 3:	# ACK
            ...
            if item == 1:
              self.sdr_name = str(self.sdr_data[2:-1])		# Remove the NULL byte from these strings
            elif item == 2:
              self.sdr_serial = str(self.sdr_data[2:-1])
            elif item == 3:
        ...
        if byte == 0x80:
          ...
          self.sdr_state = 9
    if self.rx_samples:
      samples = self.rx_samples
      self.rx_samples = bytearray(0)
    else:
      samples = None
    return samples
...
-4839, -6689, -6252, -3635, -23, 3489, 5520, 5669, 3780, 831, -2350, -4428, -5013, -3730, -1400, 1417, 3431, 4330,
 ...
 8014, -14444, 13756, -378 )
    ...
    self.SetFrequency(self.sdriq_freq)
    self.SetGain(self.sdriq_gstate, self.sdriq_gain)
