...
class StarCoinV2(Contract):
    ...
    def __init__(self, provider, timeout=60, *args, **kwargs):
        super(StarCoinV2, self).__init__(provider, timeout, *args, **kwargs)
    ...
    def get_all_status(self):
        ...
        for oper in self.server_operator_private:
            server_operator_address = keys.privatekey_to_address(oper)
            print("account state in account operator is \t {} , {}".format(self.getAccountOperator(server_operator_address), oper))
            print("account state in operator pool is \t {} , {}".format(
                self.call_concise_function('getStateOperatorPool', server_operator_address), oper))
            print("account state in reward pool is \t {} , {}".format(
                self.call_concise_function('getStateRewardPool', server_operator_address), oper))
            print("account state in central bank is \t {} , {}".format(
                self.call_concise_function('getStateCentralBank', server_operator_address), oper))

            print("管理账号的 eth 数量 \t {} , {}".format(
                self.getBalance(server_operator_address), oper))
    ...
    def grantAllPrivate(self, python_operators=[]):
        """
        给多个账号赋予最高权限
        :return:
        """
