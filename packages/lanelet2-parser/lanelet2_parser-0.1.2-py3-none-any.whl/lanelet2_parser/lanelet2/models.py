...
from schematics.types import IntType, StringType, FloatType, DecimalType, ListType, DictType, ModelType, BooleanType
...
class Parameter(Model):
    refers = IntType(required=True)
    ref_line = IntType(required=False)
    right_of_way = IntType(required=False)
    param_yield = IntType(required=False)
    cancels = IntType(required=False)
    cancel_line = IntType(required=False)
...
class RegulatoryElement(Model):
    ...
    subtype = StringType(required=True)
    attributes = []
    parameters = ModelType(Parameter, required=True)

    def __init__(self, id, subtype, attributes={}, parameters={}):
