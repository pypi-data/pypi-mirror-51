...
class CloudStorageUtility:
    ...
    def get_file_synchronizer(filepath, s3_prefix, s3_wrapper, master=FileSynchronizer.LOCAL, s3_full_path=None, ignore_file_states=None):
        ...
        return synchronizer #ThreadedFileSynchronizer(synchronizer)
    ...
    @staticmethod
    def sync_files_v2(account_uid, entity_type, entity_uid, filepath, s3_prefix, s3_wrapper=None,
                   master=FileSynchronizer.LOCAL, s3_full_path=None, ignore_file_states=None,
                   quiet=False, include=None, prefix=None, refresh=False):
        if s3_wrapper is None:
            s3_wrapper = CloudStorageUtility.get_utility(account_uid, entity_type, entity_uid, refresh)

        synchronizer = CloudStorageUtility.get_file_synchronizer(filepath, s3_prefix, s3_wrapper, master, s3_full_path,
                                                                 ignore_file_states)
        ...
        s3_lister = S3.FileGenerator(s3_wrapper.s3)
        bucket = os.getenv('DATASET_BUCKET', 'onepanel-datasets')
        full_full_path = len(bucket + "/" + s3_full_path)

        for s3_path, content in s3_lister.list_files(bucket, s3_full_path, page_size=300):
            local_path = os.path.abspath(s3_path[full_full_path:])
            if not os.path.exists(local_path):
                fd = FileDifference(local_path, s3_path[len(bucket) + 1:], FileDifference.State.NEW)
                synchronizer.synchronize_single(fd)
