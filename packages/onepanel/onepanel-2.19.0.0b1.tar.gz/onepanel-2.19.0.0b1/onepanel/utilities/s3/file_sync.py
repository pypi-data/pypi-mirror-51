...
class ThreadedS3WrapperSynchronizerWrapper:
    def __init__(self, bucket_name, credentials_provider):
        self.bucket_name = bucket_name
        self.credentials_provider = credentials_provider
        self.queue = []
        self.lock = Lock()

    def synchronize_file(self, synchronizer, file_difference, hooks, max_threads):
        with self.lock:
            if len(self.queue) == 0:
                print('creating new wrapper')
                wrapper = S3Wrapper(synchronizer.s3_wrapper.bucket_name, synchronizer.s3_wrapper.credentials_provider)
            else:
                print('reusing wrapper')
                wrapper = self.queue.pop()

        copy_synchronizer = FileSynchronizer(synchronizer.filepath, synchronizer.s3_prefix, wrapper, synchronizer.master, synchronizer.s3_full_path, synchronizer.ignore_file_states)
        copy_synchronizer.synchronize_single(file_difference, hooks, max_threads)
...
class ThreadedFileSynchronizer:
    def __init__(self, synchronizer, hooks=None):
        ...
        self.sync_wrapper = ThreadedS3WrapperSynchronizerWrapper(synchronizer.s3_wrapper.bucket_name, synchronizer.s3_wrapper.credentials_provider)
    ...
    def _execute_item(self, file_difference):
        ...
        task = self.executor.submit(self.sync_wrapper.synchronize_file, self.synchronizer, file_difference, self.hooks,
                                    self.max_threads)
