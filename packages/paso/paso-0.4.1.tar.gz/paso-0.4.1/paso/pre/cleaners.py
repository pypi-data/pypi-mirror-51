...
class Balancer(pasoFunction):
    ...
    def transform(self, X, *args, **kwargs):
        ...
        if self.target not in self.Xcolumns:
            ...
            Xres = X[X.columns.difference([self.target])].values
...
#            logger.debug('\n{}     {}'.format(type(Xres), type(y)))
#            logger.debug('\n{}'.format(self.model))


        Xres, yres = self.model.fit_resample(Xres, y)

#        logger.debug('\n{}     {}'.format(type(Xres), type(yres)))
#       logger.debug('\n{}'.format(self.model))
        # arrays -> df
        X = pd.DataFrame(
            Xres, columns=[item for item in self.Xcolumns if item != self.target]
        )
        logger.debug('\n{}\n{}'.format([item for item in self.Xcolumns if item != self.target],self.Xcolumns))
        X[self.target] = yres
        return X
...
class Augmenter(pasoFunction):
    ...
    def transform(self, X, *args, **kwargs):
        ...
        if self.ratio <= 0.0:
            ...
            each_class_count = X.groupby([self.target]).count()
            min_class_size = int(each_class_count.iloc[:,0].min())
            stub_size = min_class_size * self.ratio
            lowest_class = each_class_count.index.argmin()
