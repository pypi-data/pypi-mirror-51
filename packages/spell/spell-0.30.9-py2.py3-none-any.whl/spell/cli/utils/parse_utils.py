...
def parse_conditions(raw):
    ...
    Examples:
        ...
        StopConditions {
            stop_conditions : [[{"metric":"keras/val","operator":"<","value":.5,"min_steps":2}],
                               [{"metric":"pos_walk","operator":">","value":0,"min_steps":0}]]
        }

        >>>parse_conditions("keras/val_acc < .5:2 && walk > 3","pos_walk > 10:5")
        StopConditions {
            stop_conditions : [[{"metric":"keras/val","operator":"<","value":.5,"min_steps":2},
                                {"metric":"walk","operator":">","value":3,"min_steps":0}],
                               [{"metric":"pos_walk","operator":">","value":10,"min_steps":5}]]
        }
    """
    if len(raw) > 5:
    ...
    for arg in raw:
        ...
        for stop_condition in and_conditions:
            ...
            for m in re.finditer(conditionsPattern, stop_condition):
                ...
                value, min_steps = parse_condition_requirements(stop_condition[m.end(0):])
            stop_conditions.append(ConditionSpecs(metric, condition_op, value, min_steps).to_payload())
