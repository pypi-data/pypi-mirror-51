"""
# MaeGuias Integration tests

This is a simple integration test that builds the MaeGuias
documentation against all of the builtin themes.

From the root of the MaeGuias git repo, use:

    python -m maeguias.tests.integration --help


TODOs
    - Build with different configuration options.
    - Build documentation other than just MaeGuias as it is relatively simple.
"""
...
log = logging.getLogger('maeguias')

DIR = os.path.dirname(__file__)
MAEGUIAS_CONFIG = os.path.abspath(os.path.join(DIR, '../../maeguias.yml'))
MAEGUIAS_THEMES = utils.get_theme_names()
...
def main(output=None):
    ...
    base_cmd = ['maeguias', 'build', '-s', '-v', '--site-dir', ]

    log.debug("Building installed themes.")
    for theme in sorted(MAEGUIAS_THEMES):
        log.debug("Building theme: {0}".format(theme))
        project_dir = os.path.dirname(MAEGUIAS_CONFIG)
