...
from maeguias import exceptions, utils
from maeguias.config import base, defaults
from maeguias.config.config_options import BaseConfigOption
...
class ConfigBaseTests(unittest.TestCase):
    ...
    def test_load_from_file(self):
        ...
        config_file = open(os.path.join(temp_dir.name, 'maeguias.yml'), 'w')
        os.mkdir(os.path.join(temp_dir.name, 'docs'))
        try:
            config_file.write("site_name: MaeGuias Test\n")
            config_file.flush()
            config_file.close()

            cfg = base.load_config(config_file=config_file.name)
            self.assertTrue(isinstance(cfg, base.Config))
            self.assertEqual(cfg['site_name'], 'MaeGuias Test')
        finally:
    ...
    def test_load_from_open_file(self):
        ...
        config_fname = os.path.join(temp_path, 'maeguias.yml')
    ...
    def test_load_from_file_with_relative_paths(self):
        ...
        config_fname = os.path.join(config_dir.name, 'maeguias.yml')
