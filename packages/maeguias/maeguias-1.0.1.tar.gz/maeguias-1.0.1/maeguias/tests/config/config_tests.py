...
import maeguias
from maeguias import config
from maeguias import utils
from maeguias.config import config_options
from maeguias.exceptions import ConfigurationError
from maeguias.tests.base import dedent
...
class ConfigTests(unittest.TestCase):
    ...
    def test_config_option(self):
        ...
        Allows users to specify a config other than the default `maeguias.yml`.
        """
        expected_result = {
            'site_name': 'Example',
            'pages': [
        ...
        with TemporaryDirectory() as temp_path:
            ...
            config_path = os.path.join(temp_path, 'maeguias.yml')
    ...
    def test_theme(self):
        with TemporaryDirectory() as mytheme, TemporaryDirectory() as custom:
            ...
            maeguias_dir = os.path.abspath(os.path.dirname(maeguias.__file__))
            maeguias_templates_dir = os.path.join(maeguias_dir, 'templates')
            theme_dir = os.path.abspath(os.path.join(maeguias_dir, 'themes'))

            results = (
                {
                    'dirs': [os.path.join(theme_dir, 'maeguias'), maeguias_templates_dir],
                    ...
                    'dirs': [os.path.join(theme_dir, 'readthedocs'), maeguias_templates_dir],
                    'static_templates': ['404.html', 'sitemap.xml'],
                    'vars': {
                    ...
                    'dirs': [mytheme, maeguias_templates_dir],
                    'static_templates': ['sitemap.xml'],
                    'vars': {}
                }, {
                    'dirs': [custom, os.path.join(theme_dir, 'readthedocs'), maeguias_templates_dir],
                    'static_templates': ['404.html', 'sitemap.xml'],
                    'vars': {
                        'include_search_page': True,
                        'search_index_only': False,
            ...
            for config_contents, result in zip(configs, results):
                ...
                c = config.Config(schema=(('theme', config_options.Theme(default='maeguias')),))
    ...
    def test_empty_nav(self):
        ...
        conf.load_dict({
            'site_name': 'Example',
            'config_file_path': os.path.join(os.path.abspath('.'), 'maeguias.yml')
        })
        conf.validate()
    ...
    def test_doc_dir_in_site_dir(self):
        ...
        conf = {
            'config_file_path': j(os.path.abspath('..'), 'maeguias.yml')
        }
