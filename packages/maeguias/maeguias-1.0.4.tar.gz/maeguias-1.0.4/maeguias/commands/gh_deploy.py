...
default_message = """Deployed {sha} with MaeGuias version: {version}"""
...
def _check_version(branch):
    ...
    currentv = parse_version(maeguias.__version__)
    if not previousv:
        log.warn('Version check skipped: No version specificed in previous deployment.')
    elif currentv > previousv:
        log.info(
            'Previous deployment was done with MaeGuias version {}; '
            'you are deploying with a newer version ({})'.format(previousv, currentv)
        )
    elif currentv < previousv:
        log.error(
            'Deployment terminated: Previous deployment was made with MaeGuias version {}; '
...
def gh_deploy(config, message=None, force=False, ignore_version=False):
    ...
    message = message.format(version=maeguias.__version__, sha=sha)
