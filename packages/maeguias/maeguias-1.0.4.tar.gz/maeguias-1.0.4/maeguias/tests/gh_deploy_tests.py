...
from maeguias.tests.base import load_config, LogTestCase
from maeguias.commands import gh_deploy
from maeguias import __version__
...
class TestGitHubDeploy(unittest.TestCase):
    ...
    def test_get_remote_url_ssh(self, mock_popeno):
        ...
        mock_popeno().communicate.return_value = (
            b'git@github.com:maeguias/maeguias.git\n',
            b''
        )

        expected = (u'git@', u'maeguias/maeguias.git')
    ...
    def test_get_remote_url_http(self, mock_popeno):
        ...
        mock_popeno().communicate.return_value = (
            b'https://github.com/maeguias/maeguias.git\n',
            b''
        )

        expected = (u'https://', u'maeguias/maeguias.git')
        self.assertEqual(expected, gh_deploy._get_remote_url('origin'))
    ...
    def test_get_remote_url_enterprise(self, mock_popeno):
        ...
        mock_popeno().communicate.return_value = (
            b'https://notgh.com/maeguias/maeguias.git\n',
            b''
        )
    ...
    @mock.patch('maeguias.commands.gh_deploy._is_cwd_git_repo', return_value=True)
    @mock.patch('maeguias.commands.gh_deploy._get_current_sha', return_value='shashas')
    @mock.patch('maeguias.commands.gh_deploy._get_remote_url', return_value=(None, None))
    @mock.patch('maeguias.commands.gh_deploy._check_version')
    @mock.patch('maeguias.commands.gh_deploy.ghp_import.ghp_import', return_value=(True, ''))
    ...
    @mock.patch('maeguias.commands.gh_deploy._is_cwd_git_repo', return_value=True)
    @mock.patch('maeguias.commands.gh_deploy._get_current_sha', return_value='shashas')
    @mock.patch('maeguias.commands.gh_deploy._get_remote_url', return_value=(
        u'git@', u'maeguias/maeguias.git'))
    @mock.patch('maeguias.commands.gh_deploy._check_version')
    @mock.patch('maeguias.commands.gh_deploy.ghp_import.ghp_import', return_value=(True, ''))
    ...
    @mock.patch('maeguias.commands.gh_deploy._get_current_sha', return_value='shashas')
    @mock.patch('maeguias.commands.gh_deploy._check_version')
    @mock.patch('maeguias.utils.ghp_import.ghp_import')
    @mock.patch('maeguias.commands.gh_deploy.log')
...
class TestGitHubDeployLogs(LogTestCase):
    ...
    @mock.patch('subprocess.Popen')
    def test_maeguias_newer(self, mock_popeno):

        mock_popeno().communicate.return_value = (b'Deployed 12345678 with MaeGuias version: 0.1.2\n', b'')

        with self.assertLogs('maeguias', level='INFO') as cm:
            gh_deploy._check_version('gh-pages')
        self.assertEqual(
            cm.output, ['INFO:maeguias.commands.gh_deploy:Previous deployment was done with MaeGuias '
                        'version 0.1.2; you are deploying with a newer version ({})'.format(__version__)]
        )

    @mock.patch('subprocess.Popen')
    def test_maeguias_older(self, mock_popeno):

        mock_popeno().communicate.return_value = (b'Deployed 12345678 with MaeGuias version: 10.1.2\n', b'')

        with self.assertLogs('maeguias', level='ERROR') as cm:
            self.assertRaises(SystemExit, gh_deploy._check_version, 'gh-pages')
        self.assertEqual(
            cm.output, ['ERROR:maeguias.commands.gh_deploy:Deployment terminated: Previous deployment was made with '
                        'MaeGuias version 10.1.2; you are attempting to deploy with an older version ({}). Use '
    ...
    def test_version_unknown(self, mock_popeno):
        ...
        with self.assertLogs('maeguias', level='WARNING') as cm:
            gh_deploy._check_version('gh-pages')
        self.assertEqual(
            cm.output,
            ['WARNING:maeguias.commands.gh_deploy:Version check skipped: No version specificed in previous deployment.']
        )
