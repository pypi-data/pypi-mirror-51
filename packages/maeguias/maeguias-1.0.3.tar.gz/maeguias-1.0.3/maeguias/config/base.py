...
log = logging.getLogger('maeguias.config')
...
class Config(utils.UserDict):
    ...
    def load_file(self, config_file):
        try:
            ...
            # MaeGuias knows and understands ConfigurationErrors
            raise exceptions.ConfigurationError(
                "MaeGuias encountered as error parsing the configuration file: {}".format(e)
            )
...
def _open_config_file(config_file):
    ...
    if config_file is None:
        config_file = os.path.abspath('maeguias.yml')
...
def load_config(config_file=None, **kwargs):
    ...
    the default `maeguias.yml` filename will loaded.
