...
def objective(PopN, Params, DATA, Conc, Time, NR, ConcT, TimeT, model):
    ...
    sum_resid = 0
    timevec = np.sort(Time)[1:]
    concvec = np.sort(Conc)
    
    #Adding noise:
    T = sum(np.where(timevec>=TimeT,1,0))
    C = sum(np.where(concvec<=ConcT,1,0))
    NH  = T*C*NR
    NL = NR*len(timevec)*len(concvec)-NH
    #Matrix with noise:
    sigMat = np.ones((len(concvec), len(timevec)))/(2*sigL**2)
    sigMat[:C,len(timevec)-T:] =1/(2*sigH**2)
