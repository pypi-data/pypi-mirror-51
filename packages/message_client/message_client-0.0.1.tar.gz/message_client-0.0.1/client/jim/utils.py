...
from descriptors.client_descriptor import *


def dict_to_bytes(data_dict):
    """
    Преобразование dict-объекта в байт-код
    :param data_dict: объект типа dict
    :return: byte код
    """
...
def bytes_to_dict(data_bytes):
    ...
    :param data_bytes: byte код
    :return: объект типа dict
    """
...
class ParserSysArg:
    """
    Класс - парсингс аргуиентов командной троки
    """
    ...
    def setting_server_address(self):
        """
        получение IP-адреса, номера PORTа сервера, имени клиента(если
        переданы аргументы)
        :return:
        """
            ...
            while True:
                ...
                if self.sys_arg_list[i] == '-a':
                    ...
                    if re.match(pattern, self.sys_arg_list[i]):
                        ...
                        print('Переданный IP не валиден! Будет использован IP'
                              'по умолчанию.')
    ...
    def get_address(self):
        """
        Метод возвращает кортеж из IP-фдресса и порта
        :return:
        """
