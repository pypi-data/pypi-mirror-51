...
try:
    ...
    from .utils.modelUtils import getDateInterval,convertDate,print_config,elapse_time,open_log,get_files_under_dates,setEnv,tensorflow_save_parameters_with_partition
...
class EstimatorManager(object):
    ...
    def export_model(self, save_path=None, remove_subdir=False, pbtxt=True):
        ...
        self.estimator.export_saved_model(save_path, self.parser.serving_parse_fn(), as_text=pbtxt)
    ...
    def export_model_online(self, save_path=None, out_node_name=None, input_node_map=None, output_data_type='npy', from_pb=True):
...
def run(mode, conf, useSpark, date='', tw=1, wi=1, ps=1, logpath='', **kwargs):
    assert ps > 0
