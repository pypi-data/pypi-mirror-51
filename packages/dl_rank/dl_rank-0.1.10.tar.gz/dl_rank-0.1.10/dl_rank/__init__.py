...
def set_env(conf=None, date=None, ev_num=None, ps_num=None, logpath=None, distribution=None,
...
def infer(conf, date):
    if _env['distribution']:
        process = _distribute.distributeInfer(conf, date,_env['ps_num'], _emr['emrName'], logpath=_env['logpath'])
...
def export4online(conf='', from_pb=False, **kwargs):
    ...
    if conf == '' or kwargs:
        ...
        _main.run('export_online', conf=conf, useSpark=False, out_data_type=_env['save_data_type'],
