...
_env = {'distribution': True, 'date': '', 'ev_num': 1, 'ps_num': 1, 'conf': '', 'tb_port': 6006,
...
@_update_env
def test(conf, date):
    pred_generator = _main.run('test', conf=conf, useSpark=False, date=date, useps=True,
              logpath=_env['logpath'])
...
def infer(conf, date):
    if _env['distribution']:
        process = _distribute.distributeInfer(conf, date, _emr['emrName'], logpath=_env['logpath'])
...
def export4online(conf='', from_pb=False, **kwargs):
    ...
    if conf == '' or kwargs:
        ...
        _main.run('export_online', conf=conf, useSpark=False, from_pb=from_pb, logpath=_env['logpath'])
