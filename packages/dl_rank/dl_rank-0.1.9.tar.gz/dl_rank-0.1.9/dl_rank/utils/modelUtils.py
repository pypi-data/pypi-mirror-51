...
def setEnv(home_path='/home/hadoop'):
    ...
    if 'HADOOP_HOME' not in os.environ or os.environ['HADOOP_HOME'] != '/usr':
        os.system("sudo sed -i -e '$a\export $SPARK_HOME=/usr' {emr_home}/.bashrc".format(emr_home=home_path))
...
def tensorflow_save_parameters_partition(sess, save_path, save_prefix='embedding', prefix='', partition=1):
