...
class SzpltData(dict):
    ...
    def GenerateDataFromOtherFormat(self,dataset):
        ...
        def cal_zone_info(name_zone,value_location):
            ...
            d['passiveVarList'] = [0 if zone_value.get(i,empty).size>0 else 1 for i in self.nameVars] * len(self.nameVars)
...
class write_tecio:
    ...
    def __init__(self,filename,dataset=None ,verbose = True):
        ...
        if not isinstance(dataset,SzpltData) or dataset.added_new_zone is True:
