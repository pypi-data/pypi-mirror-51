...
def pytest_html_results_summary(prefix, summary, postfix):
    ...
    failed = int(failed.replace('failed', '').strip())
    error = int(error.replace('errors', '').strip())
    # %
    passrate = str("%.2f%%" % (float(count[0]) / float(passed + failed + error) * 100))

    file_path = os.path.join(gl.loadcasePath, 'result.txt')
    result_text = "; ".join([count[0], count[1], passed.__str__(), failed.__str__(), error.__str__(), passrate.__str__()])
