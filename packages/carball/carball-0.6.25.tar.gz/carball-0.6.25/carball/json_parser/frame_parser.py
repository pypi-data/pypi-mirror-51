...
class FrameParser(object):
    ...
    def __init__(self, replay_frames, game):
        ...
        self.game = game
        self.name_map = game.replay['names']
        self.object_map = game.replay['objects']
        self.net_cache = game.replay['net_cache']
        for val in self.net_cache:
            val['object_name'] = self.object_map[val['object_ind']]
            for prop in val['properties']:
                prop['object_name'] = self.object_map[prop['object_ind']]
        ...
        for i, frame in enumerate(self.replay_frames):
            ...
            for replication in frame['new_actors']:
                ...
                self.actors[actor_id] = {
                    'Id': actor_id,
                    'TypeName': self.object_map[replication['object_id']],
                    # replication['value']['spawned']['object_name'],
                    'ClassName': self.name_map[replication['name_id']],
            ...
            for replication in frame['updated_actors']:
                ...
                # update properties
                for prop, value in replication['attribute'].items():
                    obj_index = self.object_map.index(actor['TypeName'])

                    actor[prop] = value

            for replication in frame['deleted_actors']:
                actor_id = replication['actor_id']
