    ...
    def update_data_verifier_from_transformer(self, data_transformer: DataTransformer) -> None:
        ...
        for transformer_mapper in data_transformer.transformer_and_mapper_list:
            for f in transformer_mapper.mapper.features:
                col_name = getColumnName(f)
                if col_name in column_names and isImputer(f[1]):
...
def getColumnName(features: List[Any]) -> Any:
    """
