    ...
    def _reset_tsindex(self):
        ...
        if self.index.names != index_keys:
            ...
            if any(self.duplicated(index_keys)):
                error_message = (
                    "There is more than one value for a `time_index` and `grain` combination. "
                    "Ensure that grain columns (containing series ids) "
                    "and time column together uniquely determine a single value.")
