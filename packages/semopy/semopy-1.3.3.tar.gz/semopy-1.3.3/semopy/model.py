...
#                    fl = True
    ...
    def load_params_from_df(self, df):
        ...
        for i, j in self.parameters['Beta']:
            c += 1
            a, b = self.beta_names[0][i], self.beta_names[0][j]
        ...
        for i, j in self.parameters['Lambda']:
            c += 1
            b, a = self.lambda_names[0][i], self.lambda_names[0][j]
        ...
        for i, j in self.parameters['Psi']:
            c += 1
            a, b = self.psi_names[0][i], self.psi_names[0][j]
        ...
        for i, j in self.parameters['Theta']:
            c += 1
            a, b = self.theta_names[0][i], self.theta_names[0][j]
            val = df[(df.lval == a) & (df.op == ops.COVARIANCE.value) & (df.rval == b)].Value
            if not len(val):
                continue
            val = float(val)
