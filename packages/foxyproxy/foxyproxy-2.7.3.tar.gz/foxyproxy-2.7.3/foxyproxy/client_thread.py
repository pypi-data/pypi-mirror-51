...
class ClientThread(Process):
    ...
    def __init__(self, signer, queue_in, queue_out, queue_ack, _id):
        """
    ...
    def read_request(cls, connection, protocol, data_in=None):
        ...
        if result and (create_work.req_reader is not None):
    ...
    @classmethod
    def create_new_worker(cls, signer, name, work_q, result_q, ack_q, index):
        """
        ...
        new_client = ClientThread(signer, work_q, result_q, ack_q, index)
    ...
    @classmethod
    def restart_worker(cls, signer, name, work_q, result_q, ack_q, index):
        """
            ...
            try:
                ...
                if new_work is not None:  # type: ClientWork
                    ...
                    if new_work.command == ClientWork.CMD_KILL:
                        ...
                        self.real_reader.name = new_work.real_reader_name
    ...
    def serve_request(self):
        ...
        try:
            ...
            if self.alt_result_queue is not None:
                self.alt_result_queue.put(self.sync_back)
    ...
    @classmethod
    def _read_tcp(cls, client_request):
        """
        Reads request from TCP connection
        :param client_request
        :type client_request: ClientWork
        :return:
        """

        if client_request.connection is None:
            logging.error("_read_tcp called without connection")
            return False

        more_coming = True
        client_request.req_commands = []
        client_request.req_reader = None
        empty_line = False
        while more_coming:
            ...
            # noinspection PyBroadException
            try:
                data_raw = client_request.connection.recv(4096)
            ...
            for line in lines:
                ...
                try:
                    ...
                    if (len(line) == 0) and len(client_request.req_commands) > 0:
                        more_coming = False
                        continue
                    if line[0] == '#':
                    ...
                    if not client_request.req_reader:
                        cmd_parts = line.split(u':')
                        # noinspection PyUnusedLocal
                        client_request.req_reader = cmd_parts[0]  # if '|' is not in string, it will take the whole line
                        if len(cmd_parts) > 1:
                            password = cmd_parts[1]
                    else:
                        cmd_parts = line.split(':')
                        if len(cmd_parts) == 1:
                            client_request.req_reader = cmd_parts[0]
                            continue
                        ...
                        if item['name'] == 'EMPTYLINE':
                            ...
                            client_request.req_commands.append(item)
                except Exception:
                    pass
            # let's see - we have a command or more and empty_line is False => we end
            if (len(client_request.req_commands) > 0) and (not empty_line):
                more_coming = False

        if client_request.req_reader is None:
            logging.error("No reader ID sent")
            time.sleep(0.1)  # sleep little before making another receive attempt
            return False
        elif len(client_request.req_commands) < 1:
