...
def main():
    ...
    for i in range(0, workers.CAPACITY + 1):  # one for "virtual", 120 for physical
        ...
        new_worker.worker = ClientThread(signer, new_worker.work, new_worker.result, new_worker.ack, i)
    ...
    if not register:
        ...
        for worker in workers.workers:  # type: WorkerRecord
            kill_cmd = ClientWork()
            kill_cmd.command = ClientWork.CMD_KILL
            if worker.worker.is_alive():
                worker.work.put(kill_cmd)
                # noinspection PyBroadException
                try:
                    worker.ack.get(timeout=1.0)
                except Exception:
                    if worker.worker.is_alive():
                        logging.error("Can't stop a worker %s" % worker.worker.process_id)
