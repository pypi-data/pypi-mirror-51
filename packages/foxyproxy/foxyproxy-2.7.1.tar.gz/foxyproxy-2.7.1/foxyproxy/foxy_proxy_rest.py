...
from csp import ClientResult
from foxyproxy.csp import ClientWork, WorkerRecord
            ...
            try:
                ...
                if reader_index >= 0:
                    ...
                    if (new_lock is not None) and (new_client is not None):
                        ...
                        result_q = Queue()
                        work.alt_result_queue = result_q
                        ...
                        while not confirmed:
                            ...
                            try:
                                ...
                                if ack_cmd is not None:
                                    ...
                                    new_client.worker = ClientThread.create_new_worker(self.signer, work.real_reader_name,
                                                                                       new_client.work, new_client.result,
                                                                                       new_client.ack, reader_index)
                            pass
                        ...
                        # noinspection PyBroadException
                        try:
                            result = result_q.get(timeout=200)  # max time for processing = 200 seconds
                        except Exception:
                            pass
                        finally:
                            if result is None:
