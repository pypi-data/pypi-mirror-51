                            ...
                            try:
                                ...
                                if ack_cmd is not None:
                                    if ack_cmd != ClientResult.TYPE_ACK:
                                        logging.error("Worker sent not ACK: type=%d", ack_cmd.type)
                                        confirmed = True
                                    else:
                                        confirmed = True
                                        logging.error("Worker sent ACK: type=%d", ack_cmd.type)
                                elif not new_client.worker.is_alive():
                                    # it expired, let's create a new one
                                    new_client.worker = ClientThread.create_new_worker(signer,
