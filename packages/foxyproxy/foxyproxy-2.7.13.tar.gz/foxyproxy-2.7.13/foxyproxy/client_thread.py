...
class ClientThread(Process):
    ...
    def __init__(self, signer, queue_in, queue_out, queue_ack, queue_rest, _id, mylock=None):
        ...
        self.internal_jobs = Queue()  # type: Queue
                    ...
                    try:
                        if new_work.command == ClientWork.CMD_KILL:
                            ...
                            while not self.internal_jobs.empty():
                                self.internal_jobs.get(block=False)
                    except Exception:
                        pass
                    finally:
                        self.internal_jobs.put(new_work)
    ...
    def run(self):
        ...
        feeder = threading.Thread(target=self.serve_work_queue)
