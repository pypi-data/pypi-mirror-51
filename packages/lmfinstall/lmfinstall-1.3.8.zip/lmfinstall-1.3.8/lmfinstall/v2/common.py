...
def ssh(pin,user='root'):
    ...
    for conp in pin:
        c=Connection(conp[0],connect_kwargs={"password":conp[1]})
        if user=='root':
            c.run("ssh-keyscan -t rsa,dsa -H %s >  /root/.ssh/known_hosts"%(' '.join([ w[0][w[0].index('@')+1:w[0].index(':')] for w in pin]),' '.join([ w[2] for w in pin]) ) )
            #c.run("ssh-keyscan -t rsa,dsa -H %s >>  /root/.ssh/known_hosts"%(' '.join([ w[2] for w in pin])) )
        else:
            cmd="""sudo -u %s ssh-keyscan -t rsa,dsa -H %s > /home/%s/.ssh/known_hosts """%(user,' '.join([ w[0][w[0].index('@')+1:w[0].index(':')] for w in pin]),
