...
import re
from .functions import to_decimal
...
class CFDI:
    ...
    def __init__(self):
        ...
        self.subTotal = ""
        self.descuento = ""
        self.motivoDescuento = ""
        self.TipoCambio = ""
        self.Moneda = ""


        self.FolioFiscalOrig = ""
        self.SerieFolioFiscalOrig = ""
        self.FechaFolioFiscalOrig = ""
        self.MontoFolioFiscalOrig = ""
        ...
        self.emisor_referencia = ""
        self.emisor_municipio = ""
        self.emisor_estado = ""
        self.emisor_pais = ""
        self.emisor_codigoPostal = ""
        #Expedido En
        self.ExpedidoEn = None
        self.expedido_calle = ""
        self.expedido_noExterior = ""
        self.expedido_noInterior = ""
        self.expedido_colonia = ""
        self.expedido_localidad = ""
        self.expedido_referencia = ""
        self.expedido_municipio = ""
        self.expedido_estado = ""
        self.expedido_pais = ""
        self.expedido_codigoPostal = ""
        #Receptor
        self.receptor_rfc = ""
        ...
        self.retenciones = []
        #Complemento nómina
        self.registro_patronal = ""
        self.num_empleado = ""
        self.curp = ""
        self.tipo_regimen = ""
        self.num_cta_pago = ""
        self.num_seguridad_social = ""
        self.fecha_pago = ""
        self.fecha_inicial_pago = ""
        self.fecha_final_pago = ""
        self.dias = ""
        self.departamento = ""
        self.clabe = ""
        self.banco = ""
        self.fecha_inicio_rel_laboral = ""
        self.antiguedad = ""
        self.puesto = ""
        self.tipo_contrato = ""
        self.tipo_jornada = ""
        self.periodicidad_pago = ""
        self.salario_base_cot_apor = ""
        self.riesgo_puesto = ""
        self.salario_diario_integrado = ""
        #Percepciones
        self.percepciones = []
        self.total_gravado_p = ""
        self.total_exento_p = ""
        #Deducciones
        self.deducciones = []
        self.total_gravado_d = ""
        self.total_exento_d = ""
        #Incapacidad y hora extra
        self.incapacidades = ""
        self.horas_extra = ""
        self.comentarios = ""
        self.error_conexion = ""
        ...
        try:
            ...
            self.prodigia_contrato = settings.PRODIGIA_CONTRATO
            self.prodigia_usuario = settings.PRODIGIA_USUARIO
            self.prodigia_password = settings.PRODIGIA_PASSWORD
        ...
        try:
            ...
            self.DFACTURE_USUARIO_PRUEBA = "DEMOAdminT"
            self.DFACTURE_PASSWORD_PRUEBA = "cfdi"
        except:
            self.DFACTURE_USUARIO_PRUEBA = ""
            self.DFACTURE_PASSWORD_PRUEBA = ""
        try:
            self.DFACTURE_USUARIO = settings.LOGIN_DFACTURE[0]
            self.DFACTURE_PASSWORD = settings.LOGIN_DFACTURE[1]
        except:
            self.DFACTURE_USUARIO = ""
            self.DFACTURE_PASSWORD = ""

        #NTLink
        self.ntlink_url = "http://ntlink.com.mx/ntfacturacionws/ServicioTimbrado.svc?wsdl"
        self.ntlink_usuario = "juanefren@dmspitic.com"
        self.ntlink_password = settings.NTLINK_PASSWORD
        self.ntlink_pruebas_url = "http://pruebas.ntlink.com.mx/Certificador/ServicioTimbrado.svc?wsdl"
        self.ntlink_pruebas_usuario = "CHO1006237R4@ntlink.com.mx"
        self.ntlink_pruebas_password = "Factura.2013"
           ...
           self.leyendasFiscales:
            ...
            if self.ine:
                ...
                if self.ine["tipo_proceso"] == "Ordinario":
                    xml += 'TipoComite="%s" ' % ( self.escape(self.ine["tipo_comite"]) )
               
                xml += 'TipoProceso="%s">' % self.escape(self.ine["tipo_proceso"]) 
                if self.ine.get("clave_entidad"):
                    xml += '<ine:Entidad '

                    if self.ine["tipo_proceso"] != "Ordinario":
                        if "estatal" in self.escape(self.ine["tipo_comite"]).lower():
                            xml += 'Ambito="Local" '
                        else:
                            xml += 'Ambito="Federal" '

                    xml += 'ClaveEntidad="%s">' % self.escape(self.ine["clave_entidad"])
                    xml += '<ine:Contabilidad IdContabilidad="%s" />' % self.escape(self.ine["id_contabilidad"])
                    xml += '</ine:Entidad>'
            ...
            if self.iedu:
                ...
                xml += 'nombreAlumno="%s" ' % self.escape(self.iedu["nombreAlumno"])
                xml += 'CURP="%s" ' % self.escape(self.iedu["CURP"])
                xml += 'nivelEducativo="%s" ' % self.escape(self.iedu["nivelEducativo"])
                xml += 'autRVOE="%s" ' % self.escape(self.iedu["autRVOE"])
                
                rfcPago = self.escape(self.iedu["rfcPago"])
                if rfcPago:
        ...
        <detallista:Amount>0</detallista:Amount>
            ...
            if self.nomina:
                ...
                if self.nomina["Receptor"].get("Puesto"):
                    xml += 'Puesto="%s" ' % self.escape(self.nomina["Receptor"]["Puesto"])
            ...
            if self.id_nom:
                xml += '<nomina:Nomina xmlns:nomina="http://www.sat.gob.mx/nomina" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" '
                xml += 'xsi:schemaLocation="http://www.sat.gob.mx/nomina http://www.sat.gob.mx/sitio_internet/cfd/nomina/nomina11.xsd" Version="1.1" '
                if self.registro_patronal:
                    xml += 'RegistroPatronal="%s" ' % (self.escape(self.registro_patronal))
                xml += 'NumEmpleado="%s" ' % (self.escape(self.num_empleado))
                xml += 'CURP="%s" ' % (self.escape(self.curp))
                xml += 'TipoRegimen="%s" ' % (self.escape(self.tipo_regimen))
                if self.num_seguridad_social:
                    xml += 'NumSeguridadSocial="%s" ' % (self.escape(self.num_seguridad_social))
                xml += 'FechaPago="%s" ' % (self.escape(self.fecha_pago))
                xml += 'FechaInicialPago="%s" ' % (self.escape(self.fecha_inicial_pago))
                xml += 'FechaFinalPago="%s" ' % (self.escape(self.fecha_final_pago))
                xml += 'NumDiasPagados="%s" ' % (self.escape(self.dias))
                if self.escape(self.departamento):
                    xml += 'Departamento="%s" ' % (self.escape(self.departamento))
                if self.escape(self.clabe):
                    xml += 'CLABE="%s" ' % (self.escape(self.clabe))
                if self.escape(self.banco):
                    xml += 'Banco="%s" ' % (self.escape(self.banco))
                if self.escape(self.fecha_inicio_rel_laboral):
                    xml += 'FechaInicioRelLaboral="%s" ' % (self.escape(self.fecha_inicio_rel_laboral))
                if self.escape(self.antiguedad):
                    xml += 'Antiguedad="%s" ' % (self.escape(self.antiguedad))
                if self.escape(self.puesto):
                    xml += 'Puesto="%s" ' % (self.escape(self.puesto))
                if self.escape(self.tipo_contrato):
                    xml += 'TipoContrato="%s" ' % (self.escape(self.tipo_contrato))
                if self.escape(self.tipo_jornada):
                    xml += 'TipoJornada="%s" ' % (self.escape(self.tipo_jornada))
                xml += 'PeriodicidadPago="%s" ' % (self.escape(self.periodicidad_pago))
                if self.escape(self.salario_base_cot_apor):
                    xml += 'SalarioBaseCotApor="%s" ' % (self.escape(self.salario_base_cot_apor))
                if self.escape(self.riesgo_puesto) > "0":
                    xml += 'RiesgoPuesto="%s" ' % (self.escape(self.riesgo_puesto))
                if self.escape(self.salario_diario_integrado):
                    xml += 'SalarioDiarioIntegrado="%s"' % (self.escape(self.salario_diario_integrado))
                xml += '>'

                if self.percepciones:
                    xml += '<nomina:Percepciones '
                    xml += 'TotalGravado="%s" ' % (self.escape(self.total_gravado_p))
                    xml += 'TotalExento="%s"' % (self.escape(self.total_exento_p))
                    xml += '>'
                    for p in self.percepciones:
                        xml += '<nomina:Percepcion '
                        xml += 'TipoPercepcion="%s" ' % (self.escape(p['tipo_percepcion']))
                        xml += 'Clave="%s" ' % (self.escape(p['clave']))
                        xml += 'Concepto="%s" ' % (self.escape(p['concepto']))
                        xml += 'ImporteGravado="%s" ' % (self.escape(p['importe_gravado']))
                        xml += 'ImporteExento="%s"' % (self.escape(p['importe_exento']))
                        xml += '/>'
                    xml += '</nomina:Percepciones>'

                if self.deducciones:
                    xml += '<nomina:Deducciones '
                    xml += 'TotalGravado="%s" ' % (self.escape(self.total_gravado_d))
                    xml += 'TotalExento="%s"' % (self.escape(self.total_exento_d))
                    xml += '>'
                    for d in self.deducciones:
                        xml += '<nomina:Deduccion '
                        xml += 'TipoDeduccion="%s" ' % (self.escape(d['tipo_deduccion']))
                        xml += 'Clave="%s" ' % (self.escape(d['clave']))
                        xml += 'Concepto="%s" ' % (self.escape(d['concepto']))
                        xml += 'ImporteGravado="%s" ' % (self.escape(d['importe_gravado']))
                        xml += 'ImporteExento="%s"' % (self.escape(d['importe_exento']))
                        xml += '/>'
                    xml += '</nomina:Deducciones>'

                if self.incapacidades:
                    xml += '<nomina:Incapacidades>'
                    for i in self.incapacidades:
                        xml += '<nomina:Incapacidad '
                        xml += 'DiasIncapacidad="%s" ' % (self.escape(i['dias_incapacidad']))
                        xml += 'TipoIncapacidad="%s" ' % (self.escape(i['tipo_incapacidad']))
                        xml += 'Descuento="%s"' % (self.escape(i['descuento']))
                        xml += '/>'
                    xml += '</nomina:Incapacidades>'

                if self.horas_extra:
                    xml += '<nomina:HorasExtras>'
                    for h in self.horas_extra:
                        xml += '<nomina:HorasExtra '
                        xml += 'Dias="%s" ' % (self.escape(h['dias']))
                        xml += 'TipoHoras="%s" ' % (self.escape(h['tipo_horas']))
                        xml += 'HorasExtra="%s" ' % (self.escape(h['horas_extra']))
                        xml += 'ImportePagado="%s"' % (self.escape(h['importe_pagado']))
                        xml += '/>'
                    xml += '</nomina:HorasExtras>'
            ...
            if self.construccion_licencia:
                xml += '<servicioparcial:parcialesconstruccion Version="1.0" NumPerLicoAut="%s" ' % self.escape(self.construccion_licencia)
                ...
                xml += '<servicioparcial:Inmueble Calle="%s" ' % self.escape(self.construccion_calle)
                if self.construccion_no_exterior:
                    xml += 'NoExterior="%s" ' % self.escape(self.construccion_no_exterior)

                if self.construccion_no_interior:
                    xml += 'NoInterior="%s" ' % self.escape(self.construccion_no_interior)

                if self.construccion_colonia:
                    xml += 'Colonia="%s" ' % self.escape(self.construccion_colonia)

                if self.construccion_localidad:
                    xml += 'Localidad="%s" ' % self.escape(self.construccion_localidad)

                if self.construccion_referencia:
                    xml += 'Referencia="%s" ' % self.escape(self.construccion_referencia)

                xml +=  'Municipio="%s" Estado="%s" CodigoPostal="%s" />' % (
                    self.escape(self.construccion_municipio), 
                    self.escape(self.construccion_estado), 
                    self.escape(self.construccion_codigo_postal)
                )
    ...
    def generar_sello(self):
        ...
        co_path = self.tmp_path + str(clave) + "_" + str(self.id) + ".txt"
        xml_path = self.tmp_path + str(clave) + "_" + str(self.id) + ".xml"
    ...
    def set_post_timbrado(self):
        ...
        try:
            if not self.TIMBRADO_PRUEBAS:
                ...
                from clients.models import Cliente as ClienteClients, Timbre
                cliente_clients = ClienteClients.objects.using("central").get(clave=self.clave_central)
                cliente_clients.timbres_consumidos += 1
                fecha = self.fecha
                if not fecha:
                    fecha = self.Fecha

                cliente_clients.ultimo_timbre = fecha
                 
                cliente_clients.save(using="central")

                timbre = Timbre()
                timbre.cliente = cliente_clients
                timbre.uuid = self.cfdi_uuid
                timbre.pac = self.PAC
                timbre.tipo_comprobante = self.TipoDeComprobante
                timbre.save(using="central")

                # guardamos los timbres disponibles en la conf del cliente
                cf = self.cf
                cf.timbres_disponibles = cliente_clients.get_timbres_disponibles()
                cf.save()

        except Exception as error:
            from utils.functions import send_to_sentry
    ...
    def registrar_emisor_ntlink(self):
        class Object:
            pass
        c  = Object()
        c.municipio = self.emisor_municipio
        c.colonia = self.emisor_colonia
        c.cp = self.emisor_codigoPostal
        c.calle = self.emisor_calle
        c.email = "%s@dmspitic.com" % self.emisor_rfc
        c.estado = self.emisor_estado
        c.razon_social = self.emisor_nombre
        c.regimen_fiscal = self.regimen_fiscal 
        c.rfc = self.emisor_rfc
    ...
    def _timbrar_ntlink(self, registrar=True):
        ...
        try:
            from clients.models import Cliente as ClienteClients
            cliente_clients = ClienteClients.objects.using("central").get(clave=self.clave_central)
            cliente_clients.timbres_consumidos += 1
            cliente_clients.ultimo_timbre = self.fecha
            cliente_clients.save(using="central")
        except Exception as error:
            from django.core.mail import mail_admins
            mail_admins("Error", "%s %s %s %s timbre" % (error, self.emisor_rfc, self.clave_central, self.total ) )
    ...
    def _timbrar_dfacture(self):
        ...
        if self.TIMBRADO_PRUEBAS:
            ...
            usuario = self.DFACTURE_USUARIO_PRUEBA
            password = self.DFACTURE_PASSWORD_PRUEBA
        else:
            url = "https://timbradobalancer.dfacture.com/api/timbrarCFDI33"
            usuario = settings.LOGIN_DFACTURE[0]
            password = settings.LOGIN_DFACTURE[1]
    ...
    def dfactura_consulta_cfdi(self, tipo_consulta):
        ...
        if self.TIMBRADO_PRUEBAS:
            ...
            usuario = self.DFACTURE_USUARIO
            password = self.DFACTURE_PASSWORD
...
class CfdiPago():
    ...
    def generar_cfdi(self, conf, clave, cliente_clients, timbrado_prueba=False, pac=None):
        """
        ...
        cfdi = obtener_cfdi_base(conf, clave, cliente_clients, timbrado_prueba=timbrado_prueba, pac=pac)
        cfdi.SubTotal = "0"
        cfdi.Total = "0"
        cfdi.Moneda = "XXX"
        ...
        cfdi.Emisor = {}
        cfdi.Emisor["Rfc"] = conf.rfc
        cfdi.Emisor["Nombre"] = conf.razon_social
        cfdi.Emisor["RegimenFiscal"] = conf.regimen_fiscal
        ...
        if not cfdi.pagos:
            ...
            cfdi.pagos.append(recepcion_pago)

        for rp in cfdi.pagos:
            rp["DoctoRelacionado_set"] = []
            for documento in rp["documentos"]:
                if not documento['cfdi_uuid']:
                    self.cfdi_instance.cfdi_status = u"La factura %s no tiene UUID o no está timbrada." % documento['folio_serie']
