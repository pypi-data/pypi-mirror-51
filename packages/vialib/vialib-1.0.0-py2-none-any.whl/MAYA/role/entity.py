...
_UUID_ = MAYA.uuid.fromString("~fd000a0251-0b-fd0a96b73e")
...
class Role(MAYA.uform.UForm):
	...
	def __init__(self, retain_object_relation=0):
		...
		assert(not filter(lambda i, M=MAYA.uform: not M.isa(i), implied_roles_list))
		
		uf = MAYA.role.createRoleUForm(self.__UUID_,
					       self.__module__,
					       implied_roles_list,
					       self.__ATTRIBUTES_)
		if retain_object_relation:
			...
			uf.eform[MAYA.role._IMPLIED_ROLES_ATTR_] = implied_roles_list		
			uf.eform[MAYA.role._IMPLIED_ROLES_CLOSURE_ATTR_] = implied_roles_closure_list
			
		MAYA.uform.UForm.__init__(self,
					  uf.uuid,
					  uf.eform)
		
	
class UForm(MAYA.uform.UForm):
	"""
	Entitiy UForm
	"""
	...
	def __init__(self, uu=None, eform=None, create_attributes=1, repos=None):
		assert(uu == None or MAYA.uuid.isa(uu))
		assert(eform == None or type(eform) == type({}))

		roles_list = []
		map(lambda i, r=roles_list: r.append(i(retain_object_relation=1)), self.__ROLES_)
		
		assert(not filter(lambda i, M=MAYA.uform: not M.isa(i), roles_list))

		uf = MAYA.role.createUFormOfRole(uu,
						 roles=roles_list,
						 repos=repos,
