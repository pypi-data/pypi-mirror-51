...
class adevinta_yapo_bi_pySpark_postgresql(object):
    ...
    def write_dataFrame_csv( self, df, path_file ):
        try:
            self.logger_db.info('Write DataFrame to CSV App Name %s ' % self.app_name)
            df.write.format('com.databricks.spark.csv').save( path_file )
        except Exception as e:
            self.logger_db.error('%s' % e )

    def write_DataFrame_( self, df, table_write, mode_write ):
        """
        Method [ write_DataFrameWriter ] allow write a dataframe in postgresql database.
        ...
        try:
            ...
            # est instruccion escribe pero es muy lenta
            #df.write \
            #    .mode(mode_write) \
            #    .format("jdbc") \
    ...
    def write_to_parquet( self, df, path_filename ):
        try:
