...
saved_pickle_model = os.path.join(here, '..', 'output', 'r2_0.84_2019-07-23_17:05.pkl')
...
def load_dataset_RV(catalog_exoplanet='exoplanet.eu_catalog_15April.csv',
    ...
    catalog_exoplanet = os.path.join(here, catalog_exoplanet)
...
def random_forest_regression(dataset,
    ...
    if fit:
        ...
        direc_output = os.path.join(here, '..', 'output/')
        name_Rf = direc_output + 'r2_' + str(round(rf.best_score_, 2)) + '_' + str(datetime.datetime.now().strftime("%Y-%m-%d_%H:%M")) + '.pkl'
...
def plot_learning_curve(regr, dataset, save=False, fit=False):
    ...
    direc = os.path.join(here, '..', 'output/')
    if fit:
