...
class Xanity(object):
    ...
    def _parse_anal_data(self):
        ...
        if self.experiments and self.action == RUN_COMMANDS.run:
            ...
            for anal in self.analyses.values():
                rm = []
                for exp in anal.experiments.values():
                    if not any(xanity_data.get_rundirs(experiments=exp.name, include_saved=True)):
                        rm.append(exp.name)
                for name in rm:
                    del anal.experiments[name]
    ...
    def find_file(self, filename=None, experiment=None, include_debug=False):
        ...
        run_dirs = xanity_data.get_rundirs(experiments=experiment, include_saved=True, include_debug=include_debug)
    ...
    def load_variable(self, name, experiment=None, run=None):
        ...
        if isinstance(self.status.focus, Experiment) and self.status.data_dir and os.path.isdir(self.status.data_dir):
            test = xanity_data.fetch_variable(variable=name, runids=self.run_id)
            if test is not None:
                return test
        ...
        # else:
        #     runids = None
        frame = xanity_data.fetch_variable(experiment=experiment, variable=name, most_recent=True, runids=run)
