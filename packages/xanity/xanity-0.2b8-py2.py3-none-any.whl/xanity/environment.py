...
from .common import CommandSet, digest_string, digest_file
...
def condabashcmd(cmd):
    get_root()
    rcfile = os.path.join(XANITY_ROOT, XANITY_FILES.rcfile)
...
def check_environment():
    get_root()
    conda_env = os.path.join(XANITY_ROOT, XANITY_FILES.conda_env)
...
def check_conda_env():
    get_root()
    conda_env_hash_file = os.path.join(XANITY_ROOT, XANITY_FILES.env_hash)
...
def hash_conda_env():
    ...
    conda_env_path = os.path.join(XANITY_ROOT, XANITY_FILES.conda_env)
...
def check_conda_file():
    get_root()
    conda_file_hash = os.path.join(XANITY_ROOT, XANITY_FILES.conda_hash)
...
def hash_conda_env_file():
    get_root()
    hash_file = os.path.join(XANITY_ROOT, XANITY_FILES.conda_env_file)
    return digest_file(hash_file)


def freeze_conda():
    get_root()
    conda_hash_file = os.path.join(XANITY_ROOT, XANITY_FILES.conda_hash)
    env_hash_file = os.path.join(XANITY_ROOT, XANITY_FILES.env_hash)
...
def save_conda_snapshot():
    get_root()
    archive_file = os.path.join(XANITY_ROOT, RELATIVE_PATHS.tmp, 'conda_env_state.txt')
...
def dump_pip_requirements():
    ...
    get_root()
    archive_file = os.path.join(XANITY_ROOT, RELATIVE_PATHS.tmp, 'pip-requirements.txt')
    last_hash_file = os.path.join(XANITY_ROOT, XANITY_FILES.last_pip_hash)
...
def setup(project_root):
    ...
    env_spec_filepath = os.path.join(project_root, XANITY_FILES.conda_env_file)
    conda_env_path = os.path.join(project_root, XANITY_FILES.conda_env)
...
def setup_entry(args=None):
    ...
    if not dirspec:
        ...
        _xanity._orient()
        get_root()
        dirspec = XANITY_ROOT  # if root else os.getcwd()
