...
class PathStr(ComparableMixin, str):
    ...
    def __init__(self, *args):
        ...
        if isinstance(args[0], list):
            ...
            # self._pathAsList will have empty values at the start or end of it's 
            # array to indicate if self.raw starts or ends with a slash.
            pathAsList = []
            ignore = True
            lastNonEmptyVal = None
            numIgnoredVals = 0
            precedingSlash = (args[0][0] == '')
            trailingSlash = (args[0][-1] == '')
            ...
            self._pathAsList = prefix + pathAsList + suffix
    ...
    def getPathAsList(self, slashIndicators=False):
    ...
    def getPathRelativeTo(self, otherPathStr, dotSlash=True):
        ...
        # add slashes between each list element
        relativePath = self.sep.join(relativePathList)

        return PathStr(relativePath)
