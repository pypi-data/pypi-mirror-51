...
# from .exceptions import PorjectNotDefinedException
# from .request_logger_config import RequestLoggerConfig as config
...
class RequestLogger:
    ...
    def log(self, db_store=DB_STORE, **kwargs):
        """
        ...
        if len(func_resp) > RESPONSE_STR_LIMIT:
            func_resp = func_resp[:RESPONSE_STR_LIMIT] + '[...]'
        ...
        #         raise PorjectNotDefinedException()
        #
        #     # Enqueue a task to save the request
        #     queue = QueueUtil(config.REQUEST_LOG_QUEUE_NAME, project=config.project)
        #     queue.enqueue('POST', config.REQUEST_LOG_QUEUE_URI, log_entry)

        # return FireStoreUtil(config.REQUEST_LOG_COLLECTION).set(log_entry)
        return log_entry

    def trace(self, fn, db_store=DB_STORE, *args, **kw):
        """
...
@decorator
def trace(fn, db_store=DB_STORE, *args, **kw):
    return RequestLogger(fn.__module__).trace(fn, db_store=db_store, *args, **kw)
