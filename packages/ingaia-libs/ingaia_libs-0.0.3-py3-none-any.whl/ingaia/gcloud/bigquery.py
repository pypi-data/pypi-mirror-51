...
class BigQueryUtil:
    ...
    Examples:
        from gcloud import BigQueryUtil, ProjectReference

        bq_util = BigQueryUtil('dataset_name', 'table_name', project=ProjectReference('project_id','location'))

        # import json file to bigquery table
        uri = f"gs://[storage-bucket-name]/[file-name]"
        bq_util.import_data_from_json_file(uri)

        # gets dataset object (from original google library)
        dataset = bq_util.dataset

        # gets table object (from original google library)
        table = bq_util.table
    """
    ...
    def dataset(self) -> bigquery.dataset.DatasetReference:
        if not self._dataset:
            try:
                dataset_ref = bigquery.dataset.DatasetReference(dataset_id=self.dataset_id,
                                                                project=self.project.project_id)
                self._dataset = self.client.get_dataset(dataset_ref)
            except NotFound:
