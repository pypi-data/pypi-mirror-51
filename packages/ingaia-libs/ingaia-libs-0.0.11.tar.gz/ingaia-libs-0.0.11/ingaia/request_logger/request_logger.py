...
class RequestLogger:
    ...
    def log(self, db_store=config.DB_STORE, **kwargs):
        ...
        project_id = os.environ.get('GOOGLE_CLOUD_PROJECT', 'not-set_OR_local')
        ...
        if db_store and rl_token:
            ...
            r = requests.post(url=config.REQUEST_LOGGER_DATA_STORE_URL, json=json.dumps(log_entry), headers=headers)
            logging.info(r.text)
