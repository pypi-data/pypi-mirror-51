...
class Transaction(object):
    ...
    def begin(self):
        ...
        if not response.ok:
            raise uniconnect.exceptions.DatabaseError(
    ...
    def commit(self):
        query = uniconnect.client.PrestoQuery(self._request, COMMIT)
        try:
            list(query.execute())
    ...
    def rollback(self):
        query = uniconnect.client.PrestoQuery(self._request, ROLLBACK)
        try:
            list(query.execute())
        except Exception as err:
            raise uniconnect.exceptions.DatabaseError(
