...
import uniconnect.exceptions
import uniconnect.client
import uniconnect.logging
import uniconnect.redirect
...
class Connection(object):
    ...
    def __init__(
        ...
        redirect_handler=uniconnect.redirect.GatewayRedirectHandler(),
    ...
    def _create_request(self):
        return uniconnect.client.uniconnectRequest(
            self.host,
            self.port,
            self.user,
            self.source,
...
class Cursor(object):
    ...
    def cancel(self):
        if self._query is None:
            raise uniconnect.exceptions.OperationalError(
...
ROWID = DBAPITypeObject()  # nothing indicates row id in uniconnect
