...
class ELETTRAWidget(widget.OWWidget):
    author = "Matteo Altissimo"
    maintainer_email = "matteo.altissimo@elettra.eu"

    outputs = [{"name": "elettra_data",
    ...
    def __init__(self):
        ...
        self.elettra_output = QtWidgets.QTextEdit()
        self.elettra_output.setReadOnly(True)

        out_box = gui.widgetBox(out_tab, "System Output", addSpace=True, orientation="horizontal")
        out_box.layout().addWidget(self.elettra_output)

        self.elettra_output.setFixedHeight(600)
        self.elettra_output.setFixedWidth(600)
    ...
    def plot_results(self, calculated_data, progressBarValue=80):
        if not self.view_type == 0:
            if not calculated_data is None:
                ...
                elettra_data = calculated_data.get_content("elettra_data")
                ...
                for index in range(0, len(titles)):
                    ...
                    try:
                        self.plot_histo(elettra_data[:, x_index],
                                        elettra_data[:, y_index],
    ...
    def writeStdOut(self, text):
        cursor = self.elettra_output.textCursor()
        cursor.movePosition(QtGui.QTextCursor.End)
        cursor.insertText(text)
        self.elettra_output.setTextCursor(cursor)
        self.elettra_output.ensureCursorVisible()
    ...
    def plot_histo(self, x, y, progressBarValue, tabs_canvas_index, plot_canvas_index, title="", xtitle="", ytitle="",
        ...
        ELETTRAWidget.plot_histo_2(self.plot_canvas[plot_canvas_index], x, y, title, xtitle, ytitle, color, replace)
    ...
    def compute(self):
        ...
        try:
            ...
            if calculation_output is None:
                ...
                self.send("elettra_data", self.calculated_data)
    ...
    def extract_data_from_calculation_output(self, calculation_output):
        calculated_data = DataExchangeObject("ELETTRA", self.get_data_exchange_widget_name())

        calculated_data.add_content("elettra_data", calculation_output)
