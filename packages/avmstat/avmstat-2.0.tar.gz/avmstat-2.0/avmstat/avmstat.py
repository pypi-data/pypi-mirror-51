...
class modelPerformance:
    ...
    def __init__(self, df, URN, modelName, predictedPrice, confidence, actualPrice, confidenceTollerence = 0.01):
        ...
        self.withinConf = self.__withinConfidence(self.absErr, self.fsd, confidenceTollerence)
    ...
    def __withinConfidence(self, aErr, conf, tollerence):
        wC_l = list()
        for a, c in zip(aErr, conf):
            if ((a) <= (c + tollerence)):
                wC = 1
            else:
                wC = 0
        ...
        for i in numModels:
            for j in numModels:
                ...
                if (i == j):
                    ...
                    perfnum = pd.crosstab(df['Performance' + self.comparison[i].name], df['Performance' + self.comparison[j].name], margins=True).applymap('{:,.2%}'.format)
