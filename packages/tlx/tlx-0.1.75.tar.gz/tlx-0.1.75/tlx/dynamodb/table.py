...
def add_key(table, key, item):
    ...
    logger.debug('submitting item: {}'.format(full_item))  # noqa: E999   - only invalid in old pythons

    logger.info("Attempting to add new record for: {key} ".format(key=key))
    res = table.put_item(
        Item=full_item,
        ConditionExpression="attribute_not_exists({})".format(key_names[0]),  # TODO fix for items with Partition and Sort key
    )

    logger.info("Successfully added new record.")
    logger.debug("{full_item}")
...
def append_to_list_field(table, key, field_to_update, expression_attribute_names, new_item, add_missing_key=False):
    ...
    try:
        ...
        logger.info("field not found: {str_path}".format(str_path=str_path))
