...
class Netizens(AbstractGrapheneChain):
    ...
    def account_status_give(self, giver, referral_status_type=0, start_bonus='', account=None, **kwargs):
        """ Upgrade an account to Lifetime membership
        ...
        if not account:
            raise ValueError("You need to provide an account")
        account = Account(account, blockchain_instance=self)
        giver = Account(giver, blockchain_instance=self)

        op = operations.Account_status_give(
            **{
                "fee": {"amount": 0, "asset_id": "1.3.0"},
                "account_to_upgrade": account["id"],
                "giver": giver["id"],
                "referral_status_type":int(referral_status_type),
                "start_bonus":str(start_bonus),
                "prefix": self.prefix,
            }
        )
        return self.finalizeOp(op, account["name"], "active", **kwargs)
    ...
    def new_invoice_create(self, customer, merchant,  amount_init=0, ntz_amount_init=0, tax_init=0, delivery_init=0,  memo="", merchant_order_id = "", account=None, **kwargs):
        from .memo import Memo
        if not account:
            if "default_account" in self.config:
                account = self.config["default_account"]
        if not account:
            raise ValueError("You need to provide an account")
        amount = Amount(amount_init, "1.3.0", blockchain_instance=self)
        ntz_amount = Amount(ntz_amount_init, "1.3.0", blockchain_instance=self)
        tax = Amount(tax_init, "1.3.0", blockchain_instance=self)
        delivery = Amount(delivery_init, "1.3.0", blockchain_instance=self)

        # print(amount)
        merchant = Account(merchant, blockchain_instance=self)
        customer = Account(customer, blockchain_instance=self)
        creator = Account(account, blockchain_instance=self)

        memoObj = Memo(from_account=account, to_account=customer, blockchain_instance=self)

        op = operations.New_invoice_create(
            **{
                "fee": {"amount": 0, "asset_id": "1.3.0"},
                "creator": creator["id"],
                "merchant": merchant["id"],
                "customer": customer["id"],
                "amount": {"amount": int(amount), "asset_id": "1.3.0"},
                "ntz_amount": {"amount": int(ntz_amount), "asset_id": "1.3.0"},
                "tax": {"amount": int(tax), "asset_id": "1.3.0"},
                "delivery": {"amount": int(delivery), "asset_id": "1.3.0"},
                "merchant_order_id": merchant_order_id,
                "memo": memoObj.encrypt(memo),
            }
        )
    ...
    def new_invoice_pay(self, invoice, merchant, customer, core_amount=0, bonus_amount=0, account=None, **kwargs):
        if not account:
            if "default_account" in self.config:
                account = self.config["default_account"]
        if not account:
        ...
        customer = Account(customer, blockchain_instance=self)
        merchant = Account(merchant, blockchain_instance=self)
        payer = Account(account, blockchain_instance=self)

        op = operations.New_invoice_pay(
            **{
                "fee": {"amount": 0, "asset_id": "1.3.0"},
                "invoice": invoice,
                "customer": customer["id"],
        ...
        return self.finalizeOp(op, payer, "active", **kwargs)