...
class Price(GraphenePrice):
    """ This class deals with all sorts of prices of any pair of assets to
        ...
        each being an instance of :class:`netizens.amount.Amount`. The
        ...
        Way to obtain a proper instance:
            ...
            * ``args`` can be a floating number and ``base`` and ``quote`` being instances of :class:`netizens.asset.Asset`
            ...
            * ``args`` being a list of ``[quote, base]`` both being instances of :class:`netizens.amount.Amount`
            * ``args`` being a list of ``[quote, base]`` both being instances of ``str`` (``amount symbol``)
            * ``base`` and ``quote`` being instances of :class:`netizens.asset.Amount`

        This allows instanciations like:

        * ``Price("0.315 USD/NTZ")``
        * ``Price(0.315, base="USD", quote="NTZ")``
        * ``Price(0.315, base=Asset("USD"), quote=Asset("NTZ"))``
        ...
        * ``Price(1.0, "USD/GOLD")``
        ...
        .. code-block:: python
            ...
            >>> from netizens.price import Price
            >>> Price("0.3314 USD/NTZ") * 2
            0.662600000 USD/NTZ
    ...
    def market(self):
        """ Open the corresponding market
            ...
            :returns: Instance of :class:`netizens.market.Market` for the
...
class Order(Price):
    """ This class inherits :class:`netizens.price.Price` but has the ``base``
