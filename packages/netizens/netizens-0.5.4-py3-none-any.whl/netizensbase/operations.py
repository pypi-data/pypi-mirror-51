...
class New_invoice_create(GrapheneObject):
    def __init__(self, *args, **kwargs):
        # Allow for overwrite of prefix
        if isArgsThisClass(self, args):
            self.data = args[0].data
            ...
            super().__init__(
                OrderedDict(
                    [
                        ...
                        ("merchant_order_id", String(kwargs["merchant_order_id"])),
                        ("amount", Asset(kwargs["amount"])),
                        ("ntz_amount", Asset(kwargs["ntz_amount"])),
                        ("tax", Asset(kwargs["tax"])),
                        ("delivery", Asset(kwargs["delivery"])),
                        ("memo", memo)
                    ]
                )
            )

class New_invoice_pay(GrapheneObject):
    def __init__(self, *args, **kwargs):
        # Allow for overwrite of prefix
        if isArgsThisClass(self, args):
            self.data = args[0].data
            ...
            super().__init__(
                OrderedDict(
                    [
                        ("fee", Asset(kwargs["fee"])),
                        ("invoice", ObjectId(kwargs["invoice"], "new_invoice")),
                        ("customer", ObjectId(kwargs["customer"], "account")),
                        ("merchant", ObjectId(kwargs["merchant"], "account")),
...
class Account_status_give(GrapheneObject):
    def __init__(self, *args, **kwargs):
        # Allow for overwrite of prefix
        if isArgsThisClass(self, args):
            self.data = args[0].data
            ...
            super().__init__(
                OrderedDict(
                    [
                        ("fee", Asset(kwargs["fee"])),
                        ("account_to_upgrade", ObjectId(kwargs["registrar"], "account")),
                        ("giver", ObjectId(kwargs["referrer"], "account")),
                        ("referral_status_type", Uint8(kwargs["referral_status_type"])),
                        ("start_bonus", String(kwargs["name"])),
                    ]
                )
            )
