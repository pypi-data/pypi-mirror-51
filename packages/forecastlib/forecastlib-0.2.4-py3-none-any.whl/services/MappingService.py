...
class MappingService(object):
    ...
    @staticmethod
    def add_upa_vector(df: pd.DataFrame, last_known_year: int):
        df["UPA_LOG"] = np.log10(df["UPA_ACTUAL"])
        grouped = df.groupby(by=["PROD_ID", "CHAN_CD"])

        frames = []
        for name, group in grouped:
            x = group[group["YEAR"] <= last_known_year]["OFFER_PERC"]
            y = group[group["YEAR"] <= last_known_year]["UPA_LOG"]

            A = np.c_[x, np.ones(x.__len__())]
            p, residuals, rank, svals = np.linalg.lstsq(A, y)

            group.loc[:,"VEC_A"] = p[0]
            group.loc[:,"VEC_B"] = p[1]
            group.loc[:,"VEC_COUNT"] = group.shape[0]

            frames.append(group[["VEC_A", "VEC_B", "VEC_COUNT"]])

        concatenated = pd.concat(frames)
        df = df.merge(concatenated.sort_index(axis=0), left_index=True, right_index=True)

        df["UPA_VECTOR_LOG"] = df["OFFER_PERC"] * df["VEC_A"] + df["VEC_B"]
        df["UPA_VECTOR"] = df["UPA_VECTOR_LOG"].apply(lambda x: np.power(10, x))

        df["USE_VECTOR"] = np.where(df["VEC_COUNT"] > 2, 1, 0)
    ...
    def add_full_history(df: pd.DataFrame, last_known_year: int):
        ...
        #AVG_UPA per concept
        df = MappingService.group_upa(df, no_last_year, ['CONCEPT_CD', 'CHAN_CD', 'OFFER_PERC_M'], "UPA_CONCEPT", "UPA_ACTUAL")
        df = MappingService.group_upa(df, no_last_year, ['SEGMENT_CD', 'CHAN_CD', 'OFFER_PERC_M'], "UPA_SEGMENT", "UPA_ACTUAL")
        df = MappingService.group_upa(df, no_last_year, ['SECTOR_CD', 'CHAN_CD', 'OFFER_PERC_M'], "UPA_SECTOR", "UPA_ACTUAL")
        df = MappingService.group_upa(df, no_last_year, ['CATEGORY_CD', 'CHAN_CD', 'OFFER_PERC_M'], "UPA_CATEGORY", "UPA_ACTUAL")
        ...
        df["UPA_FALLBACK"] = df.groupby('CHAN_CD')["UPA_FALLBACK"].fillna(method='ffill')
    ...
    @staticmethod
    def group_upa(dest_df: pd.DataFrame, source_df: pd.DataFrame, group_features, group_name: str, upa_column_name: str) -> pd.DataFrame:
        with_campaign = group_features.copy()
        with_campaign.append('CAMPAIGN_CD')

        with_upa = with_campaign.copy()
        with_upa.append(group_name)

        group_upa = source_df.groupby(by=with_campaign)[upa_column_name].mean().reset_index()
        group_upa["CUMSUM"] = group_upa.groupby(by=group_features)[upa_column_name].cumsum() - group_upa[upa_column_name]
        group_upa["CUMSUM_WITH_FIRST"] = group_upa.groupby(by=group_features)[upa_column_name].cumsum()
        group_upa["CUMCOUNT"] = group_upa.groupby(by=group_features)[upa_column_name].cumcount()
        group_upa[group_name] = (group_upa['CUMSUM'] / group_upa['CUMCOUNT']).replace({pd.np.nan: None})


        df = dest_df.merge(group_upa[with_upa], left_on=with_campaign, right_on=with_campaign, how="left")
        df[group_name] = df.groupby(by=group_features)[group_name].fillna(method='ffill')
