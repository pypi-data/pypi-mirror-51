import whatap.util.hash_util as hash_util
...
class UseridUtil(object):
    ...
    def setUserId(req, res, cookieValue):
        try:
            if not conf.user_header_ticket_enabled:
    ...
    META_DICT = {'Cookie': lambda x: x.get('HTTP_COOKIE')}

    @staticmethod
    def getHeader(args, name):
        for arg in args:
            if isinstance(arg, dict):
                if name in UseridUtil.META_DICT:
                    fn = UseridUtil.META_DICT[name]
                    return fn(arg)
