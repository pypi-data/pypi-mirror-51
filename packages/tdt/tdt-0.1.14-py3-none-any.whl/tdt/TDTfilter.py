    ...
    if data.streams:
        for k in data.streams.keys():
            ...
            for j in range(time_ranges.shape[1]):
                onset = np.uint64(np.max(np.round((time_ranges[0,j] - data.streams[k].start_time)*fs),0))
                ...
                if np.isinf(time_ranges[1,j]):
                    ...
                    offset = np.uint64(np.max(np.round((time_ranges[1,j] - data.streams[k].start_time)*fs),0))
