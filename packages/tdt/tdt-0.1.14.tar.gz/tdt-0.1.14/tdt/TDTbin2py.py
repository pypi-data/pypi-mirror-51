...
def read_block(block_path, *, bitwise='', channel=0, combine=None, headers=0,
    ...
    for var_name in header.stores.keys():
        ...
        if hasattr(header.stores, var_name):
            try:
                ...
                data[current_type_str][var_name].notes = StructType()
                ts = notes.ts[loc][0]
    ...
    for current_name in header.stores.keys():
        ...
        current_data_format = ALLOWED_FORMATS[data[current_type_str][current_name].dform]
        ...
        if current_type_str == 'epocs':
            ...
            if len(data[current_type_str][current_name].data) > 0:
                data[current_type_str][current_name].data = data[current_type_str][current_name].data[np.newaxis].T
                data[current_type_str][current_name].onset = data[current_type_str][current_name].onset[np.newaxis].T
                data[current_type_str][current_name].offset = data[current_type_str][current_name].offset[np.newaxis].T
            ...
            if current_name in sev_names:
                ...
                data[current_type_str][current_name].start_time = 0
            else:
                ...
                for jj in range(num_ranges):
                    ...
                    minSample = np.uint64(np.maximum(np.ceil((valid_time_range[0,jj]-data[current_type_str][current_name].start_time[jj]) * current_freq),0))
                    maxSample = np.uint64(np.minimum(np.maximum(np.floor((valid_time_range[1,jj]-data[current_type_str][current_name].start_time[jj]) * current_freq),0) + 1, data[current_type_str][current_name].data[jj].shape[1]))
