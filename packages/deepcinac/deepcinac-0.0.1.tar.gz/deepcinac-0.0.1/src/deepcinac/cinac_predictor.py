...
import time
from deepcinac.utils import horizontal_flip, vertical_flip, v_h_flip, create_one_tiff_file_by_frame
...
from deepcinac.cells_map_utils import CellsCoord
...
class CinacTiffMovie(CinacMovie):
    def __init__(self, tiff_file_name):
        ...
        try:
            ...
            #       f"{np.round(stop_time_bis - start_time, 3)} s")
...
class CinacRecording:
    ...
    def set_rois_from_suite_2p(self, is_cell_file_name, stat_file_name):
        ...
        self.coord_obj = CellsCoord(coord=coord, nb_lines=dimensions[0], nb_col=dimensions[1], from_matlab=False)
    ...
    def set_rois_using_pixel_mask(self, pixel_masks):
        ...
        coord_list = []
        for cell in np.arange(pixel_masks):
...
class CinacPredictor:
    ...
    # TODO: allows different network for each cell
    #  make a dict with key being a tuple of string representing the json and weights file_name, and the value
    #  will be the list of cell indices
