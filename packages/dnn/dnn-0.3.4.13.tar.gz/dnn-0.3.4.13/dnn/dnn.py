...
class DNN:
    ...
    def restore (self, for_train  = False):
        ...
        if for_train:
            with open (os.path.join (self.train_dir, "objects"), "rb") as f:
                 ...
                 if len (objects) == 5:
                    global_step, epoch, self.max_performance, self.overfitwatch, self.cherry = objects
                 else:
                    global_step, self.max_performance, self.overfitwatch, self.cherry = objects    
                    metas = [int (os.path.basename (fn).split ("-", 1)[0]) for fn in glob.glob (os.path.join (self.train_dir, "*.meta"))]
                    epoch = metas and max (metas) or 0
    ...
    def save (self, filename = None):
        ...
        with open (os.path.join (self.train_dir, "objects"), "wb") as f:
            pickle.dump ((self.eval (self.global_step), self.epoch, self.max_performance, self.overfitwatch, self.cherry), f)
