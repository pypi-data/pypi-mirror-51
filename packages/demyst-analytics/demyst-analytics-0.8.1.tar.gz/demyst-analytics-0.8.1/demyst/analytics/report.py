...
def attribute_cleanup(attributes, data):
    clean = (
        attributes.assign(
            product_match_rate=attributes.product_name_match.astype(float) / len(data),
            attribute_fill_rate=attributes.attribute_fill.astype(float) / attributes.product_name_match,
            nunique=attributes.unique_values
        )[['product_name', 'product_match_rate', 'attribute_name', 'attribute_fill_rate', 'attribute_type', 'unique_values', 'std', 'median', 'mean', 'max_value', 'min_value', 'variance']]
    )
    # Assume objects are strings
    return clean

def attribute_display(attributes):
    kept_product_names = attributes.product_name.drop_duplicates().values

    for c in kept_product_names:
        sub = attributes[attributes.product_name == c].copy().reset_index()
        display(Markdown("### %s" % sub.product_name[0]))
        display(Markdown("*%0.0f%% match rate  --  %d attributes*" % (
                    sub.product_match_rate[0]*100, len(sub))))
        sub = sub.drop(columns=['index', 'product_name', 'product_match_rate', 'column_0'])
        sub = sub.sort_values(['attribute_fill_rate', 'attribute_name'], ascending=False)
        display(sub)
