...
class Analytics(object):
    ...
    def do_product_stats(self, provider_names, all_products):
        ...
        self.add_extra_stats_columns(df_res)
        # Reorder columns
        return(df_res[[
                    'input_entity',
                    'stats_updated_on',
    ...
    def add_extra_stats_columns(self, df):
        public_providers_list = self.C.auth_get(self.C.get("MANTA_URL") + "public/providers").json()
        public_providers = {}
        ...
        def extract_attribute_use_case(row):
            return ", ".join(map(lambda tag: tag["name"], self.C.lookup_provider(row.at["product"])["tags"]))
    ...
    # In stg, a couple of columns appear to be missing from Hogfish results.
    # Add them if they're not there.
    def fixup_hogfish_df(self, df):
        cols = list(df)
        def ensure_nan_col(name):
            if not name in cols: 
                print("Warning: adding NaN column: " + name, file=sys.stderr)
                df[name] = np.nan
    ...
    def enrich_with_hosted_inputs(self, providers, hosted_input, row_limit=None, filters=None):
        ...
        region = self.C.get("REGION")
        providers = [("hosted_" + region + "_enriched_" + p) for p in providers]
