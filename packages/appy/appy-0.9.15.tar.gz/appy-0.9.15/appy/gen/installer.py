...
class FakeSiteErrorLog:
    ...
    def raising(self, info):
        error = getattr(info[0], '__name__', None)
        if error and (error == 'EvaluationError'):
...
class ZopeInstaller:
    ...
    def installTool(self):
        ...
        tool = self.app.config
        tool.createOrUpdate(True)
        appyTool = tool.appy()
        appyTool.log('Appy version is "%s".' % appy.version.short, noUser=True)
    ...
    def installAppyTypes(self):
        ...
        for klass in self.classes:
            ...
            # Special field "SearchableText" must be added for every class and
            # will allow to display a search widget for entering keywords for
            # searching in index "SearchableText".
            ...
            for baseClass in klass.wrapperClass.__bases__:
                ...
                for name, appyType in baseClass.__dict__.iteritems():
                    ...
                    if appyType.type == 'Switch':
                        # Inject switch sub-fields on the wrapper class
                        appyType.injectFields(wrapperClass)
