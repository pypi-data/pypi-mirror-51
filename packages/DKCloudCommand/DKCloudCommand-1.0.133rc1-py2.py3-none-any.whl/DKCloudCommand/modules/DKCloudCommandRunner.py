        ...
        try:
            ...
            if os.path.exists(os.path.join(rp, recipe_name_param)):
                ...
                if remote_only_recipe_tree is not None:
                    r = DKRecipeDisk(dk_api.get_ignore(), recipe_sha=remote_only_recipe_tree['ORIG_HEAD'], recipe=remote_only_recipe_tree['recipes'][recipe_name_param], path=rp)
                ...
                if merged_different_files is not None:
                    r = DKRecipeDisk(dk_api.get_ignore(), recipe_sha=rl['recipe_sha'], recipe=merged_different_files, path=rp)
                    if not r.save_recipe_to_disk(update_meta=False):
                        rc.set(rc.DK_FAIL, 'Problems saving differences and remote only files to disk. %s' % str(
            ...
            if isinstance(recipe_info, dict) and 'recipes' in recipe_info:
                ...
                if not rc.ok():
                    ...
                    for r in recipes[recipe_name_param]:
                        rs += '  %s\n' % r
                    rc.set_message(rs)
                    d = DKRecipeDisk(dk_api.get_ignore(), recipe_info['ORIG_HEAD'], recipes[recipe_name_param], recipe_path)
        ...
        for item in os.listdir(path):
            if not dk_api.get_ignore().ignore(item):
                ...
                if os.path.isfile(item_path):
                    ...
                    result.update(DKCloudCommandRunner._get_all_files(dk_api, item_path, b64_encode_binary_files=b64_encode_binary_files))
        return result
