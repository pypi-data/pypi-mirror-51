...
hadoop_bin_path = '%s/bin' % os.getenv('HADOOP_HOME')
...
def get_job_id_by_proj(project_id):
    def filter_app(app):
        target_proj_stage = app['name']
        state = app['state']
        if state != 'RUNNING' and state != 'ACCEPTED':
            return False
        if project_id not in target_proj_stage:
            return False
        return target_proj_stage.split('_')[-2] == project_id 
