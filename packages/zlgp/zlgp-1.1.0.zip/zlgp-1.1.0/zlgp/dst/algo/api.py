...
def add_quyu_aliyun(quyu):
    conp_hawq=['gpadmin','since2015','192.168.4.183:5433','base_db','algo']
...
def add_quyu_kunming(quyu):
    conp_hawq=['gpadmin','since2015','192.168.169.90:5433','base_db','algo']
    add_quyu_tmp(quyu,conp_hawq)
...
def add_quyu_all(loc='aliyun',total=None):
    ...
    cost_total=0
    if loc=='aliyun':
        df=db_query("""with a as (SELECT quyu,split_part(quyu,'_',1) as sheng  FROM "public"."cfg" where quyu!~'^zlsys|^zlshenpi|^zljianzhu')
        ...
        if total is None:total=db_query("""select count(*) total   FROM "public"."cfg" where quyu!~'^zlsys|^zlshenpi|^zljianzhu' """,
