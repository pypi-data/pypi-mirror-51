...
def main():
    args = get_core_args()
    Settings.code_root = os.path.realpath(os.path.split(__file__)[0] + '/../..')
    initialize_logger()
    migrate_data()
...
def init_control_center():
    package_name = os.path.basename(os.path.realpath(os.path.split(__file__)[0] + '/..'))
    cc_assets_path = os.path.join(PathManager.get_module_dir(), package_name, 'control_center', 'assets')
