...
from mattapi.api.keyboard.key import Key, KeyModifier
from mattapi.api.keyboard.keyboard_util import get_active_modifiers, is_shift_character
...
class _Keyboard:
    ...
    def type(text: Key or str = None, modifier=None, interval: int = None):
        ...
        if modifier is None:
            ...
            from mattapi.api.keyboard.keyboard_util import get_active_modifiers
            modifier_keys = get_active_modifiers(modifier)
            num_keys = len(modifier_keys)
            logger.debug('Type Method: [Modifiers ({}): {}] + [Text: {}]'
                         .format(num_keys, ' '.join(key.name for key in modifier_keys), text))
