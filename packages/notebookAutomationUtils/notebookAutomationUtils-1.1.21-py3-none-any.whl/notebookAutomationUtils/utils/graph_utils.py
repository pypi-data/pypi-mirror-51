...
class GraphUtils:
    ...
    #     print(obj_[obj_['appVersion']==last_release].head)
        sns.boxplot(ax=ax1, x="entitytype", y="EPT", data=obj_[obj_['releaseVersion']==last_server], order=objects_to_colors.keys(), palette=objects_to_colors.values())
        sns.boxplot(ax=ax2, x="entitytype", y="EPT", data=obj_[obj_['releaseVersion']==current_server], order=objects_to_colors.keys(), palette=objects_to_colors.values())
        plt.ylim(0,None)
        ax1.set_title(str(last_server) + ' - Wifi')
        ax2.set_title(str(current_server) + ' - Wifi')
    ...
    def cmp_aura_flexipage_release_version(self, obj, xaxislimit: int):
      try:
        if 'entitytype' in obj.columns:
          ...
          fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(12,8))
          sns.distplot(obj[obj['release_version']==self.this_release_version]['EPT'], ax=ax, label=str(self.this_release_version), color=self.release_version_colors[self.this_release_version])
          sns.distplot(obj[obj['release_version']==self.last_release_version]['EPT'], ax=ax, label=str(self.last_release_version), color=self.release_version_colors[self.last_release_version])
          plt.legend()
          ax.set_title('EPT')
        plt.show()
      except Exception as e:
