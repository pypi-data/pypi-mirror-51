...
class Test:
    ...
    def run_monkey_with_config(self,ui,change_process):
            ...
            if data is None:
                ...
                if over_num == len(array):
                    ...
                    if CURRENT_INDEX <= cycles:
                        self.run_monkey_with_config(ui,change_process)  
    ...
    def kill_monkey_with_package_name(self,dev,packages=None):
        adb_result = None
        if  platform in ["Windows/Cygwin","Windows"]:
            adb_result= os.popen('adb -s ' + dev + ' shell ps | findstr "monkey"')
        elif platform in ["Mac OS X","OS/2","OS/2 EMX","Linux"]:
...
class TestUI:
    ...
    def get_desktop_path(self):
        ...
        if  platform in ["Windows/Cygwin","Windows"]:
            ...
            desktop_path = _winreg.QueryValueEx(key, "Desktop")[0]
        elif platform in ["Mac OS X","OS/2","OS/2 EMX","Linux"]:
            desktop_path = Config().get_sys_root_path()

        sym_path = "monkeytest/"
        cache_path = desktop_path + Config().get_config("File-store-path","cache_path") + sym_path
