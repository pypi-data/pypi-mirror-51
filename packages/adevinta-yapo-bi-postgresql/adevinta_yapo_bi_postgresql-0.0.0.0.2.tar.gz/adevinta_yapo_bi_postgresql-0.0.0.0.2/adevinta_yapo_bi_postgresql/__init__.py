...
class connect_db(object):
    def __init__(self, db_connection, host, port, dbname, user, password ):
    ...
    def get_connection(self, type ):
        try:
            if type == 'engine':
                self.logger_db.info('Connect to database %s method engine.' % self.db_connection )
                ...
                self.logger_db.info('Connect to database %s method psycopg2.' % self.db_connection )
    ...
    def delete_table( self, query ):
        try:
            self.get_connection('conn_psycopg2')
            cur = self.conn_psycopg2.cursor()
            self.logger_db.info('DELETE %s' % query )
            cur.execute(query)
            self.conn_psycopg2.commit()
            cur.close()
            self.close_connection('conn_psycopg2')
        except Exception as e:
            self.logger_db.error('%s' % e)
            cur.close()
    ...
    def select_table( self, query ):
        try:
            self.logger_db.info('QUERY %s '% query )
    ...
    def insert_table( self, query, schema_write, table_write, if_exists, index, chunksize ):
        try:
            ...
            self.logger_db.info('INSERT INTO %s.%s mode %s commit %s ' % (schema_write, table_write, if_exists, chunksize))
            query.to_sql(name=table_write, con=self.engine, schema=schema_write, if_exists=if_exists, index=index, chunksize=chunksize)
            self.close_connection('engine')
        except Exception as e:
            self.logger_db.error('%s' % e)
            self.engine.connect().close()
            exit(e)
    ...
    def close_connection( self, type ):
        try:
            if type == 'engine':
                self.logger_db.info('Close connection to database %s method engine.' % self.db_connection )
                self.engine.connect().close()
            elif type == 'conn_psycopg2':
                self.logger_db.info('Close connection to database %s method psycopg2.' % self.db_connection )
                self.conn_psycopg2.close()
        except Exception as e:
            self.logger_db.error('%s' % e)
            exit(e)
