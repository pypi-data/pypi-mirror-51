...
class BaseNode(HasName):
    ...
    def formatNodeInfo(self, nodeInfo):
        inputs = {name: Port(**port) for name, port in nodeInfo["inputs"].items()}
        outputs = {name: Port(**port) for name, port in nodeInfo["outputs"].items()}
        params = {name: Param(**param) for name, param in nodeInfo["params"].items()}
