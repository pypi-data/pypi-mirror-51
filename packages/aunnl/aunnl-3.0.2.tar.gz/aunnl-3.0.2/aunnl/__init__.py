        ...
        for x in range(len(self.actfunc)):
            if isinstance(self.actfunc[x], str):
                try:
                    ...
                    actfunc[x] = eval("activations." + self.actfunc[x])
        self.weights = [None] * (len(struct) - 1)
        for x in range(len(self.weights)):
            i = self.struct[x]
            if actfunc[x] == 'sigmoid':
            ...
            elif actfunc[x] == 'linear': # Uses LeCun Normal
        ...
        for x in range(epochs):
            for y in range(n_batches):
                ...
                if verbose == 1:
                    ...
                    pass # print progress bar
                elif verbose == 3:
                    pass # print progress bar for each epoch
