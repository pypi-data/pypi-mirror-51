...
def ccExplore_Recursive(sh: TopoDS_Shape, target_type=ccType[-1]):
...
def ccSh_Pd(poly_data: vtk.vtkPolyData):
    ...
    if sewing.SewedShape().ShapeType() is TopAbs_SHELL and sewing.SewedShape().Closed():
        return ccSolid(sewing.SewedShape())
    else:
...
def ccUniformPointsOnEdge(edge, n, u=None):
    if n < 2:
        return
