...
from . import __urlopen_proxy__ as _urllib2
...
def _register(eureka_server, instance_dic):
    url = f'{_format_url(eureka_server)}apps/v2/{instance_dic["app"]}/{instance_dic["instanceId"]}'
    res = pool.request(
        'POST',
        url,
        body=json.dumps({"instance": instance_dic}).encode(_DEFAULT_ENCODING),
        headers={'Content-Type': 'application/json'},
        timeout=_DEFAULT_TIME_OUT
    )
    # print(res.data)
    # req = _urllib2.Request(url)
    # req.add_header('Content-Type', 'application/json')
    # req.get_method = lambda: "POST"
    # response = _urllib2.urlopen(req, json.dumps({"instance": instance_dic}).encode(_DEFAULT_ENCODING),
    #                            timeout=_DEFAULT_TIME_OUT)
    # response.close()


def cancel(eureka_server, app_name, instance_id):
    url = f"""{_format_url(eureka_server)}apps/{app_name}/{instance_id}"""
    res = pool.request('DELETE', url, timeout=_DEFAULT_TIME_OUT)
    # print(res.data)
    # req = _urllib2.Request(url)
    # req.get_method = lambda: "DELETE"
    # response = _urllib2.urlopen(req, timeout=_DEFAULT_TIME_OUT)
    # response.close()


def send_heart_beat(eureka_server, app_name, instance_id, last_dirty_timestamp, status=INSTANCE_STATUS_UP,
                    overriddenstatus=""):
    url = f"""{_format_url(eureka_server)}apps/{app_name}/{instance_id}?status={status}&lastDirtyTimestamp={str(
    ...
    res = pool.request('PUT', url, timeout=_DEFAULT_TIME_OUT)
    # print(res.data)
    # req = _urllib2.Request(url)
...
def delete_status_override(eureka_server, app_name, instance_id, last_dirty_timestamp):
    url = f"""{_format_url(eureka_server)}apps/{app_name}/{instance_id}/status?lastDirtyTimestamp={str(
        last_dirty_timestamp)}"""
    res = pool.request('DELETE', url, timeout=_DEFAULT_TIME_OUT)
    # print(res.data)
...
def _get_applications_(url, regions=[]):
    ...
    # print(res.data)
    txt = str(res.data, encoding=_DEFAULT_ENCODING)
    # f = _urllib2.urlopen(_url, timeout=_DEFAULT_TIME_OUT)
...
def get_application(eureka_server, app_name):
    url = f"""{_format_url(eureka_server)}apps/{app_name}"""
    res = pool.request(
        method='GET',
        url=url,
        timeout=_DEFAULT_TIME_OUT
    ...
    # f = _urllib2.urlopen(url, timeout=_DEFAULT_TIME_OUT)
    # txt = f.read().decode(_DEFAULT_ENCODING)
    # f.close()
...
class RegistryClient:
    ...
    def __init__(self,
        ...
        def try_to_get_client_ip(url):
            url_addr = _urllib2.get_url_and_basic_auth(url)[0]
        ...
        self.__instance = {
            ...
            if instance_id != ""
            else f'{(self.__instance_host, app_name.lower(), instance_port)}',
    ...
    def __try_all_eureka_server(self, fun):
        with self.__net_lock:
            ...
            while len(untry_servers) > 0:
                ...
                try:
                    fun(url)
                        ...
                        _urllib2.HTTPError, _urllib2.URLError, urllib3.exceptions.HTTPError,
            ...
            if not ok:
                raise _urllib2.URLError("All eureka servers are down!")
...
class DiscoveryClient:
    ...
    def walk_nodes(self, app_name="", service="", prefer_ip=False, prefer_https=False, walker=None):
        ...
        raise _urllib2.HTTPError(url='service/error', code=500, msg="Try all up instances in registry, but all fail",
                                 hdrs=None, fp=None)
    ...
    def do_service(self, app_name="", service="", return_type="string",
        ...
        def walk_using_urllib(url):
            ...
            response = _urllib2.urlopen(req, data=data, timeout=timeout, cafile=cafile, capath=capath,
                                        cadefault=cadefault, context=context)
