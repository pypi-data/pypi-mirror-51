...
def jsonEncoder(avroType, value, tagged=True):
    ...
    if isinstance(avroType, AvroNull) and value is None:
        ...
        return float(value)
    elif isinstance(avroType, AvroInt) and isinstance(value, np.integer) and value is not True and value is not False:
        return value.item()
    elif isinstance(avroType, AvroLong) and isinstance(value, np.integer) and value is not True and value is not False:
        return value.item()
    elif isinstance(avroType, AvroFloat) and isinstance(value, (np.integer, np.floating)) and value is not True and value is not False:
        return float(value.item())
    elif isinstance(avroType, AvroDouble) and isinstance(value, (np.integer, np.floating)) and value is not True and value is not False:
        return float(value.item())
