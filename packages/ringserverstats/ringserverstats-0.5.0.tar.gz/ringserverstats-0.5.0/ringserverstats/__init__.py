...
def influx_group_query(idbclient: InfluxDBClient, start: int, end: int):
    ...
    We can not use influxdb's continuous queries because of the inner scheduling of the CQ considering that there will never be data written in the past. But this program does write data for the past day.
    """
    # check if start < end
    if end < start:
