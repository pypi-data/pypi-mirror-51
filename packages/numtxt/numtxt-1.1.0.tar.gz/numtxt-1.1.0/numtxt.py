...
##
##    heads = ['number', 'cardinal', 'ordinal', 'precedence']
##    funcs = [str, cardinal, ordinal, precedence]
##
##    h = len(heads)
##    line = '|'.join([' {:^{}} '] * h)
##
##    lst = [[f(i) for f in funcs] for i in range(n)]
##    mxs = [len(max([q[j] for q in lst], key=len)) for j in range(h)]
##    mxs = [max(mxs[i], len(heads[i])) for i in range(h)]  # account for headers
##    
##    print(line.format(*[q for u in zip(heads, mxs) for q in u]))
##    print('+'.join(['-' * (i + 2) for i in mxs]))  # +2 for spacing
##    for i in lst: print(line.format(*[q for u in zip(i, mxs) for q in u]))
##
##def gen10_dict(n):
##    "generate n powers of ten names; {10^i: ith-illion}"
##    return {i: approx(10 ** i).split(' ', 1)[-1] for i in range(3, n * 3, 3)}
##
##def gen1000_dict(n):
##    "generate n powers of thousand names; {1000^i: ith-illion}"
##    return {i: approx(1000 ** i).split(' ', 1)[-1] for i in range(1, n + 1)}
