...
def run_create(index_path, ref, profile_name, profile_url):
    ...
    if ((settings and settings["os_build"] == "Linux") or
       (not settings and cur_os == "Linux")) and docker_present():
        ...
        create = "conan create recipe {}@cbot/stable".format(ref)
        inside = "mkdir -p /tmp/conan_home " \
                 "&& pip install conan --upgrade " \
                 "&& pip install cbot-client --upgrade " \
                 "&& cbot install_hook " \
                 "&& ls -la recipe/test_package && " \
                 "rm -rf ./recipe/test_package/build && " + create
        cmd = 'docker run --rm -v{}:/home/conan/recipe -v{}:/tmp/conan_home/profiles ' \
              '-e CONAN_HOOK_ERROR_LEVEL=40 -e CONAN_USER_HOME=/tmp/conan_home ' \
              '{} /bin/bash -c "{}"'.format(recipe_folder, os.path.dirname(profile_path),
                                            docker_image, inside)
    else:
        cmd = "conan create . {}@cbot/stable --profile {}".format(ref, profile_path)
