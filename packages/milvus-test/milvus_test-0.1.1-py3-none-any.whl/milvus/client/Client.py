...
class Milvus(ConnectIntf):
    ...
    def search_vectors_in_files(self, table_name, file_ids, query_records, top_k, query_ranges=None, raw=False):
        """
        ...
        """
        with self.mutex:
            return self._search_vectors_in_files_bin(table_name, file_ids, query_records, top_k, query_ranges, raw)
        ...
        try:
            ...
            for topks in results:
                ...
                if struct.calcsize('l') == 8:
                    ...
                    raise struct.error("Error occur when unpack")
    ...
    def _search_vectors_in_files_bin(self, table_name, file_ids, query_records, top_k, query_ranges=None, raw=False):
        if not self.connected():
            raise NotConnectError('Please Connect to the server first!')
        ...
        try:
            ...
            if raw is True:
                return Status(Status.SUCCESS, message='Search Vectors successfully!'), top_k_query_results

            for topks in top_k_query_results:
                ids = topks.id_array
                distances = topks.distance_array
                count = len(ids) // 8
                assert count == len(distances) // 8
