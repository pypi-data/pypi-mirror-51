...
class RelayAgentRemoteId(Base):
	...
	"""
	YANG_NAME = 'relay-agent-remote-id'
	...
	def __init__(self, parent):
		super(RelayAgentRemoteId, self).__init__(parent)
	...
	def Increment(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.relayagentremoteid.increment.increment.Increment)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.relayagentremoteid.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.relayagentremoteid.decrement.decrement.Decrement)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.relayagentremoteid.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.relayagentremoteid.random.random.Random)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.relayagentremoteid.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `relay-agent-remote-id` resource
