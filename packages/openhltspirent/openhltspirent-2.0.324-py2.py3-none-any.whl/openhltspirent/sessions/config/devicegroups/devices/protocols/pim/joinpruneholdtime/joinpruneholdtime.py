...
class JoinPruneHoldtime(Base):
	"""Hold time in seconds to advertise in Join/Prune messages.
	"""
	YANG_NAME = 'join-prune-holdtime'
	...
	def __init__(self, parent):
		super(JoinPruneHoldtime, self).__init__(parent)
	...
	def Increment(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.pim.joinpruneholdtime.increment.increment.Increment)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.pim.joinpruneholdtime.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.pim.joinpruneholdtime.decrement.decrement.Decrement)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.pim.joinpruneholdtime.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.pim.joinpruneholdtime.random.random.Random)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.pim.joinpruneholdtime.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `join-prune-holdtime` resource
