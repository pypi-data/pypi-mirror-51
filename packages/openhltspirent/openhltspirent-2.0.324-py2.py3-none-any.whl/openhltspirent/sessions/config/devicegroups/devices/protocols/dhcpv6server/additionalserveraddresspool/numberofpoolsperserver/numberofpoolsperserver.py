...
class NumberOfPoolsPerServer(Base):
	"""Number of pools per server.
	"""
	YANG_NAME = 'number-of-pools-per-server'
	...
	def __init__(self, parent):
		super(NumberOfPoolsPerServer, self).__init__(parent)
	...
	def Increment(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.numberofpoolsperserver.increment.increment.Increment)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.numberofpoolsperserver.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.numberofpoolsperserver.decrement.decrement.Decrement)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.numberofpoolsperserver.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.numberofpoolsperserver.random.random.Random)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.numberofpoolsperserver.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `number-of-pools-per-server` resource
