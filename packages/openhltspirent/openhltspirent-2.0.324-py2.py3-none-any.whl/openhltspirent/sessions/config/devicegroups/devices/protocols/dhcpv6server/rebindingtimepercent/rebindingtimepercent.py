...
class RebindingTimePercent(Base):
	"""Rebinding lease time (T2) as a percentage of Lease Time.
	"""
	YANG_NAME = 'rebinding-time-percent'
	...
	def __init__(self, parent):
		super(RebindingTimePercent, self).__init__(parent)
	...
	def Increment(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.rebindingtimepercent.increment.increment.Increment)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.rebindingtimepercent.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.rebindingtimepercent.decrement.decrement.Decrement)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.rebindingtimepercent.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.rebindingtimepercent.random.random.Random)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.rebindingtimepercent.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `rebinding-time-percent` resource
