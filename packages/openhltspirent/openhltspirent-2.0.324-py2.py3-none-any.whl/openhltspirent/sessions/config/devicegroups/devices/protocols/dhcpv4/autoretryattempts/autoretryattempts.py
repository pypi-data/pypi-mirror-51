...
class AutoRetryAttempts(Base):
	"""Number of retry attempts.
	"""
	YANG_NAME = 'auto-retry-attempts'
	...
	def __init__(self, parent):
		super(AutoRetryAttempts, self).__init__(parent)
	...
	def Increment(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv4.autoretryattempts.increment.increment.Increment)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv4.autoretryattempts.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv4.autoretryattempts.decrement.decrement.Decrement)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv4.autoretryattempts.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv4.autoretryattempts.random.random.Random)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv4.autoretryattempts.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `auto-retry-attempts` resource
