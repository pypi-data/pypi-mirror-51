...
class StartIpv6Address(Base):
	"""Pool starting IPv6 address.
	"""
	YANG_NAME = 'start-ipv6-address'
	...
	def __init__(self, parent):
		super(StartIpv6Address, self).__init__(parent)
	...
	def Increment(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.startipv6address.increment.increment.Increment)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.startipv6address.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.startipv6address.decrement.decrement.Decrement)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.startipv6address.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.startipv6address.random.random.Random)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6server.additionalserveraddresspool.startipv6address.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `start-ipv6-address` resource
