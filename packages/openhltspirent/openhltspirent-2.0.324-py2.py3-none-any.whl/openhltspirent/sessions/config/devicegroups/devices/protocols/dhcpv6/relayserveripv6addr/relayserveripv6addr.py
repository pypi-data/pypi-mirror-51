...
class RelayServerIpv6Addr(Base):
	"""Relay server IPv6 address.
	"""
	YANG_NAME = 'relay-server-ipv6-addr'
	...
	def __init__(self, parent):
		super(RelayServerIpv6Addr, self).__init__(parent)
	...
	def Increment(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6.relayserveripv6addr.increment.increment.Increment)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6.relayserveripv6addr.increment.increment import Increment
	...
	def Decrement(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6.relayserveripv6addr.decrement.decrement.Decrement)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6.relayserveripv6addr.decrement.decrement import Decrement
	...
	def Random(self):
		...
		Returns:
			obj(openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6.relayserveripv6addr.random.random.Random)
		"""
		from openhltspirent.sessions.config.devicegroups.devices.protocols.dhcpv6.relayserveripv6addr.random.random import Random
	...
	def update(self, PatternType=None, PatternFormat=None, Single=None, ValueList=None):
		"""Update the current instance of the `relay-server-ipv6-addr` resource
