...
def tar(tar: str, package: str, env: str = '', exclude: Optional[Tuple] = None) -> None:
    ...
    command = f'tar {exclude_option} -cvf {tar} {package}'
    _run(command, env)
...
def fab_pypi_project(ctx, interpreter: str, project_name: str, version: str):
    retry = 5
    python = interpreter
    current_dir = File.this_dir(__file__)
    update_version(f'{current_dir}/setup.py', version)
    local(f'bash {current_dir}/pypi.sh')
    ...
    for i in range(retry):
        if current_version != f'{project_name}=={version}':
            ...
            local(f'{python} -m pip install {project_name}=={version}')
        else:
            break
        time.sleep(2)
