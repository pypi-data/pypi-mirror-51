...
class Learner:
    def __init__(
        self, path, data, base_model, input_shape, dropout=0.0, l1=1e-8, l2=1e-8
    ):
        self.path = str(path)
    ...
    def save(self):
        save_keras_model(
            self.model, f"{self.path}/model.json", f"{self.path}/weights.h5"
        )

    def load(self, weights_only=False):
        if weights_only:
            self.model.load_weights(f"{self.path}/weights.h5")
        else:
            self.model = load_keras_model(
                f"{self.path}/model.json", f"{self.path}/weights.h5"
            )
