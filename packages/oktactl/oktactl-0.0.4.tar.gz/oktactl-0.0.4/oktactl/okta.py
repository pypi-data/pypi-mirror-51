...
def create_groups():
    ...
    with click.progressbar(zip(group_name_list, group_desc_list)) as bar:
        for group_name, group_desc in bar:
            ...
            try:
                ...
                if group_response.status_code == 200:
                    successfully_created_groups.append(group_id)
                    
                    app_url = "{}/apps/{}/groups/{}".format(base_url, app_id, group_id)
                    requests.put(app_url, headers=headers)
                    click.secho("Group {} has been assigned to the app {} successfully".format(group_id, app_id), fg='green')
                else:
    ...
    if successfully_created_groups:
        click.secho("List of groups created successfully: {}".format(successfully_created_groups))
    if failed_groups:
        click.secho("List of group failed to create: {}".format(failed_groups))
