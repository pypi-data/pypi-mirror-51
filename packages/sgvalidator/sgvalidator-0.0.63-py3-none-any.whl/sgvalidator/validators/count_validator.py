...
class CountValidator:
    TOTAL_COUNT_TRUTHSET_PATH = pkg_resources.resource_filename("sgvalidator",
                                                                "validators/data/brand_count_truthset.csv")
    ...
    def getPoiCountsByState(data):
        valueCounts = data["state"].value_counts()
        poiCountsByState = pd.DataFrame(valueCounts).reset_index().rename({"index": "state", "state": "count"}, axis=1)
        return CountValidator.cleanStateAndFilterMissing(poiCountsByState)

    @staticmethod
    def cleanStateAndFilterMissing(data):
        notBlank = data[data["state"].apply(ValidatorUtils.is_not_blank)]
        notBlank["canonicalized_state"] = notBlank["state"].apply(CountValidator.getCanonicalizedState)
        return notBlank[notBlank["canonicalized_state"].apply(ValidatorUtils.is_not_blank)]
    ...
    def getExpectedCountsByStateFromTruthset(domain=None):
        ...
        if not domain:
            ...
            domain = domain
        return truthset[truthset["website"] == domain]
