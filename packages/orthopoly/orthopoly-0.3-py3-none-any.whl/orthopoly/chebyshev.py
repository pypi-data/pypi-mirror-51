...
The first couple derivatives of the polynomials can be evaluated with respect to :math:`\theta`, using the chain rule to convert to :math:`\hat{x}` coordinates. Derivatives then must be scaled to :math:`[x_a,x_b]` by dividing by :math:`((x_b - x_a)/2)^p`, where :math:`p` is the order of the derivative.
