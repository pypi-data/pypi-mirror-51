...
def create_default_config():
    """Create the default configuration file in the home directory if it doesn't
    exist."""
    home = os.getenv('HOME')
    location = home + '/.chromaterm.yml'

    if os.access(location, os.F_OK):  # Already exists
        return

    if not os.access(home, os.W_OK):  # Directory is not writable
        return
