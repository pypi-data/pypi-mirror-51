...
def run_analyzer_on_local_code(
    ...
    if isinstance(analyzer_input, LocalCode):
        ...
        # This may run into issues if there are multiple special analyzers. Let's
        # ignore that for now.
        for fetcher in fetchers:
            copy_local_input(analyzer, fetcher.versioned_analyzer, analyzer_input)

            # Clear any cached output.
            for output_store in [json_output_store, filesystem_output_store]:
                output_store.delete(analyzer_input, specified_analyzer)
    ...
    if manifest.output_type == AnalyzerOutputType.json:
        json_output_store.get(analyzer_input, specified_analyzer, output_path_used)
    elif manifest.output_type == AnalyzerOutputType.filesystem:
        filesystem_output_store.get(
            analyzer_input, specified_analyzer, output_path_used
        )
...
def copy_local_input(
    analyzer: Analyzer, va: VersionedAnalyzer, analyzer_input: LocalCode
) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix=LOCAL_RUN_TMP_FOLDER) as mount_folder:
        ...
        if os.name == "nt":
            ...
            shutil.copytree(
                code_dir, output_fs_path, symlinks=True, ignore_dangling_symlinks=True
            )
