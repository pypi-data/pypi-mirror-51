...
def shComputeI(generation, generations, population_size, minimal_size):
    a = math.floor(math.log(population_size/float(minimal_size),2)+1) / float(generations+1)
    return int(math.floor(a*generation))
...
def eaMuPlusLambda(population, toolbox, mu, lambda_, cxpb, mutpb, ngen, pbar,
                   ...
                   successive_halving=False, min_individuals=None):
    ...
    *ngen* generations are done, the algorithm returns a tuple with the final
    ...
    for gen in range(1, ngen + 1):
        ...
        if successive_halving:
            if min_individuals:
                lambda_ = int(initial_population_size / math.pow(2, shComputeI(gen, ngen, initial_population_size, min_individuals)))
            elif (lambda_ / 2) > 1:
                lambda_ = int(lambda_ / 2)
            else:
                print('Warning, lambda_ cannot be divided anymore (gen ' + str(gen) + ')')
        ...
        if successive_halving:
            if min_individuals:
                mu = int(initial_population_size / math.pow(2, shComputeI(gen, ngen, initial_population_size, min_individuals)))
            elif (mu / 2) > 1:
                mu = int(mu / 2)
            else:
                print('Warning, mu cannot be divided anymore (gen ' + str(gen) + ')')
