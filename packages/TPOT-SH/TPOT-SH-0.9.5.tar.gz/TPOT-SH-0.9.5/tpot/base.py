...
class TPOTBase(BaseEstimator):
    ...
    def __init__(self, generations=100, population_size=100, offspring_size=None,
                 ...
                 successive_halving=False, min_budget=100, max_budget=None, min_individuals=None):
    ...
    def _fit_init(self):
        ...
        if self.successive_halving:
            self.successive_halving_seed = random.randint(1, 2**32-1)
        else:
            self.successive_halving_seed = None
    ...
    def _check_periodic_pipeline(self):
        ...
        self._current_generation = self._current_generation + 1
    ...
    def _evaluate_individuals(self, individuals, features, target, sample_weight=None, groups=None):
        ...
        # Successive Halving (double the dataset size)
        if self.successive_halving and self._current_generation >= 0:
            if not self.max_budget:
                self.max_budget = features.shape[0]

            if self.max_budget < self.min_budget:
                raise Exception("Min budget is superior to the maximum budget")

            sh_budget = lambda i, b=120., B=2500., G=100. : b * math.pow(2, math.floor(i * math.floor(math.log(B/b, 2) + 1)/(G+1)))

            currentBudget = int(sh_budget(i=self._current_generation, b=self.min_budget, B=self.max_budget, G=self.generations))

            features = RandomState(self.successive_halving_seed).permutation(features)[:currentBudget]
            target = RandomState(self.successive_halving_seed).permutation(target)[:currentBudget]
