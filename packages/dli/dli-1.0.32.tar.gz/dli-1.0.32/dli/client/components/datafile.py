...
class Datafile(SirenComponent):
    ...
    @property
    def _root(self):
        return self._get_root().datafiles_root()
    ...
    def register_datafile_metadata(
        ...
        return siren_to_entity(self._root.create_datafile(__json=payload))
    ...
    def get_s3_datafile(self, datafile_id):
        ...
        keys = self.client.get_s3_access_keys_for_dataset(datafile.dataset_id)
        s3_access = Client(
            keys.access_key_id,
    ...
    def download_datafile(self, datafile_id, destination):
        ...
        keys = self.client.get_s3_access_keys_for_dataset(datafile.datasetId)
        s3_access = Client(
            keys.access_key_id,
            keys.secret_access_key,
            keys.session_token
    ...
    def _process_s3_upload(self, files, s3_location, dataset_id):
        s3_access_keys = self.client.get_s3_access_keys_for_dataset(dataset_id)
        ...
        return s3_client.upload_files_to_s3(files, s3_location, lambda: self.client.get_s3_access_keys_for_dataset(dataset_id))
    ...
    def _get_datafile(self, datafile_id):
        ...
        datafile = self._root.get_datafile(datafile_id=datafile_id)
