...
class RegisterDatafileTestCase(SdkIntegrationTestCase):
    ...
    def test_download_datafile_should_keep_folder_structure_as_in_datafile(self):
        ...
        def object_summary(Prefix):
            summary = MagicMock(key=Prefix)
            summary.Object().download_file = download_file
            return [summary]

        s3_resource.Bucket().objects.filter = object_summary
        ...
        with patch('dli.client.components.datafile.Client', TestS3Client), \
            ...
            files_created = [
                f[0][0] for f in download_file.call_args_list
            ]
