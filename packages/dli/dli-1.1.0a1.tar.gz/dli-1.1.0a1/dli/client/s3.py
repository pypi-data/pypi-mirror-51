...
class Client:
    ...
    def from_dataset(cls, client, dataset_id):
        def _credentials():
            ...
            expiry_time = (
                ...
                datetime.now().replace(tzinfo=tzlocal()) + timedelta(hours=30)
            ).isoformat()
    ...
    def __init__(self, boto_session):
        ...
        self.s3fs.s3 = self.s3_resource.meta.client
    ...
    def download_files_from_s3_path(self, s3_path, destination):
        ...
        try:
            self.s3_resource.meta.client.head_bucket(Bucket=parse_result.netloc)
        except self.s3_resource.meta.client.exceptions.ClientError as e:
            raise S3FileDoesNotExist(path)

        bucket = self.s3_resource.Bucket(parse_result.netloc)

        for obj_summary in bucket.objects.filter(Prefix=path):
            # Ignore 'folders'
            if not obj_summary.key.endswith('/'):
                destinatation_path = os.path.join(
                    destination, obj_summary.key
                )
            ...
            obj_summary.Object().download_file(destinatation_path)
...
class S3DatafileWrapper:
    ...
    def files(self):
        ...
        collection = self._client.s3_resource.Bucket(bucket).objects.filter(
            Prefix=prefix.lstrip('/')
        )

        return ['{}/{}'.format(bucket, c.key) for c in collection]
