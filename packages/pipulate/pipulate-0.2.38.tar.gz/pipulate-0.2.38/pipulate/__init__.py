...
def get_email(oauthfile=filename):
    ...
    service = create_google_service("oauth2", "v2", oauthfile)
...
def create_google_service(api_name, version, oauthfile=filename):
...
def ga(oauthfile=filename):
    ...
    return create_google_service("analytics", "v3", oauthfile)


def gsc(oauthfile=filename):
    """Return instance of Google Search Console object."""
    return create_google_service("webmasters", "v3", oauthfile)
