...
def handle_creation(request, node_model, node_model_name, available_objects_dict):
    print("\nHandle\n")
    admin_request_post = dict(request.POST)
    admin_request_files = dict(request.FILES)
    admin_request = {**admin_request_post, **admin_request_files}
...
def node_creation_view(request, node_model_name):
    ...
    if request.user.has_perm("create_" + node_model_name.lower()) or request.user.has_perm("create"):
        ...
        if request.POST or request.FILES:
            ...
            response = pool.apply_async(handle_creation, args={"request": request, "node_model": node_model, "node_model_name": node_model_name, "available_objects_dict": available_objects_dict})

            print("RESPONSE = ", response)
            print("RESPONSE = ", response.__dict__)
            # print("RESPONSE = ", response.get())
            print("RESPONSE = ", response.ready())
            # print("RESPONSE = ", response.successful())
