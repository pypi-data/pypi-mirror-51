...
class WorkflowTaskInfo:
    ...
    def get_etl_durations(self, dag_id, task_id, batch_size, extra_where="", force_refresh=False):
        end_date = date.today() - timedelta(days=1)
        start_date = end_date - timedelta(days=14)

        if force_refresh:
            self._write_etl_durations(dag_id, task_id, batch_size, extra_where)

        cached_results = self._get_cached_durations_df(dag_id, task_id, start_date, end_date)
    ...
    def _get_cached_durations_df(self, dag_id, task_id, start_date, end_date):
        df_list = []
        sdate = start_date
        while sdate < end_date:
            iso_strdate = sdate.strftime("%Y%m%d")

            filename = "{}_{}_{}.csv".format(dag_id, task_id, iso_strdate)
    ...
    def _write_etl_durations(self, dag_id, task_id, batch_size, extra_where=""):
        """
        ...
        for isodate, new_rows in records.items():
            filename = "{}_{}_{}.csv".format(dag_id, task_id, isodate)
            ...
            with self._fs.open(s3_fname, "w") as fout:
                ...
                print("Wrote rows: {}".format(new_rows))
