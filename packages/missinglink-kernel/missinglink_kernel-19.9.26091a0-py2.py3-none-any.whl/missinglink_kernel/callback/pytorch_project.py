...
class _Experiment(BaseVanillaExperiment):
    ...
    @contextmanager
    def test(self, model=None, test_data_object=None, target_attribute_name='label', steps=None, name=None, **kwargs):
        """
        ...
        with super(_Experiment, self).test(steps, name=name) as test_phase:
            if model is None:
                model = self.model
