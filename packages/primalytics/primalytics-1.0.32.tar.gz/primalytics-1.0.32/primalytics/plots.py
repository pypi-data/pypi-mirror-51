...
def plot_univariate(df: pandas.DataFrame, x_list: list, y_list: list,
    ...
    if len(x_list) > 1 and order is not None:
        if not isinstance(order, dict) and order not in order_choices:
            raise Exception(
                'Error! order provided is invalid. Possible choices are: {} or a dict shaped as {{x: order for x in x_list}}'.format(
                    order_choices))
        if isinstance(order, dict):
            if sum([x not in order.keys() for x in x_list]) > 0:
                raise Exception(
                    'Error! not all the variables specified in \'x_list\' are present in \'order\' dict')
    ...
    for x in x_list:
        ...
        if order is None:
            ...
            if order in df.columns:
                ...
                raise Exception(
                    'Error! cannot order by variable \'{}\' as is not present in input dataset'.format(
                        order))
            ...
            if len(x_list) == 1:
                ...
                raise Exception(
                    'Error! {} plotting variables specified but only one order list provided.'.format(
                        len(x_list)))
