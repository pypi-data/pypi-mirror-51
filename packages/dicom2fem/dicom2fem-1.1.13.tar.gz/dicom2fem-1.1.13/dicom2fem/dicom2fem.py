...
smooth_methods = {
    "taubin vol.": (
        smooth_mesh,
        {"n_iter": 10, "volume_corr": True, "lam": 0.6307, "mu": -0.6347},
    ),
    "taubin": (
        smooth_mesh,
        {"n_iter": 10, "volume_corr": False, "lam": 0.6307, "mu": -0.6347},
    ),
}
...
class MainWindow(QMainWindow):
    ...
    def init_MeshGenTab(self):
        ...
        supp_write = [k for k, v in supported_capabilities.iteritems() if "w" in v]
    ...
    def loadDcmDir(self):
        ...
        if self.dcmdir is not None:
            dcr = dcmreader.DicomReader(os.path.abspath(self.dcmdir), qt_app=self)
        else:
    ...
    def getRescaleValues(self, new_vsize, old_vsize, labels):
        ...
        value, ok = QInputDialog.getText(self, labels[0], labels[1], text=aux)
    ...
    def rescaleDcm(self, event=None, new_vsize=[1.0, 1.0, 1.0]):
        ...
        if event is not None:
            zoom = self.getRescaleValues(
                new_vsize, self.voxel_sizemm, ("Rescale DICOM data", "Voxel size [mm]:")
            )

        if zoom is not None:
            self.dcm_zoom *= zoom
            self.dcm_3Ddata = ndimage.zoom(
                self.dcm_3Ddata, zoom, prefilter=False, mode="nearest"
            )
    ...
    def cropDcm(self):
        ...
        pyed = QTSeedEditor(self.dcm_3Ddata, mode="crop", voxelSize=self.voxel_sizemm)
        pyed.exec_()
            ...
            if filename is None:
                filename = str(
                    QFileDialog.getSaveFileName(
                        self, "Save DCM file", filter="Files (*.dcm)"
                    )
                )
            if len(filename) > 0:
                savemat(
        ...
        if filename is None:
            filename = str(
                QFileDialog.getOpenFileName(
                    self, "Load DCM file", filter="Files (*.dcm)"
                )
            )
    ...
    def maskRegion(self):
        ...
        pyed = QTSeedEditor(self.dcm_3Ddata, mode="mask", voxelSize=self.voxel_sizemm)
    ...
    def saveSeg(self, event=None, filename=None):
        if self.segmentation_data is not None:
            ...
            if filename is None:
                filename = str(
                    QFileDialog.getSaveFileName(
                        self, "Save SEG file", filter="Files (*.seg)"
                    )
                )
    ...
    def loadSeg(self, event=None, filename=None):
        if filename is None:
            filename = str(
                QFileDialog.getOpenFileName(
                    self, "Load SEG file", filter="Files (*.seg)"
                )
            )
    ...
    def rescaleSeg(self, event=None, new_vsize=[1.0, 1.0, 1.0]):
        ...
        zoom = self.getRescaleValues(
            new_vsize,
            self.voxel_sizemm,
            ("Rescale segmentation data", "Grid size [mm]:"),
        )

        if zoom is not None:
            self.dcm_zoom *= zoom
            self.segmentation_data_scaled = ndimage.zoom(
                self.segmentation_data, zoom, prefilter=False, mode="nearest"
            )
    ...
    def saveMesh(self, event=None, filename=None):
        if self.mesh_data is not None:
            ...
            if filename is None:
                ...
                filename = str(
                    QFileDialog.getSaveFileName(
                        self, "Save MESH file", filter="Files (*%s)" % file_ext
                    )
                )

            if len(filename) > 0:

                io = MeshIO.for_format(
                    filename, format=self.mesh_out_format, writable=True
                )
    ...
    def generMesh(self):
        def getScaleFactor(self, value=0.25):
            label = (
                "Marching cubes - volume remeshing",
                "Characteristic Length Factor:",
            )
            value, ok = QInputDialog.getText(
                self, label[0], label[1], text="%.2f" % value
            )
            if ok and value is not None:
                value = float(value)
        ...
        if segdata is not None:
            ...
            self.setLabelText(
                self.text_mesh_data,
                "%d %s" % (self.mesh_data.n_el, elem_tab[self.mesh_data.descs[0]]),
            )
    ...
    def smoothMesh(self):
        ...
        if self.mesh_data is not None:
            ...
            etype = "%d_%d" % (self.mesh_data.dim, self.mesh_data.conns[0].shape[-1])
