...
class MessageHandler(object):
    ...
    def __handle_outgoing_message(self, message):
        ...
        if len(message) == 2:
            self.producer.produce(topic=self.output_topic,key=message[0],value=message[1], callback=self.__delivery_report)
        elif len(message) == 1:
            self.producer.produce(self.output_topic,message, callback=self.__delivery_report)
        else:
            raise KeyError("message length is invalid: %s".format(message))
        self.producer.flush()
    ...
    def log_formatter(self, correlationId, format_string, *args, **kwargs):
        return str.format('|'+str(correlationId)+'| '+ format_string, args, kwargs)
