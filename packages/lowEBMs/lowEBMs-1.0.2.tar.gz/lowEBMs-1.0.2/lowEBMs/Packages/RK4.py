...
def rk4alg(config,progressbar=True,monthly=False):
    ...
    explained above by using the solution of the model equation from ``lowEBMs.Packages.ModelEquations``. 
    ...
    The required input-argument is the configuration dictionary returned by ``lowEBMs.Packages.Configuration.importer``.
