...
class Price(GraphenePrice):
    """ This class deals with all sorts of prices of any pair of assets to
        ...
        each being an instance of :class:`meta1-testnet.amount.Amount`. The
        ...
        Way to obtain a proper instance:
            ...
            * ``args`` can be a floating number and ``base`` and ``quote`` being instances of :class:`meta1-testnet.asset.Asset`
            ...
            * ``args`` being a list of ``[quote, base]`` both being instances of :class:`meta1-testnet.amount.Amount`
            * ``args`` being a list of ``[quote, base]`` both being instances of ``str`` (``amount symbol``)
            * ``base`` and ``quote`` being instances of :class:`meta1-testnet.asset.Amount`

        This allows instanciations like:

        * ``Price("0.315 USD/TEST1")``
        * ``Price(0.315, base="USD", quote="TEST1")``
        * ``Price(0.315, base=Asset("USD"), quote=Asset("TEST1"))``
        ...
        * ``Price(1.0, "USD/GOLD")``
        ...
        .. code-block:: python
            ...
            >>> from meta1-testnet.price import Price
            >>> Price("0.3314 USD/TEST1") * 2
            0.662600000 USD/TEST1
    ...
    def market(self):
        """ Open the corresponding market
            ...
            :returns: Instance of :class:`meta1-testnet.market.Market` for the
...
class Order(Price):
    """ This class inherits :class:`meta1-testnet.price.Price` but has the ``base``
