...
PARAMS = ["ZPiezoconst", "LockinRC", "Vertmandelay", "VertSpecBack", "LockinAmpl", "Current[A]", "SetPoint", "DSP_Clock",
            "DAC-Type", "Gainpreamp", "Length x[A]", "Length y[A]"]
re_string = f"({'|'.join(PARAMS)})=(.+)\\n"
re_string = re_string.replace('[', r'\[').replace(']', r'\]')
param_re = re.compile(re_string)
...
def load_VERT_file(filename, stm = STM()):
    ...
    file = file.split('DATA') # makes list of len 2: first elem is params, second elem is data
    params = param_re.findall(file[0])
    p_dict = dict()
    for p, v in params:
        try:
            p_dict[p] = float(v)
    ...
    data = np.fromstring(file[1], sep='\t')
    N, x, y = data[0], data[1], data[2]
    final_elem = int(N * 5)  # to remove comments at the end
    data = data[4:final_elem + 4].reshape((int(N), 5))
    ...
    ret.__dict__['ZPiezoconst'] = p_dict["ZPiezoconst"]
    ret.__dict__['LockinRC'] = p_dict["LockinRC"] * LockinRC_factor # in Hz
    ret.__dict__['Speclength'] = p_dict["Vertmandelay"] * N / p_dict["DSP_Clock"]
    ret.__dict__['VertSpecBack'] = int(p_dict["VertSpecBack"])
    ret.__dict__['LockinAmpl'] = p_dict["LockinAmpl"]
    try:
        ret.__dict__['Current'] = p_dict["Current[A]"]
    except KeyError:
        ret.__dict__['Current'] = p_dict['SetPoint']
    ret.N = int(N)
    ret.__dict__['x'] = x * 10./2.**(20-1) * 40 * 10 # value in DAC units * Voltage range / 2^(bit count - 1 (account for positive vs negative)) * piezo constant * high voltage gain (hard coded for now)
    ret.__dict__['y'] = y * 10./2.**(20-1) * 40 * 10
    ret.__dict__['Length_x'] = p_dict["Length x[A]"]
    ret.__dict__['Length_y'] = p_dict["Length y[A]"]

    #hysteresis correction factor
    hyst = int(N / (ret.__dict__['Speclength'] * p_dict["LockinRC"] * LockinRC_factor * np.pi))
    ret.__dict__['hyst'] = hyst
