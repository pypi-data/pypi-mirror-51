...
class ParaSpec(NamedTuple):
    ...
    def body(text: str) -> 'ParaSpec':
        """
        Paragraph containing the body of the document. It's equivalent to `text`.
        """
        return ParaSpec(Paragraph.TYPE_BODY, str(text) if text is not None else '')

    @staticmethod
    def text(text: str) -> 'ParaSpec':
        """
        Paragraph containing regular text of the document. It's equivalent to `body`.
        """
        return ParaSpec.body(text)
