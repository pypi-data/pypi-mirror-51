...
class NoaaIsdWeather(AbstractOpenDataset):
...
at the same time! Please use either of them.')
            ...
            AbstractOpenDataset.__init__(self, worker=worker, partition_prep_func=prep_partition_year_month)
    ...
    def get(dataset: Dataset, enable_telemetry: bool = True):
        ...
        if enable_telemetry:
            AbstractOpenDataset.log_get_operation(isd.worker)
        return isd
