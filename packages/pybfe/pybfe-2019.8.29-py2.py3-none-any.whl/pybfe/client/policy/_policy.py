...
import json
from typing import (
    Dict, List, Optional, TYPE_CHECKING, Text
)
...
from pybfe.datamodel.policy import Assert, Policy, Test
    ...
    for test_dict in test_dicts:
        ...
        test_details_dict = json.loads(
            session.get_snapshot_object_text(test_details_rsc,
                                             snapshot=snapshot))
        asserts = [Assert(
            assert_dict.get('name'),
            assert_dict.get('status'),
            assert_dict.get('message'),
            assert_dict.get('expected'),
            assert_dict.get('actual'),
            assert_dict.get('key_present')) for assert_dict in
            test_details_dict.get('assertions')]
        test_name = test_dict.get('name')
        tests.append(Test(test_name, asserts))

    p = Policy(policy_name, policy_id, tests=tests, timestamp=timestamp)
