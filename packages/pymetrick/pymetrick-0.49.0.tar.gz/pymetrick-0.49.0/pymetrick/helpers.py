...
def timestamp2datetime(__timestamp__=None):
    ...
    try:
        t = ''
        if __timestamp__ is not None:
            if type(__timestamp__).__name__=='int' or type(__timestamp__).__name__=='float':
...
def stem(words=None,language='ES'):
    '''Con origen en una lista de palabras, se transforman en singular y plural, obteniendo una list con diferentes combinaciones (ES)'''
    try:
            ...
            for k in list(d.keys()):
                try:
                    if not d[k]:
                        ...
                        if type(d[k]).__name__ == 'dict' or type(d[k]).__name__ == 'list':
            ...
            for k in d:
                try:
                    if not k:
                        ...
                        if type(k).__name__ == 'dict' or type(k).__name__ == 'list':
                            k = remove_empty(k)
                            if not k:
                                # solicitar nueva comprobacion
                                empty_check = True
