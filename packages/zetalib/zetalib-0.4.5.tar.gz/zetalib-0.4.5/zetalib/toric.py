...
class ToricDatum(object):
    ...
    def simplify(self):
        ...
        while true:
            ...
            for i in idx_nonmon:  # or 'xrange(len(T.cc))', to be safe
                ...
                logger.debug('Condition %s || %s becomes...', lhs, rhs)
    ...
    def reduce(self, i, j, term_i=None, term_j=None, strict=False):
        ...
        logger.debug('Condition (%d) becomes %s || %s.', j, lhs[j], rhs[j])
        ...
        if (initials[i] is None) or (initials[j] is None):
            ...
            logger.debug('New intial form of (%d) is %s.', j, 'undetermined' if initials[j] is None else initials[j])
