...
class LaurentPolynomial(object):
    ...
    def is_term(self):
        """
        Return if ``self`` exponents list is a singleton.
        """
    ...
    def __repr__(self):
        return "LaurentPolynomial(" + repr(self.f) + ")"
...
class LaurentIdeal(object):
    ...
    def __init__(self, gens, initials=None, RS=None, ring=None, normalise=False):
        ...
        mins = util.minimal_elements(list(range(len(new_gens))), lte)
    ...
    def simplify(self):
        ...
        while changed:
            ...
            gens = I.gens[:]
            initials = I.initials[:]
            for i,j in itertools.product(list(range(len(gens))), repeat=2):
                if i == j:
                    continue
    ...
    def balance(self, strategy='min'):
        ...
        if self.is_balanced():
            yield self
            return
        idx, weights = list(zip(*[(i, len(self.gens[i].exponents())) for i in range(len(self.gens)) if self.initials[i] is None]))
