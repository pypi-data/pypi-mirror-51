...
class SCS(object):
    ...
    def _get_page_products(self, soup):
        ...
        if type(soup) is not NoneType:
            for product in soup:
                product = product.find("div", {"class": "main_box"})
                ...
                if valid:
                    ...
                    name = desc.find("a").get_text()
                    image = "https:" + product.find("img", {"class": "swapping-image"})["src"]

                    product_url = "https://speedcubeshop.com" + desc.find("a")["href"]
                    price = desc.find("span", {"class": "money"}).get_text()

                    try:
                        reviews_wrapper = product.find("div", {"class": "reviews__wrapper"})
                        review_score = reviews_wrapper.find("span", {"class": "spr-badge"})["data-id"]
                        review_count = reviews_wrapper.find("span", {"class": "spr-badge-caption"}).get_text()
                    except TypeError:
    ...
    def get_category(self, cat="", page=1):
        ...
        try:
            products_soup = soup.find("div", {"class": "product_c"}).find("div", {"class": "row"}).find_all("div")
    ...
    def search(self, query, page=1):
        ...
        # Find all the products on the page
        try:
            products_soup = soup.find("div", {"class": "product_c"}).find_all("div", {"class": "row"})[1].find_all("div")
        except AttributeError:
            return None
    ...
    def get_addresses(self):
        ...
        Get all saved addresses from the Address Book.

        Returns:
        list - A list of Address classes (scsapi.address.Address).
        Each instance will have the functions 
        """
        url = "https://speedcubeshop.com/account/addresses"
        
        r = self._s.get(url, headers=self._headers)
        soup = BeautifulSoup(r.content, "html.parser")

        addresses_soup = soup.find("div", {"class": "box-address-book"}).find_all("div", {"class": "clearfix"})

        addresses = []

        for address in addresses_soup:
            # Get the address ID for editing
            address_id = address.find("div")
            if type(address_id) is not type(None):
                address_id = address_id["id"].strip("edit_address_")

                info = address.find("p")

                name = info.find("strong").get_text()
    ...
    def get_order_history(self):
        ...
        orders_soup = soup.find("div", {"class": "box-order-history"}).find("table").find("tbody").find_all("tr")
