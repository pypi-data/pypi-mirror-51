...
class LocationFileLine(object):
    ...
    def __init__(self, arguments, processor_factory=None):
        ...
        self.line_crc32 = arguments.get('line_crc32')
    ...
    def _find_line_using_crc(self, aug, filepath, file_contents):
        ...
        line_crc32 = hex(zlib.crc32(lines[self.lineno - 1]) & 0xffffffff)
        if line_crc32 == self.line_crc32:
            return self.lineno

        if self.line_unique:
            crc32s = [hex(zlib.crc32(line) & 0xffffffff) for line in lines]
            firstIndex = None
            secondFound = False
