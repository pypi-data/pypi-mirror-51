...
def updateQuarchpy(versionNumber):
    ...
    if versionNumber !=None:
        ...
        (bytes(subprocess.check_output(['pip', 'install', 'quarchpy'+versionNumber], stderr=subprocess.STDOUT)).decode())
    else:
        (bytes(subprocess.check_output(['pip', 'install', 'quarchpy', '--upgrade'], stderr=subprocess.STDOUT)).decode())
