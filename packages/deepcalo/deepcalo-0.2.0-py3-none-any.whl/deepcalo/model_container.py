...
class ModelContainer:
    ...
    def _plot_models(self):
        ...
        if 'images' in self.data['train'] and boolify(self.data['train']['images']):
            ...
            # Network in network
            if self.params['network_in_network']['use']:
                ks.utils.plot_model(self.network_in_network, to_file=self.dirs['fig'] + 'architecture_network_in_network.pdf', show_shapes=True)
            ...
            # Gate net
            if any(img_name.startswith(self.gate_img_prefix) for img_name in self.data['train']['images']):
                ks.utils.plot_model(self.gate_net, to_file=self.dirs['fig'] + 'architecture_gate_net.pdf', show_shapes=True)
