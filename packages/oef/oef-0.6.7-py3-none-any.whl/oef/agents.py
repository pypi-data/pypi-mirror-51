...
CFP_TYPES = bytes
PROPOSE_TYPES = Union[bytes, List[dict]]

Query = dict
Description = dict

SearchResultItem = dict
...
class OEFAgent(OefMessageHandler):
    ...
    def incomingAgentMessage(self, agentMessage, connection_name=None, conn=None, **kwargs):
        ...
        for label, k,breaker in [
            ('wide', 'agentsWide', self.break_SearchResultWide),
        ...
        if not r:
            print("OefAgent[{}].incoming:unknown:".format(id(self)), connection_name, agentMessage)
            print(agentMessage)
            return False
        else:
            return True
...
# breakers
    ...
    def break_SearchResultWide(self, data):
        items = [
            ValueBag([
                ('public_key', 'key', ),
                ('core_key',   lambda x: None, ),
                ('core_addr',  'ip', ),
                ('core_port',  'port', ),
                ('distance',   'distance', ),
            ])
            for x
            in data.get('agentsWide', {}).get('result', [])
        ]
