...
def convert_oag_status_to_normal_form(status):
    if ['scheduled', 'proposed'] in status:
        return 'scheduled'
    elif ['outgate', 'inair'] in status:
        return 'en route'
    elif ['landed', 'ingate'] in status:
        return 'arrived'
    else:
        return status[1]
...
def parse_od_scheduled_timestamp(self, lookup_key, flight):
...
def parse_od_actual_timestamp(lookup_key, flight):
    actual_local_gatetime, actual_local_runway= None, None
    ...
    return actual_local_runway, actual_local_gatetime
