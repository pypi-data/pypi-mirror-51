...
DEFAULT_CONF = '%s/../etc/log.toml' % os.path.dirname(os.path.abspath(__file__))
_LOG_CONFIG = {}
_LOGGER_MAP = {}
...
def init_logger(logger_key='common'):
    ...
    :return:
    """
    for k in _LOG_CONFIG.get('log'):
        log_conf = _LOG_CONFIG.get('log').get(k)
...
def generate_sample_config(filename):
    ...
    shutil.copy(DEFAULT_CONF, file_path)
