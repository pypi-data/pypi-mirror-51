...
class Microservices(BaseClass, ABC):
    ...
    async def _message(self, msg):
        ...
        try:
            ...
            await self.__response(msg, error.errorCode, nats_route=msg.get('uuid', None)) # Enviar el error
        except Exception as e:
            self.logs.error(e)
            await self.__response(msg, -99, nats_route=msg.get('uuid', None)) # Enviar el error
