    ...
    def compute_sensor_response(self, channels=[]):
        ...
        for channel in channels:
            ...
            if np.max(deriv) * self.to_per_minute < self.coeff_thresh_ignore_peak:
                log.debug('[compute_sensor_response] insufficient rate ({} na/sample). ignored'.format(np.max(deriv)))
                continue
            ...
            if t_rt < 0:
                log.debug('[compute_sensor_response] not a real step. ignoring.')
                continue
            ...
            log.debug('[compute_sensor_response] [channel={}] step analyzed, rt={:04.1f}m, dI={:05.2f}'.format(
...
class PSenseResponse(object):
    ...
    def find_steps_in_signal(self, signal, dsignal=None, numsteps=5, samplewindow=8):
        ...
        log.info('[find_steps_in_signal] data content (channel {}): range({}, {})'.format(
            signal.name, signal_max, signal_min))
        log.debug('[find_steps_in_signal] estimates based on {} steps'.format(numsteps))
        log.debug('[find_steps_in_signal] thresholds: {:05.2f}, {:05.2f}, {:05.2f} nA'.format(
        ...
        log.debug('[find_steps_in_signal] found {} possible steps'.format(len(start_tm)))
    ...
    def run(self, channels=[], tm_start=[], tm_finish=[], numsteps=5, samplewindow=8):
        ...
        for channel in channels:
            log.info('[run] analysis starting for channel {}'.format(channel))
            if len(tm_start) > 0:
                ...
                try:
                    ...
                    log.info('[run] find_steps_in_signal() failed. skipping sensor.')
                    continue

            self.result[channel] = pd.DataFrame()
            log.info('[run] checking steps for sensor response')
            for start, finish in zip(tm_start, tm_finish):
                ...
                if sr.is_valid():
                    log.debug('[run] adding step to result')
