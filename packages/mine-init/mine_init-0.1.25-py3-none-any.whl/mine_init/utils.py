...
JAVA_PARAMS = {
    'memory': os.getenv('JAVA_MEM', '4096'),
    'fml_confirm': os.getenv('FML_CONFIRM', True)
}
...
class JavaRunner:
    ...
    def signal_handler(self, signal, frame):
        print('SIGINT Recieved, stopping children.')
        self.logger.info('Asking Minecraft to stop.')
        self.server.communicate(input='/stop')
        if self.server.returncode is None:
            self.logger.critical('Server is not stopping gracefully. Killing with fire...')
            self.server.kill()
        sys.exit(0)
