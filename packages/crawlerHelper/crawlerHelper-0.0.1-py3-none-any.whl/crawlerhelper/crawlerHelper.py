...
class crawlerComponent:
    ...
    def __init__(self):
        ...
        self.todayDate = str(dateStamp[0]) + str(dateStamp[1]) + str(dateStamp[2])

        # create cache file if not exist
        if not os.path.exists('ipCache.pkl'):
            # creat new cache file
            f = open('ipCache.pkl','w')
            f.close()
            self.updateIpLib()
            ...
            # if cache create data is equal to that of today, else update
            if cache['status'] == self.todayDate:
                self.ipLib = cache['ipLib']
            else:
                self.updateIpLib()
    ...
    def updateIpLib(self):
        ...
        with open('ipCache.pkl','wb') as f:
            pickle.dump(obj={'status':self.todayDate,'ipLib':ipLib},file=f)

        self.ipLib = ipLib

    def get_a_header(self):
        '''
