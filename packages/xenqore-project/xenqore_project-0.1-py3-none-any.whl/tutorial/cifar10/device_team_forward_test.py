...
# weight, bn, fc load
weight_0_path = os.path.join('DeviceSavePath/conv0W.txt')
weight_1_path = os.path.join('DeviceSavePath/conv1W.txt')
weight_2_path = os.path.join('DeviceSavePath/conv2W.txt')
weight_3_path = os.path.join('DeviceSavePath/conv3W.txt')
weight_4_path = os.path.join('DeviceSavePath/conv4W.txt')
weight_5_path = os.path.join('DeviceSavePath/conv5W.txt')
bn_0_path = os.path.join('DeviceSavePath/b0_BNFb.txt')
bn_1_path = os.path.join('DeviceSavePath/b1_BNFb.txt')
bn_2_path = os.path.join('DeviceSavePath/b2_BNFb.txt')
bn_3_path = os.path.join('DeviceSavePath/b3_BNFb.txt')
bn_4_path = os.path.join('DeviceSavePath/b4_BNFb.txt')
bn_5_path = os.path.join('DeviceSavePath/b5_BNFb.txt')
bn_6_path = os.path.join('DeviceSavePath/b6_BNFb.txt')
fc_path = os.path.join('DeviceSavePath/fc0W.txt')
fcf_path = os.path.join('DeviceSavePath/fc0Wf.txt')
...
fcf = fcf.reshape((256,10), order='F')
...
for i in range(test_step):    
    output = compute_conv(x_valid[i*batch_size:i*batch_size+batch_size], weight_0, bn=bn_0)
    output = np.sign(np.sign(output) + 1e-8)
    output = compute_conv(output, weight_1, bn=bn_1)
    output = np.sign(np.sign(output) + 1e-8)
    output = MaxPooling2D()(np.copy(output))

    output = compute_conv(output, weight_2, bn=bn_2)
    output = np.sign(np.sign(output) + 1e-8)
    output = compute_conv(output, weight_3, bn=bn_3)
    output = np.sign(np.sign(output) + 1e-8)
    output = MaxPooling2D()(np.copy(output))

    output = compute_conv(output, weight_4, bn=bn_4)
    output = np.sign(np.sign(output) + 1e-8)
    output = compute_conv(output, weight_5, bn=bn_5)
    output = np.sign(np.sign(output) + 1e-8)
    output = MaxPooling2D()(np.copy(output))
