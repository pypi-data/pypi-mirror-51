...
class RandomDataGenerator(object):
    ...
    def get_random_genomic_locations(
        size, width_mean=500, width_std=400, min_width=300, genome_assembly="hg38"
    ):
        from ngs_toolkit.utils import bed_to_index

        chrom = (pd.Series(["chr"] * size) + pd.Series(np.random.randint(1, 19, size)).astype(str)).tolist()
    ...
    @staticmethod
    def get_random_genes(size, genome_assembly="hg38"):
...
def generate_project(
    ...
    **kwargs
    ...
    if sample_input_files:
        generate_sample_input_files(an)
...
def generate_projects(
        ...
        n_replicates=[1, 3, 5]):
    import tempfile
    from ngs_toolkit import ATACSeqAnalysis, RNASeqAnalysis
            ...
            for factors in n_factors:
                for variables in n_variables:
                    for replicates in n_replicates:
                        ...
                        pep_config = os.path.join(
                            output_path, project_name, "metadata", "project_config.yaml")

                        # project and associated analysis
                        if data_type == "ATAC-seq":
                            analysis = ATACSeqAnalysis(from_pep=pep_config)
                        elif data_type == "RNA-seq":
                            analysis = RNASeqAnalysis(from_pep=pep_config)
                        # make sure the object is loaded with its dataframes
                        analysis.load_data(permissive=True)
...
def generate_bam_file(peak_set, output_bam, genome_assembly="hg38", chrom_sizes_file=None, index=True):
    ...
    i = [i for i, c in enumerate(r) for _ in range(c)]
    s = s.reindex(i)
