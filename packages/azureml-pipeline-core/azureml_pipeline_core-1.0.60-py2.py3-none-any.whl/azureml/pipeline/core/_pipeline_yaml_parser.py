...
_ADLA_COMPUTE_NAME = 'adla_compute_name'
...
_DATABRICK_STEP = 'databrick_step'
_DATAPATH = 'datapath'
...
class _PipelineYamlParser(object):
        ...
        for step_name in steps_section:
            ...
            if _DATATRANSFER_STEP in step:
                ...
                step_object = DataTransferStep(
                    ...
                    compute_target=datatransfer_parameters[_ADLA_COMPUTE_NAME],
                    allow_reuse=common_parameters[_ALLOWREUSE])

            if _DATABRICK_STEP in step:
                input_list = list(inputs.values())
                output_list = list(outputs.values())

                common_parameters = _PipelineYamlParser._get_step_parameters(step, _DATABRICK_STEP)
    ...
    def _get_step_parameters(step, step_type):
        ...
        if step_type in [_DATATRANSFER_STEP, _DATABRICK_STEP, _AZUREBATCH_STEP] and _NAME not in step_section:
    ...
    def _get_datatransferstep_parameters(step):
        ...
        if _ADLA_COMPUTE_NAME in step_section:
            data_factory_compute = step_section.get(_ADLA_COMPUTE_NAME, default=None)
        datatransferstep_param[_ADLA_COMPUTE_NAME] = data_factory_compute
    ...
    def _get_databrickstep_parameter(step):
        step_section = step[_DATABRICK_STEP]
        databrick_parameters = {}
        databrick_parameters.update(_PipelineYamlParser._get_check_yaml_keys(
            step_section, _DATABRICK_STEP, _STRING_KEYS, _yaml_keys.DatabrickStepYamlKeys.get(_STRING_KEYS)))
        databrick_parameters.update(_PipelineYamlParser._get_check_yaml_keys(
            step_section, _DATABRICK_STEP, _INTEGER_KEYS, _yaml_keys.DatabrickStepYamlKeys.get(_INTEGER_KEYS)))
