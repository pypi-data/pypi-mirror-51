...
# ---------------------------------------------------------
"""pipeline_draft.py, module for managing pipeline drafts."""
...
class PipelineDraft(object):
    ...
    Use PipelineDrafts to iterate on Pipelines. PipelineDrafts can be created from existing Pipelines or from scratch.

    .. remarks::

        A PipelineDraft can be created from a :class:`azureml.pipeline.core.Pipeline` by using the
        PipelineDraft.create() function. An example is below:
        ...
        To update a PipelineDraft use the update() method. The update() function of a pipeline draft can be used to
        update the name, description, experiment name, pipeline parameter assignments, continue on step failure setting
        and Pipeline associated with the PipelineDraft.
    ...
    def get(workspace, id, _workflow_provider=None, _service_endpoint=None):
        ...
        :param workspace: The workspace the PipelineDraft was created on.
        :type workspace: azureml.core.Workspace
        :param id: Id of the PipelineDraft.
        :type id: str
    ...
    def create(workspace, pipeline, name=None, description=None, experiment_name=None, pipeline_parameters=None,
        ...
        :param workspace: Workspace object this PipelineDraft will belong to.
        :type workspace: azureml.core.Workspace
        ...
        :param experiment_name: The experiment name for the PipelineDraft, only needed when creating from a
            :class:`azureml.pipeline.core.pipeline.Pipeline`.
        :type experiment_name: str
        :param pipeline_parameters: Optional pipeline parameter assignments for the PipelineDraft, only needed when
            creating from a :class:`azureml.pipeline.core.pipeline.Pipeline`.
        :type pipeline_parameters: dict[str, str]
        :param continue_on_step_failure: Optional, whether to continue PipelineRun when a step run fails setting for
        ...
        :param tags: Optional tags dictionary for the PipelineDraft, only needed when creating from a
            :class:`azureml.pipeline.core.pipeline.Pipeline`.
        :type tags: dict[str, str]
    ...
    def list(workspace, tags=None, _workflow_provider=None):
        """
        Get all pipeline drafts in the workspace.

        :param workspace: The workspace to list drafts on.
        :type workspace: azureml.core.Workspace
        :param tags:  If specified, returns drafts matching specified {*"tag"*: *"value"*}.
        :type tags: dict
        :param _workflow_provider: The workflow provider.
        :type _workflow_provider: _AevaWorkflowProvider object
        :return: a list of :class:`azureml.pipeline.core.pipeline_draft.PipelineDraft`
        :rtype: builtin.list
        """
        from azureml.pipeline.core._graph_context import _GraphContext
        graph_context = _GraphContext('placeholder', workspace,
