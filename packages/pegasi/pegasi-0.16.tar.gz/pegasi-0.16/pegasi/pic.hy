...
(defclass Dash[object]
  ...
  (defn --init--[self key &optional [driver "azure"]]
    (setv self.drivers {
                        "azure" "https://pcze2.azurewebsites.net/cloudpickle/" ;azure
         															"localhost" "http://0.0.0.0:8080/cloudpickle/"
                        "gcloud" "https://cloud-msrhohb4sa-ew.a.run.app/cloudpickle/" ;gcloud
                        })
    (setv self.key key
          
          self.api (get self.drivers driver)
          self.hash (hash key)
          ))
...
(defmain[]
  (setv d (Dash "eat those french fries and drink cola"))
  (print (d.legit))
  (print (d.run (fn[] (-> "https://api.myip.com/" requests.get (.json)))))
  
 	(print (d.map-a (fn[i](time.sleep 1) (time.time)) (list (range 0 1000)) :star False))
