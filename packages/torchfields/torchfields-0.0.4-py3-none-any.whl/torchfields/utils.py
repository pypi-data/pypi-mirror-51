...
import torch
class _BackPrintBounds(torch.autograd.Function):
    """Print the bounds of the gradient in the backward pass"""
    @staticmethod
    ...
    def backward(ctx, grad_output):
        print(ctx.name, "grad (min, max, #NaN):",
              grad_output.min().item(), grad_output.max().item(),
              (~torch.isfinite(grad_output)).sum().item())
        return grad_output, None


_bprint = _BackPrintBounds.apply
