...
def generate_a_waveform(m1, m2, spin1, spin2, ecc, lambda1, lambda2, iota, phiRef):
...
def generate_a_waveform_from_mcq(mc, q, spin1, spin2, ecc, lambda1, lambda2, iota, phiRef):
...
def least_match_waveform_unnormalized(paramspoints, known_bases, npts, deltaF, distance, waveFlags, approximant):
...
def least_match_quadratic_waveform_unnormalized(paramspoints, known_quad_bases, npts, deltaF, distance, waveFlags, approximant):
    overlaps = numpy.zeros(npts)
    modula = numpy.zeros(npts)
    for i in numpy.arange(0,len(paramspoints)):
        paramspoint = paramspoints[i]
...
def bases_searching_results_unnormalized(npts, nparams, nbases, known_bases, basis_waveforms, params, residual_modula, params_low, params_high, deltaF, distance, waveFlags, approximant):
    for k in numpy.arange(0,nbases-1):
        print("Linear Iter: ", k)
        params_points = generate_params_points(npts, nparams, params_low, params_high)
        basis_new, params_new, rm_new= least_match_waveform_unnormalized(params_points, known_bases, npts, deltaF, distance, waveFlags, approximant)
...
def bases_searching_quadratic_results_unnormalized(npts, nparams, nbases_quad, known_quad_bases, basis_waveforms, params_quad, residual_modula, params_low, params_high, deltaF, distance, waveFlags, approximant):
    for k in numpy.arange(0,nbases_quad-1):
        print("Quadratic Iter: ", k)
        params_points = generate_params_points(npts, nparams, params_low, params_high)
        basis_new, params_new, rm_new= least_match_quadratic_waveform_unnormalized(params_points, known_quad_bases, npts, deltaF, distance, waveFlags, approximant)
...
def initial_basis(approximant, mc_low, mc_high, q_low, q_high, s1sphere_low, s1sphere_high, \
                  s2sphere_low, s2sphere_high, ecc_low, ecc_high, lambda1_low, lambda1_high,\
                 iota_low, iota_high, phiref_low, phiref_high):
    if approximant==72: 
        nparams = 10
        ...
        hp1 = generate_a_waveform_from_mcq(mc_low, q_low, spherical_to_cartesian(s1sphere_low), spherical_to_cartesian(s2sphere_low), 0, 0, 0, iota_low, phiref_low)
    if approximant==6: 
        nparams = 11
        ...
        hp1 = generate_a_waveform_from_mcq(mc_low, q_low, spherical_to_cartesian(s1sphere_low), spherical_to_cartesian(s2sphere_low), ecc_low, 0, 0, iota_low, phiref_low)
    if approximant==73: 
        nparams = 12
        ...
        hp1 = generate_a_waveform_from_mcq(mc_low, q_low, spherical_to_cartesian(s1sphere_low), spherical_to_cartesian(s2sphere_low), 0, lambda1_low, lambda2_low, iota_low, phiref_low)
...
def surroerror(tolerance, ndim, inverse_V, emp_nodes, known_bases, deltaF, test_mc, test_q, test_s1, test_s2, test_ecc, test_lambda1, test_lambda2, test_iota, test_phiref):
    hp_test = generate_a_waveform_from_mcq(test_mc, test_q, test_s1, test_s2, test_ecc, test_lambda1, test_lambda2, test_iota, test_phiref)
...
def surros(tolerance, ndim, inverse_V, emp_nodes, known_bases, deltaF, nts, nparams, params_low, params_high):
    ...
    for i in numpy.arange(0,nts):
        ...
        surros[i] = surroerror(tolerance, ndim, inverse_V, emp_nodes, known_bases[0:ndim], deltaF, test_mc, test_q, test_s1, test_s2, test_ecc, test_lambda1, test_lambda2, test_iota, test_phiref)
...
def roqs(tolerance, ndimlow, ndimhigh, ndimstepsize, known_bases_copy, deltaF, nts, nparams, params_low, params_high):
    for num in np.arange(ndimlow, ndimhigh, ndimstepsize):
        ndim, inverse_V, emp_nodes = empnodes(num, known_bases_copy)
        if surros(tolerance, ndim, inverse_V, emp_nodes, known_bases_copy, deltaF, nts, nparams, params_low, params_high)==0:
...
def testrep(b_linear, emp_nodes, test_mc, test_q, test_s1, test_s2, test_ecc, test_lambda1, test_lambda2, test_iota, test_phiref):
    hp_test = generate_a_waveform_from_mcq(test_mc, test_q, test_s1, test_s2, test_ecc, test_lambda1, test_lambda2, test_iota, test_phiref)
...
def surroerror_quad(tolerance_quad, ndim_quad, inverse_V_quad, emp_nodes_quad, known_quad_bases, deltaF, test_mc_quad, test_q_quad, test_s1_quad, test_s2_quad, test_ecc_quad, \
                    test_lambda1_quad, test_lambda2_quad, test_iota_quad, test_phiref_quad):
    hp_test_quad = (numpy.absolute(generate_a_waveform_from_mcq(test_mc_quad, test_q_quad, test_s1_quad, test_s2_quad, test_ecc_quad, test_lambda1_quad, test_lambda2_quad, test_iota_quad, test_phiref_quad)))**2
    Ci_quad = numpy.dot(inverse_V_quad, hp_test_quad[emp_nodes_quad])
    interpolantA_quad = numpy.zeros(len(hp1_quad))+numpy.zeros(len(hp1_quad))*1j    
...
def surros_quad(tolerance_quad, ndim_quad, inverse_V_quad, emp_nodes_quad, known_quad_bases, deltaF, nts, nparams, params_low, params_high):
    test_points = generate_params_points(nts, nparams, params_low, params_high)
    surros = numpy.zeros(nts)
    count = 0
    for i in numpy.arange(0,nts):
        ...
        surros[i] = surroerror_quad(tolerance_quad, ndim_quad, inverse_V_quad, emp_nodes_quad, known_quad_bases[0:ndim_quad], deltaF, test_mc_quad, test_q_quad, test_s1_quad, test_s2_quad, test_ecc_quad, test_lambda1_quad, test_lambda2_quad, test_iota_quad, test_phiref_quad)
...
def roqs_quad(tolerance_quad, ndimlow_quad, ndimhigh_quad, ndimstepsize_quad, known_quad_bases_copy, deltaF, nts, nparams, params_low, params_high):
    for num in np.arange(ndimlow_quad, ndimhigh_quad, ndimstepsize_quad):
        ...
        if surros_quad(tolerance_quad, ndim_quad, inverse_V_quad, emp_nodes_quad, known_quad_bases_copy, deltaF, nts, nparams, params_low, params_high)==0:
...
def testrep_quad(b_quad, emp_nodes_quad, test_mc_quad, test_q_quad, test_s1_quad, test_s2_quad, test_ecc_quad, test_lambda1_quad, test_lambda2_quad, test_iota_quad, test_phiref_quad):
    hp_test_quad = (numpy.absolute(generate_a_waveform_from_mcq(test_mc_quad, test_q_quad, test_s1_quad, test_s2_quad, test_ecc_quad, test_lambda1_quad, test_lambda2_quad, test_iota_quad, test_phiref_quad)))**2
