...
class DefaultClusterGateway(ClusterGateway):
    ...
    def __init__(self, node_manager: NodeManager, connection_group=None) -> None:
        super().__init__()
        self.node_manager = node_manager
        self.group = connection_group or self._create_group()

    def _on_node_created(self, event: NodeCreatedEvent):
        print('node created', event)  # TODO
        pass

    def _on_node_removed(self, event: NodeRemovedEvent):
        print('node removed', event)  # TODO
        pass

    def _on_node_updated(self, event: NodeUpdatedEvent):
        print('node updated', event)  # TODO
        pass

    def _create_group(self) -> ConnectionGroup:
        logger.debug('creating connection group for nodes %s', self.nodes)
        return ThreadedConnectionGroup([create_connection(node) for node in self.nodes])

    def suspend(self, nodes: List[str] = None):
        raise NotImplementedError  # TODO implement suspend
    ...
    def shutdown(self, nodes: List[str] = None):
        if nodes is not None:
            raise NotImplementedError  # TODO: implement shutdown for specific nodes

        try:
            self.run('shutdown -h now')
        except GroupExecutionException:
            # TODO: find a better way to shut down nodes
            pass

    def boot(self, nodes: List[str] = None):
        macs = [node.mac for node in self.select_nodes(nodes)]
    ...
    def run(self, *args, **kwargs) -> GroupResult:
        """
        Runs a command on all configured nodes using a fabric ThreadGroup.
        :return: a fabric GroupResult object
        """
        return self.group.run(*args, **kwargs)
