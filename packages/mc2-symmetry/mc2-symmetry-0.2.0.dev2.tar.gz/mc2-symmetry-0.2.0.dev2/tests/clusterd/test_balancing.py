...
from symmetry.clusterd.policies.balancing import RoundRobin, WeightedRandom
...
class WeightedRandomPolicyIntegrationTest(PolicyIntegrationTest):
    ...
    def test_update_weighted_random_policy(self):
        ...
        try:
            ...
            self.assertEqual('WeightedRandom', policy.name)
            self.assertEqual(1, len(routes), 'expected one route to be set, was %s' % routes)
