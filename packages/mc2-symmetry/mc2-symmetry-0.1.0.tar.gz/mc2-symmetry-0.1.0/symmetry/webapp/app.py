...
def setup(api, context: AppContext):
    ...
    api.add_route('/api/nodes', NodesResource(node_manager))
    api.add_route('/api/nodes/{node}', NodeResource(node_manager))
    api.add_route('/api/nodes/{node}/state', NodeStateResource(cluster))

    api.add_route('/api/nodes/{node}/boot', NodeBootResource(cluster))
    api.add_route('/api/nodes/{node}/shutdown', NodeShutdownResource(cluster))
    ...
    api.add_route('/api/cluster/boot', ClusterBootResource(cluster))
    api.add_route('/api/cluster/shutdown', ClusterShutdownResource(cluster))
    ...
    working_dir = config.guess_root_path()
...
class NodeResource:
    ...
    def on_delete(self, req, resp, node):
        node_id = node
...
class NodeStateResource(ClusterAwareResource):
    def on_get(self, req, resp, node):
...
class NodeBootResource(ClusterAwareResource):
    def on_post(self, req, resp, node):
        node_info = self.require_node(node)
        self.cluster.boot([node_info])


class NodeShutdownResource(ClusterAwareResource):
    def on_post(self, req, resp, node):
        node_info = self.require_node(node)
        self.cluster.shutdown([node_info])


class ClusterBootResource(ClusterAwareResource):

    def on_get(self, req, resp):
        macs = self.cluster.boot()
        resp.media = macs


class ClusterShutdownResource(ClusterAwareResource):
    def on_get(self, req, resp):
