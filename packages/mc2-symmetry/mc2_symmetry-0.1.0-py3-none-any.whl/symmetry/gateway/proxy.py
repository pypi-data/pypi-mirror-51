...
from symmetry.gateway import Router, ServiceRequest, SymmetryServiceRouter, WeightedRandomBalancer
...
sink = ServiceProxy(
    SymmetryServiceRouter(WeightedRandomBalancer(RedisRoutingTable(redis.Redis(decode_responses=True)))))
api.add_sink(sink, '/')
