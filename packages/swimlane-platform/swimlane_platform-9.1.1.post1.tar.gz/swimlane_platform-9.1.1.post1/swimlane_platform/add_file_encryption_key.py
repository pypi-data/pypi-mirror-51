...
from swimlane_platform.lib import ArgsConfigQuestions, names, DockerComposeFileManager, \
    Configuration, BaseWithLog, info_function_start_finish
...
class AddFileEncryption(BaseWithLog):
    ...
    def __init__(self, args):
        ...
        super(AddFileEncryption, self).__init__(args)
    ...
    def run(self):
        ...
        key = str(uuid4().hex) if self.args.autogenerated_file_encryption_key else self.args.file_encryption_key
...
def run(parent_config):
    ...
    add_file_encryption_key = AddFileEncryption(config.to_arguments())
    add_file_encryption_key.run()
