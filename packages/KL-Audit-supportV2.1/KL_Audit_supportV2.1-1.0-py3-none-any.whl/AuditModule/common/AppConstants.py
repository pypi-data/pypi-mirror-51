...
class CassandraConstants:
    ...
    CREATE_STATEMENT = "CREATE TABLE IF NOT EXISTS {}( user_name text, timedate TIMESTAMP, id int, client_id text, module text, " \
                       "operations text, parameter_lable text, status text, client_ip text, server_ip text, time_zone text, " \
                       "PRIMARY KEY ((user_name,client_id,module, operations, parameter_lable, status, client_ip, server_ip, time_zone), timedate) )" \
                       "WITH CLUSTERING ORDER BY (timedate DESC);"
    ...
    INSERT_STATEMENT = "insert into {0}(user_name, timedate, id, client_id, module, operations, parameter_lable, status, client_address, server_address, host_name, time_zone) values(?,?,?,?,?,?,?,?,?,?,?,?)"
    INSERT_DBUPDATE_STATEMENT = "INSERT INTO audit_dbupdates (user_name, timedate, client_id, module, operations, parameter_lable, status, user_role_name) values (?,?,?,?,?,?,?,?)"
