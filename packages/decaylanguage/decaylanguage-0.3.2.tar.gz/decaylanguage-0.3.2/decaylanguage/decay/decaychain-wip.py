...
"""
DaughtersDict - dict of daughters, for counting

DecayMode.bf
         .daughters = DaughtersDict
         .to_dict()

- an actual particle decay = mother + DecayMode
...
class DecayChain(object):
    ...
    def __repr__(self):
        ...
        return "<{self.__class__.__name__}: {mother} -> {daughters}, BF={bf}>".format(
                self=self,
                mother=self.metadata['mother'],
                daughters=self.metadata['daughters'],
                bf=self.metadata['bf'])
