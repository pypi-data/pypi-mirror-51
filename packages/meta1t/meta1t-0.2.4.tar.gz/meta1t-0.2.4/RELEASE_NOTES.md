...
* `meta1t.storage` now comes with the prebuilt inRam and SQLite stores from pygraphene
...
* Instead of using
  ...
  ```
  from meta1t.storage import configStorage as config
  ```
  ...
  ```
  from meta1t.storage import get_default_config_store
...
* bd69b2c Fix meta1t->blockchain attrribute #132
* a19b9ed Fix #136
