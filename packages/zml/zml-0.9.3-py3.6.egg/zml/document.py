...
def get_document(source, path=None, get_params=None, post_params=None):
    backend = Backend()
    request = Request(path=path, get_params=get_params, post_params=post_params)
    document = Document(request=request)
    root = backend.get_document(document, address=source)
    return root.render()
...
class Document(DocumentRenderingContext):
    ...
    def __init__(self, filename=None, source=None, lookup=None,
                 ...
                 inherited_document=None, request=None,
                 *args, **kwargs):
    ...
    def render(
        ...
        if zmlsource:
            result = self.render_source(zmlsource, self.local_context,
                                 path=path, get_params=get_params,
                                 post_params=post_params, base_indent=base_indent)
            return result
        else:
    ...
    def render_source(self, source=None,
        ...
        if self.path:
            self.dispatch_routes(self.path)
...
class DocumentLookup(object):
    ...
    def get_document(self, filename, local_context={}, global_context={}, request=None):
...
def render_source(source,
                ...
                post_params={},
                base_indent=0):
    request = Request(path=path, get_params=get_params)
...
def render(tempsource, local_context={}, path=None,
    ...
    if tempsource.endswith('.zml'):
        return render_document(tempsource, local_context=local_context, global_context=global_context, path=path)
    else:
        return render_source(tempsource, local_context, global_context=global_context, base_indent=base_indent, path=path)
...
def render_document(documentfile, local_context={}, path=None,
    ...
    request = Request(path=path)
    t = lookup.get_document(documentfile, local_context=local_context, global_context=global_context, request=request)
    out = t.render(get_params=get_params, post_params=post_params, base_indent=base_indent)
    return out
...
def load(tempsource, local_context={}, path=None,
    ...
    if tempsource.endswith('.zml'):
        ...
        return import_source(tempsource, local_context, global_context=global_context, base_indent=base_indent, path=path)
...
def import_document(documentfile, local_context={}, path=None,
    ...
    t.import_document(path=path,
                   get_params=get_params, post_params=post_params, base_indent=base_indent)
    return t
